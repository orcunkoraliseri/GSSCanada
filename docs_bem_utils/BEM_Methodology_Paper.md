...
The simulation framework is organized into a `bem_utils/` Python package with the following modules:
...
For **neighbourhood simulations**, where the input IDF may lack occupancy schedules entirely, the optimizer includes a fallback mechanism: it searches for a Single Building IDF in `BEM_Setup/Buildings/*.idf`, parses the schedules for Lights, Equipment, and WaterUse, and uses them as the Default baseline. This ensures that neighbourhood Default-vs-Integrated comparisons rely on the same baseline physics as single-building comparisons.
...
**Implementation**: `LightingGenerator` class in `bem_utils/schedule_generator.py`.
...
**Implementation**: `PresenceFilter` class in `bem_utils/schedule_generator.py`.
...
**Implementation**: `PresenceFilter` class in `bem_utils/schedule_generator.py`, applied through `inject_schedules()` in `bem_utils/integration.py`.
