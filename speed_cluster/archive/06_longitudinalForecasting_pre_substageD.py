...
def run_substage_c(args, df: pd.DataFrame, device: torch.device) -> None:
    ...
    # ── Step 2: Pretext training of TrendEncoder (500 gradient steps) ──
    ...
    # ── Step 3: Joint training (W_2022_ft warm start) ──────────────────
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for batch in train_loader:
            ...
            # Recency weights per sample
            w = torch.tensor(
                [recency_weights.get(int(yr.item()), 1.0) for yr in batch["cycle_year"]],
                dtype=torch.float32, device=device,
            )  # (B,)

            # Primary loss: recency-weighted act CE + home BCE + cop BCE (hom_weight_2022=1.0)
            act_loss = (
                F.cross_entropy(
                    out["act_logits"].reshape(B * T, N_ACT),
                    batch["dec_act_seq"].reshape(B * T),
...
def run_substage_d(args, df: pd.DataFrame, device: torch.device) -> None:
    """
    Two-phase inference (HPC only).

    Phase i — 2022 Backcasting (validation gate):
        Load W_pooled_2030 + trend_encoder_2030.
        Generate reconstructed_2022_diaries.csv.
        Gate: JS divergence vs observed 2022 < 0.10 per stratum.

    Phase ii — 2030 Forward Forecast (deliverable):
        Load scenario_2030_features.csv from INPUTS_DIR.
        Generate 2030_synthetic_diaries.csv (≥37,000 rows).
        Generate 2030_drift_summary.csv (signed shifts 2022→2030 per activity).
    """
    pass
