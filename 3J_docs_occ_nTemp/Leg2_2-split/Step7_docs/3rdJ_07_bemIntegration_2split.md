...
| Property | Value |
|---|---|
| 2022 stock (`_excl`, post AT_HOME<0.30 exclusion) | ~29,538 linked persons |
...
| 2026-07-15 | **Doc-propagation note (correction, not a rewrite of history above)** | ℹ️ INFO | The 2026-06-26 entries above (rows for "2022 run complete" and "2030 run complete") correctly report **23,211 unique HH / 1,114,128 rows / stock assembled from 29,599 rows** as the state of the pipeline *at that time* — left as-is, not edited, per the append-only Progress Log rule. Those figures are now known to have been built on an internally-stale Step-5 `Full_Aggregated.csv` (its own `Full_Schedules.csv` had already been refreshed but the aggregate wasn't regenerated from it — see the root-cause writeup in `improvement/2J_to_3J_improvement_implementation.md`, Task-4-step-2 Progress Log entry, 2026-07-15). The corrected, verified frame is **23,150 unique HH / 1,111,200 rows / 29,538-row stock** — this is what the "Confirmed data characteristics" table above (§ "2022 stock (`_excl`...)") and all current-state prose in this doc now say, and what the fresh 2026-07-15 Step-7 re-run (Task 4 step 4) reproduced end-to-end. Row 434's own scorecard figures (32/0/0, 43/0/0) are unaffected by this correction — they were, and remain, the correct baseline. |
