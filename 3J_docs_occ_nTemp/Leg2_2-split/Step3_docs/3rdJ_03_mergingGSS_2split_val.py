# -*- coding: utf-8 -*-
"""
3rdJ_03_mergingGSS_2split_val.py

Step 3 Validator for Leg-2 (Residential + Office / AT_WORK) Occupancy Pipeline.

Ported from:  2J_docs_occ_nTemp/03_mergingGSS_val.py  (Leg-1)
Leg-2 additions (new relative to Leg-1 validator):
  - WORK30 channel checks (shape, alignment, values in {0,1}, diurnal curve per cycle)
  - AT_HOME vs AT_WORK overlap note (slots where both=1)
  - NOCS / NAICS / TELEWORK coverage in hetus_30min per cycle
  - All Path literals replaced with INPUT_DIR / OUTPUT_DIR variables
...
INPUT_DIR  = os.path.join(_LEG2_BASE, "Step2_docs", "outputs_step2")
OUTPUT_DIR = os.path.join(_LEG2_BASE, "Step3_docs", "outputs_step3")
...
|  AT_WORK tiling (Phase J) is a separate CSV -- purely additive.          |
|                                                                           |
|  Files produced:                                                          |
...
|  - work_30min.csv  (occID + WORK30_001..048)  [Leg-2 NEW]              |
+---------------------------------------------------------------------------+"""
...
class GSSMergeValidator2Split:
    """Validates Step 3 (Leg-2) merge, tiling & AT_WORK outputs."""
...
<head>
  ...
  <title>GSS Step 3 (Leg-2) -- Merge &amp; Tiling Validation</title>
  <style>
    :root {{
...
<body>
  <header>
    <div>
      <h1>GSS Step 3 (Leg-2) -- Merge &amp; Tiling Validation Report</h1>
      <p>Validates merged_episodes.csv, hetus_wide.csv, hetus_30min.csv,
         copresence_30min.csv, work_30min.csv &middot; Cycles 2005, 2010, 2015, 2022</p>
    </div>
  ...
  <main>
    <section class="pipeline-section" id="pipeline-overview">
      <h2>Pipeline Overview -- Step 3 (Leg-2): Merge &amp; Tiling</h2>
  ...
  <footer>
    Leg-2 Occupancy Modeling Pipeline &middot; Step 3 Merge &amp; Tiling Validation &middot;
    ...
    Generated: {ts}
        ...
        with open(out_txt, "w", encoding="utf-8") as f:
            f.write(f"Step 3 (Leg-2) Validation Report -- {ts}\n")
            f.write("=" * 60 + "\n")
    ...
    def run_all(self) -> None:
        _apply_dark()
        print("=" * 60)
        print("Step 3 (Leg-2) -- Merge & Tiling Validation")
        print("=" * 60)
        ...
        self.validate_work_channel()          # [Leg-2 NEW]
        self.validate_office_conditioning()   # [Leg-2 NEW]
...
if __name__ == "__main__":
    GSSMergeValidator2Split(INPUT_DIR, OUTPUT_DIR).run_all()
