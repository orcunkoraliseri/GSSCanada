...
Step 3 of the Leg-2 (Residential + Office / AT_WORK) Occupancy Modeling Pipeline:
Merge & Tiling.

Ported from:  2J_docs_occ_nTemp/03_mergingGSS.py  (Leg-1, verbatim)
Leg-2 deltas (ONLY changes to the Leg-1 version):

  A. Platform-detection path block  — Windows / cluster / Mac absolute paths;
     all hardcoded Path("outputs_step2")/Path("outputs_step3") literals replaced
     with INPUT_DIR / OUTPUT_DIR variables.

  B. Column additions through the merge:
     - MAIN_COMMON_COLS += "NAICS", "TELEWORK"  (office conditioning variables)
     - EPISODE_COMMON_COLS += "AT_WORK"          (right after AT_HOME)
     - build_hetus_wide() PERSON_COLS += "NAICS", "TELEWORK"  (model conditioning)

  C. Residential Phase F + Phase H = BIT-IDENTICAL.
     AT_WORK is NOT added to _build_slot_arrays / build_hetus_wide /
     downsample_to_30min — those functions are verbatim Leg-1.

  D. De-hardcoded row count.
     The template asserts df.shape[0] == 64_061 in several places.
     Replaced with dynamically captured N from hetus_wide row count.
     Asserts that hetus_30min, copresence_30min, and work_30min all share that
     same N and occID order.  Actual N printed; if it differs from 64,061
     a loud WARNING is emitted.

  E. NEW Phase J — AT_WORK tiling (the sole real build delta).
     tile_work_to_30min():
       - cloned from tile_copresence_to_30min (same 4 AM-origin slot math)
       - binary 1/0 majority vote (sum_work >= 2), NOT the co-presence 1/2 scheme
       - emits WORK30_001..WORK30_048 (Int8) + occID
       - writes outputs_step3/work_30min.csv
       - validate_work_30min() checks shape, alignment, values in {0,1},
         weighted AT_WORK presence rate per cycle, night-slot near-zero sanity

Everything else (Phases A–I) is ported verbatim from 2J_docs_occ_nTemp/03_mergingGSS.py.
...
Phases:
    ...
    J   : AT_WORK Tiling  (NEW — Leg-2 delta)
...
WINDOWS ENCODING NOTE:
    Run with  py -3 -X utf8 3rdJ_03_mergingGSS_2split.py
...
INPUT_DIR  = _LEG2_BASE / "Step2_docs" / "outputs_step2"
OUTPUT_DIR = _LEG2_BASE / "Step3_docs" / "outputs_step3"
...
def main() -> None:
    """Run the full Step 3 Leg-2 pipeline."""
    print("=" * 60)
    print("Step 3 (Leg-2) -- Merge, Tiling & AT_WORK Work-Channel")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("Step 3 (Leg-2) complete.")
    print("=" * 60)

    # Print final output file list
    print("\nOutput files:")
    for fname in [
        "merged_episodes.csv", "merged_episodes.parquet",
        "hetus_wide.csv", "hetus_30min.csv",
        "copresence_30min.csv", "work_30min.csv",
    ]:
        p = OUTPUT_DIR / fname
        if p.exists():
