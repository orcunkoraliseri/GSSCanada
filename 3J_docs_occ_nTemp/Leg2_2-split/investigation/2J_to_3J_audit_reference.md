...
| 2 | Un-raked activity channel (act30 in 2J; act30+office in 3J) | ✅ **FIXED 2026-07-15** (was: ⚠️ partially addressed) — occupancy jointly raked; activity now raked by 04T (3-way state-conditional). **The "61.12%" cited in the original audit is superseded — do not cite it** (see correction note below the table). | ~~🔴 HIGH~~ → closed |
| 3 | +4h diary→clock offset (`07_aug_to_bem.py`) | ✅ **Already fixed** (ported explicitly, dated 2026-06-08) | None — verify only |
| 4 | Multi-zone equipment/lighting injection bug (`integration.py`) | ✅ **FIXED 2026-07-15** (was: ❌ same bug confirmed present) — per-zone equip/lights/fridge carriers; verified MidRise 26 raw → 25 zones/25.0×, HighRise 50 Lights → 27 zones/27.0×, DetachedHouse → 1 zone/1.0×. `office_integration.py` deliberately NOT touched (out of scope, §0.2). | ~~🔴 HIGH~~ → closed |

> [!CAUTION]
> **⚠️ CORRECTION 2026-07-15 — the "61.12%" figure in this audit (Item 2, and §"Un-raked activity/office channel" below) must NOT be cited.** It was inherited from `Step4_docs/deepResearch/dr_S4-02_posthoc_calibration_raking_REPORT.md:70`, which measured it on a **disjoint 2,560-row Jun-18 diagnostic sample**, not the 128,122-synthetic-row `R5` sweep the pipeline uses (the `R5_raked/` dir it names does not exist in the repo).
>
> **Correct pre-04T baseline on the real pool (`R5_raked_mindwell/`): 50.24%** = **26.30% TELEWORK** (work activity with `hom30=1` — legitimate working-from-home, the paper's core signal) **+ 23.94% FLOATING** (work activity with `hom30=0 & wrk30=0` — physically impossible; the only real defect).
>
> The decomposition is the point: ~half the "mismatch" was never an error. This is why this audit's recommendation to consider the `dr_S4-02` hard-lock was **rejected** — it would have raked away the telework signal. The implemented fix (04T, 3-way state-conditional rake, after 04M) targets FLOATING only: **Gate A +20.98pp FAIL → +1.12pp PASS**, FLOATING **23.94% → 4.08%**, TELEWORK preserved at **16.65%** (observed 14.46%), `hom30`/`wrk30` byte-identical.
>
> Both 🔴 HIGH items in this audit (2 and 4) are now closed. See `improvement/2J_to_3J_improvement_implementation.md` for the implementation and its Progress Log.
...
1. ~~**Multi-zone injection bug** (Item 4)~~ — ✅ **DONE 2026-07-15.** Ported from 2J as predicted; per-zone carriers, zone-count restoration verified.
2. ~~**Un-raked activity/office channel, 61.12% discordance** (Item 2)~~ — ✅ **DONE 2026-07-15** via the 3-way conditional rake (04T). **The 61.12% figure is superseded — do not cite it; the real pre-fix baseline is 50.24%, of which only 23.94pp (FLOATING) is a defect and 26.30pp is legitimate TELEWORK.** See the CAUTION note under the issue table. Note this recommendation's fallback suggestion — *"the hard-lock priority rake already proposed in `dr_S4-02`"* — was **rejected**: it would have suppressed the telework signal, which is the paper's core finding. The 3-way conditional design was taken instead.
3. ~~**Frame-size consistency check** (Item 6)~~ — ✅ **DONE 2026-07-15, and it found something.** This audit concluded the 3J frame was "23,211 HH, no bug". **That was wrong.** The true post-exclusion frame is **23,150 HH / 29,538 rows / 735 excluded**; June's `Full_Aggregated.csv` was internally stale against its own `Full_Schedules.csv` (43.7% of rows disagreed), so the exclusion ran on stale aggregates. Verified by set identity (recomputing from June's own frozen schedules reproduces today's household set exactly, ∩ = 23,150, zero difference either way) and by per-agent `occID` identity across all 30,273 agents. Corrected across ~14 docs. **The instinct to "do this before any new simulation campaign" was exactly right** — the wrong frame had already propagated into the June Step-7 outputs as real data (61 households that should have been excluded were physically present) and fed the July re-sims.
4. Items 1, 3, 5 — no action needed, verification-only if desired.

**Status 2026-07-15: both 🔴 HIGH items closed; Item 6 closed with a correction. This audit's substantive conclusions held, with one exception — the frame item.**
