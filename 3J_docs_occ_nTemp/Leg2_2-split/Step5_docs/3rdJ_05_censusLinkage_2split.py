...
def run_linkage_smoke(sample_frac: float = 0.01) -> None:
    """1% Census sample + raked_sample pool (3,840 rows). Writes to outputs_step5/smoke/."""
    ...
    df_matched.drop(columns=["_pool_idx"]).to_csv(
        SMOKE_DIR / "3rdJ_25CEN_aug_Matched_Keys_smoke.csv", index=False
    )
    df_full.to_csv(SMOKE_DIR / "3rdJ_25CEN_aug_Full_Schedules_smoke.csv", index=False)

    print(f"\nSmoke outputs -> {SMOKE_DIR}")
    print("[smoke] DONE — both channels (hom30 + wrk30) + office_archetype_ID verified")
