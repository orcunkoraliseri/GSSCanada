...
class CensusLinkageValidator2CH:
    ...
    def validate_at_home_consistency(self) -> dict:
        ...
        self._rec(lv, f"2.2 | Per-slot max diff: {max_diff:.2f}pp — {fail_slots} slot(s) >+-3pp"
        ...
        self._rec(lv_24, f"2.4 | Slots 1-8 mean AT_HOME: {night_mean:.2f}% (>=85%)"
    ...
    def validate_at_work_consistency(self) -> dict:
        ...
        # W1: per-slot mean AT_WORK, all vs observed
        diff_wrk = np.abs(wrk_all - wrk_obs)
        max_diff_wrk = float(diff_wrk.max()) if len(diff_wrk) > 0 else float("nan")
        fail_wrk = int((diff_wrk > 3).sum()) if len(diff_wrk) > 0 else 0
        lv = "pass" if fail_wrk == 0 else ("warn" if self.is_smoke else "fail")
        self._rec(lv, f"W1 | AT_WORK per-slot max diff: {max_diff_wrk:.2f}pp — "
        ...
        if col_p and len(obs) > 0:
            ...
            self._rec(lv2, f"W3 | Colleagues co-presence: all={col_all:.2f}% "
                      f"obs={col_obs:.2f}% diff={max_col_diff:.3f}pp (<=3pp)")
        else:
    ...
    def validate_schedule_shape(self) -> dict:
        ...
        self._rec(lv_43, f"4.3 | Night sleep dominance: {sleep_rate:.2f}% (>=70%)"
