"""
3rdJ_05_censusLinkage_2split.py — Step 5: Census-GSS Slot-Native Demographic Matching
3J Leg-2 TWO-CHANNEL Pipeline (Residential AT_HOME + Office AT_WORK)

Adapts the 2J linkage (05_census_linkage.py) for:
  - 2025 Census (Aligned_Census_2025.csv), PID / SIM_HH_ID identifiers
  - Both channels: hom30 (AT_HOME) + wrk30 (AT_WORK) carried through
  ...
  - Extended BEM/validation for AT_WORK channel

CLI:
    py 3rdJ_05_censusLinkage_2split.py --smoke       # 1% Census sample + raked_sample pool
    py 3rdJ_05_censusLinkage_2split.py --full        # Full Census (30,274 agents) + full pool
    py 3rdJ_05_censusLinkage_2split.py --aggregate   # HH aggregation (Sub-step 5E)
    py 3rdJ_05_censusLinkage_2split.py --bem         # BEM prep (Sub-step 5F)
    py 3rdJ_05_censusLinkage_2split.py --regression  # Regression validation (Sub-step 5G)
    py 3rdJ_05_censusLinkage_2split.py --exclusion   # Exclusion filter (Sub-step 5H)
"""
...
SMOKE_POOL = (
    ...
    / "outputs_step4" / "raked_sample" / "augmented_diaries.csv"
)
FULL_POOL = (
    BASE / "3J_docs_occ_nTemp" / "Leg2_2-split" / "Step4_docs"
...
# Outputs
OUT_DIR = (
    BASE / "3J_docs_occ_nTemp" / "Leg2_2-split" / "Step5_docs" / "outputs_step5"
)
...
def expand_slot_schedules(
    ...
    _explicit_pool = (
        set(act_cols) | set(hom_cols) | set(wrk_cols)
...
def run_linkage_smoke(sample_frac: float = 0.01) -> None:
    """
    1% Census sample + raked_sample pool (3,840 rows).
    Writes to outputs_step5/smoke_rungI/ (isolated — does NOT clobber existing
    outputs_step5/smoke/ production outputs).
    Includes Rung-(i) W3 colleagues before/after diagnostic.
    """
    # Isolated output directory — avoids clobbering production smoke outputs
    smoke_out_dir = OUT_DIR / "smoke_rungI"
    ...
    # ── Save (isolated smoke_rungI dir — does NOT overwrite production smoke/) ──
    ...
    print("[smoke] DONE — both channels (hom30 + wrk30) + office_archetype_ID + Rung-I verified")
...
def run_aggregate() -> None:
    ...
    print(f"[5E] NOTE: wrk30 stays per-person (no HH aggregation of AT_WORK).")
    ...
    axes[0].set_title("Individual vs HH Occupancy + AT_WORK (30-min slots)")
...
def run_bem() -> None:
    ...
    axes[0].set_title("Mean AT_HOME + AT_WORK per 30-min Slot (3J 2-channel)")
    axes[0].set_xlabel("Slot")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Step 5: 3J 2-channel Census-GSS Linkage (25CEN, AT_HOME + AT_WORK)"
    )
    grp = parser.add_mutually_exclusive_group(required=True)
    grp.add_argument("--smoke",     action="store_true", help="1%% Census + raked_sample pool smoke run")
