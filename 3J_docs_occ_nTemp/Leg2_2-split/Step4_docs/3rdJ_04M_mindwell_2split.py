# -*- coding: utf-8 -*-
"""
3rdJ_04M_mindwell_2split.py

Minimum-dwell post-processor for Step-4 augmented diaries (Leg-2 two-channel).

Reads a raked augmented_diaries.csv (--in_csv), applies a minimum-dwell smoother
to the hom30_001..048 AND wrk30_001..048 binary columns, then writes the smoothed
CSV to --out_csv.  All other columns are copied unchanged.  act30_* is NOT touched.
...
Usage:
    py -3 3rdJ_04M_mindwell_2split.py \
        --in_csv  /path/to/raked/augmented_diaries.csv \
        --out_csv /path/to/out/augmented_diaries.csv \
        [--min_dwell 2]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Minimum-dwell post-processor for Step-4 raked diaries (Leg-2)"
    )
    p.add_argument("--in_csv",    required=True, help="Input raked augmented_diaries.csv")
...
def main() -> None:
    ...
    print("=" * 70)
    print("3rdJ_04M_mindwell_2split.py — minimum-dwell post-processor")
    ...
    # Identify hom30 / wrk30 columns (present only)
