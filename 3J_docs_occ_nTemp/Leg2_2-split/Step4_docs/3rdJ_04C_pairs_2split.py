# -*- coding: utf-8 -*-
"""
3rdJ_04C_pairs_2split.py — Step 4C (Leg-2): Training Pair Construction

Ported VERBATIM (logic) from 2J_docs_occ_nTemp/04C_training_pairs.py.
Leg-2 deltas: only the platform-detection path block + single output dir
(--sample reads/writes the SAME outputs_step4/ artifact names). The K=5
neighbour matching is byte-for-byte the Leg-1 scheme; the two-channel data
layer does not change pair construction (pairs key off demographics + strata).
...
Usage:
    py -3 -X utf8 3rdJ_04C_pairs_2split.py           # full dataset
    py -3 -X utf8 3rdJ_04C_pairs_2split.py --sample  # smoke test (same out dir)
"""
...
# ── Platform-detection path block (mirrors Step3 / 04A) ───────────────────────
...
def main():
    ...
    print("=" * 64)
    print(f"Step 4C (Leg-2) — Training Pairs  {'[SAMPLE MODE]' if args.sample else ''}")
    print("=" * 64)
