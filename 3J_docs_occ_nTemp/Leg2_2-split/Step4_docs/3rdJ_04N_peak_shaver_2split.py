...
def main() -> None:
    ...
    # ── LOAD hom30/wrk30 arrays for synthetic rows (GA coherence) ─────────────
    if hom_cols:
        hom_syn_arr = df.loc[syn_mask, hom_cols].to_numpy(dtype=float)
    else:
        hom_syn_arr = np.zeros((n_syn, N_SLOTS), dtype=float)
    if wrk_cols:
        wrk_syn_arr = df.loc[syn_mask, wrk_cols].to_numpy(dtype=float)
    else:
