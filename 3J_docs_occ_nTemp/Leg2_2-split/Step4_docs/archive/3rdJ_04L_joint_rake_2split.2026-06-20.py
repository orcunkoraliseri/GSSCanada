...
def parse_args():
    ...
    p.add_argument("--checkpoint", default=None,
                   help="Override model checkpoint path. Default: <r5_dir>/checkpoints/"
                        ...
                        "augmented_diaries.csv (e.g. R10_fast_twcoh) but the checkpoint "
                        "lives in the original sweep dir (R10_fast/checkpoints/).")
    ...
    p.add_argument("--telework_aware", action="store_true", default=False,
                   help="[OPT-IN] Enforce work-slot coherence before raking: "
                        "FLOATING (act==1 & wrk30=0 & hom30=0) is forbidden. "
...
def _apply_telework_coherence(aug: pd.DataFrame) -> pd.DataFrame:
    ...
    OW1 note: telework slots count as work for the activity marginal (act30=1 kept
...
def main():
    ...
    if telework_aware:
        ...
        # Verify: no FLOATING rows remain in synthetic output
        floating_after = 0
        work_slots_after = 0
        for j in range(1, N_SLOTS + 1):
            ...
            floating_after += int((work_mask & (wrk_v == 0) & (hom_v == 0)).sum())
