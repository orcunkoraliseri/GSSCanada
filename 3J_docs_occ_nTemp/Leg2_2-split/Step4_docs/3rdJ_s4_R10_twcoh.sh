...
# ── [1/3] Inference with --telework_coherent (idempotent — skip if done) ──────
echo ""
if [ -f "${TWCOH_OUT}/augmented_diaries.csv" ]; then
    echo "[1/3] SKIP — ${TWCOH_OUT}/augmented_diaries.csv already exists (inference done)."
else
