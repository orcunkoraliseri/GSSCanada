# -*- coding: utf-8 -*-
"""
3rdJ_04A_assembly_2split.py — Step 4A (Leg-2): Two-Channel Training Dataset Assembly

Ported from:  2J_docs_occ_nTemp/04A_dataset_assembly.py  (Leg-1)
              + 2J_docs_occ_nTemp/step4_Speed_Cluster/04A_sample_assembly.py
                (--sample stratified subsampling idea, folded inline here)

Merges hetus_30min.csv + copresence_30min.csv + work_30min.csv (Leg-2 NEW),
encodes the demographic conditioning vector (one-hot + continuous + binary),
recodes co-presence from GSS coding (1=Yes, 2=No) to binary (1/0) with a per-slot
availability mask, builds the AT_WORK track + work availability mask, builds the
48-slot x 11-feature aux token sequences, then splits respondents 70/15/15
stratified by CYCLE_YEAR x DDAY_STRATA (seed=42).

LEG-2 deltas vs Leg-1:
  1. Read work_30min.csv and build the AT_WORK track. aux_seq grows from width
     10 to width 11: [AT_HOME(1) | AT_WORK(1) | 9 co-presence].
     AT_WORK is column index 1; co-presence indices 2..10.
  2. Build work_avail (n,48) bool mask: True where WORK30 slot is non-NaN.
  3. Conditioning additions to cond_vec:
       - WORK_SCHEDULE one-hot (categories 1..9, NaN -> all-zero)
       - NAICS one-hot industry bucket (NaN -> all-zero)
       - TELEWORK binary (value -> 0/1, NaN -> 0) PLUS a separate
         "telework_known" indicator bit (1 if non-NaN else 0)
     Everything else in cond_vec identical to Leg-1.
  4. Emit work_pos_weight = (1-freq)/freq for AT_WORK (computed on train split,
     masked by work_avail), added to feature config.

Usage:
    py -3 -X utf8 3rdJ_04A_assembly_2split.py           # full dataset
    py -3 -X utf8 3rdJ_04A_assembly_2split.py --sample  # ~2% stratified sample
"""
...
# ── Platform-detection path block (mirrors Step3 3rdJ_03_mergingGSS_2split.py) ──
...
INPUT_DIR = os.path.join(_LEG2_BASE, "Step3_docs", "outputs_step3")
...
# One-hot categorical columns in the conditioning vector (Leg-1 identical)
CAT_COLS = [
...
# Leg-2 NEW conditioning columns
...
def load_data(input_dir: str):
    """Load the three Leg-2 input files (always full files; sampling happens later)."""
...
def merge_inputs(hetus: pd.DataFrame, cop: pd.DataFrame, work: pd.DataFrame):
    ...
    cycles). The unique row key is (occID, CYCLE_YEAR). All three files share the
    ...
    return df, cop_slot_cols, work_slot_cols
...
def build_work_track(df: pd.DataFrame):
    """
    [Leg-2 NEW] Build the AT_WORK track + availability mask from WORK30_*.
...
def encode_demographics(df: pd.DataFrame, encoders=None, scaler=None):
    ...
    # ── One-hot categorical columns (Leg-1 identical) ───────────────────
    for col in CAT_COLS:
    ...
    # ── Continuous: standardized TOTINC (Leg-1 identical) ───────────────
    ...
    # ── [Leg-2 NEW] WORK_SCHEDULE one-hot (cats 1..9, NaN -> all-zero) ──
    ...
    # ── [Leg-2 NEW] NAICS one-hot industry bucket (NaN -> all-zero) ──────
    ...
    # ── [Leg-2 NEW] TELEWORK binary + telework_known indicator bit ──────
...
def pack_tensors(sub_df, act_seq, home_seq, work_seq, work_avail,
                 cop_bin, cop_avail, cond_vec, cycle_idx):
    """
    Bundle selected rows into a dict of PyTorch tensors.
    aux_seq = [AT_HOME | AT_WORK | 9 co-pres]: shape (n, 48, 11).
    """
    aux_seq = np.concatenate(
        [home_seq[:, :, np.newaxis], work_seq[:, :, np.newaxis], cop_bin], axis=2
    )  # (n, 48, 11)

    return {
        "act_seq":    torch.tensor(act_seq,    dtype=torch.long),     # (n,48) 0-indexed
        "aux_seq":    torch.tensor(aux_seq,    dtype=torch.float32),  # (n,48,11)
...
def run(args):
    ...
    print("=" * 64)
    print(f"Step 4A (Leg-2) — Two-Channel Assembly  {'[SAMPLE MODE]' if args.sample else ''}")
    print("=" * 64)
    ...
    # ── Load ────────────────────────────────────────────────────────────
    print("\n[1/7] Loading data...")
    hetus, cop, work = load_data(input_dir)
    print(f"  hetus_30min:      {hetus.shape}")
    print(f"  copresence_30min: {cop.shape}")
    print(f"  work_30min:       {work.shape}")

    # ── A1: Merge (positional concat) ───────────────────────────────────
    print("\n[2/7] Joining hetus + copresence + work (positional, same row order)...")
    df, _, _ = merge_inputs(hetus, cop, work)
    ...
    print("\n[3/7] Encoding demographics (Leg-1 block + Leg-2 WORK_SCHEDULE/NAICS/TELEWORK)...")
    ...
    print("  OK colleagues=0 for 2005/2010; cop & work values in {0,1}")

    # AT_WORK / AT_HOME positive rates per cycle (masked by availability)
    print("\n  === AT_HOME / AT_WORK POSITIVE RATES BY CYCLE ===")
    ...
    # AT_WORK pos_weight (masked by work_avail) — [Leg-2 NEW]
    ...
    feature_config = {
        ...
        "aux_order":          ["AT_HOME", "AT_WORK"] + COP_COLS,
