    ...
    with torch.no_grad():
        for start in range(0, N, batch_sz):
            ...
            g_act, g_home, g_work, _, _ = model.generate(
                ...
                # Backcast temp matches the forecast (temperature=0.8, ~line 1835).
                # At temp=0.0 the AR work head latches into a sticky "at work"
                # attractor mid-day and never releases (weekday WORK plateaus ~0.50
                # all night), which corrupted the backcast gate AND the COVID-drift
                # direction.  The temp=0.8 deliverable has no such latch (job
                # 1005623); validating the backcast at the SAME temperature is the
                # fair test.  (Fix 2026-06-26.)
                temperature=0.8,
            )
