...
| 2026-06-26 | **Backcast temp-fix shipped + re-run D1 (job 1006500, GPU 8m35s) — latch ELIMINATED, weekday residual is a modest explained level offset** | Opus, path A. Edited `gen_backcast` (`run_substage_d_phase_i`) `temperature=0.0 → 0.8` to match the forecast (archived `archive/...preBackcastTempFix.py`, 97,750 B = original + metric fix). Re-ran `--stage D1` standalone on pg/GPU (loads W_pooled_2030, regenerates `reconstructed_2022_diaries_2split.csv` at temp 0.8). **Result vs temp=0.0:** weekday (S1) MADwork 0.2646→**0.1694**, MADhome 0.2380→**0.1414**; SHAPE now excellent — JSwork 0.1565→**0.0336**, JShome→**0.0013**. Weekends S2/S3 still PASS (MAD 0.04–0.06; small sampling-noise rise from greedy is benign). Anti-copy Gate1 0.6665 PASS, Gate2 PASS. **The evening work-latch is gone.** Weekday S1 still FAILs the strict MAD<0.10 gate, but now MADhome==dHome (0.1414) and MADwork==dWork (0.1694) EXACTLY → the per-slot gaps are same-signed across all 48 slots = a **uniform LEVEL over-prediction of both home (+14pp) and work (+17pp)** with near-perfect shape, NOT a pathology. WFH_RATE recon 0.8170 vs obs 0.7591 (+5.79pp, just over the 5pp tol). **Cause:** W_pooled_2030 is the 2030-leaning generalist (the deliverable checkpoint); backcasting the anomalous COVID year 2022 with it over-predicts WFH/home by ~6pp — expected, and confirms the checkpoint is forward-appropriate. Does NOT affect the 2030 deliverable (proven healthy, job 1005623). Residual is modest, same-signed, fully explained → NOT gamed (threshold left at 0.10). **Recommend: declare Step 6 done with this documented; optional W_2022_ft supplementary backcast (quick GPU) would show the architecture reconstructs 2022 cleanly when not forward-leaning. Awaiting user OK. Step 6 NOT yet declared done; Step 7 NOT started.** |
| 2026-06-26 | **W_2022_ft supplementary backcast (job 1006507, GPU 8m33s) — checkpoint choice is NOT the weekday driver; residual = raw head-overlap (pre-mutex)** | Opus, path B. Added a backwards-compatible `--backcast_ckpt` flag (default W_pooled_2030 unchanged; non-default writes a TAGGED CSV so canonical isn't clobbered; archived `archive/...preBackcastCkptParam.py`, local `py_compile` PASS). Ran `--stage D1 --backcast_ckpt W_2022_ft_2split.pt` (the 2022-specialised checkpoint, 50.5 MB). **Result:** weekday S1 MADhome **0.1362** MADwork **0.1534** (vs W_pooled 0.1414/0.1694 — only marginally better); WFH_RATE recon 0.8074 delta **0.0482 PASS** (vs W_pooled 0.0579 FAIL — now within 5pp). Weekends PASS, anti-copy PASS, shape excellent (JShome 0.001, JSwork 0.032). **KEY:** weekday S1 STILL FAILs MAD<0.10 with the SAME exact `MADhome==dHome` / `MADwork==dWork` uniform-over-prediction signature → the 2022-specialised checkpoint reconstructs only marginally better, so the weekday residual is NOT forward-leaning (checkpoint barely matters). Both channels uniformly over-predicted (~+14pp home, +15pp work) with near-perfect shape ⇒ the RAW two-channel heads over-fire/overlap — the deliverable resolves exactly this via the 6H `mutual_exclusion_resolve` step (987039 logged 10188 conflicts resolved), but the backcast gate scores RAW pre-mutex output against mutually-exclusive observed, so it is penalised for overlap the deliverable removes. (Alt hypothesis: model under-predicts out-of-home "other" activity — mutex would NOT fix that.) **Next (proposed): cheap CPU check — apply 6H mutual-exclusion to the existing reconstructed CSV + recompute the gate; if weekday passes, the one final principled fix = apply mutex inside the backcast gate. Decisive, zero GPU. Awaiting user OK. Step 6 NOT declared done; Step 7 NOT started.** |
| 2026-06-26 | **Mutex check (job 1006514, CPU) — overlap hypothesis REFUTED; real residual = intrinsic weekday work-hot / home-cold LEVEL bias** | Opus. Applied an exact replica of the 6H `mutual_exclusion_resolve` to BOTH existing reconstructions + recomputed the gate pre/post mutex. **Result: mutex is NOT the cause.** Conflicts (home==1 & work==1) are only 0.52% (pooled) / 0.59% (W_2022_ft) of cells; post-mutex weekday MAD barely moves (pooled MADhome 0.1414→0.1478, MADwork 0.1694→0.1662 — still FAIL) and the deliverable applies mutex anyway. **Corrects my earlier mis-read** ("over-predict both channels"): the weekday day-means show HOME **under**-predicted and WORK **over**-predicted — pooled obs 0.724/0.151/0.125 (H/W/OTHER) vs gen 0.583/0.320/0.097; W_2022_ft gen 0.588/0.304/0.108. OTHER is POSITIVE pre-mutex (no head overlap). So the model places ~15–17pp too many weekday person-slots AT WORK and too few AT HOME, with near-perfect SHAPE (JShome ~0.001, JSwork ~0.03). Identical across both checkpoints ⇒ intrinsic to the trained model, NOT checkpoint/temp/metric/mutex. **Root: the work up-weighting in the loss** — `work_pos_weight=7.873` (feat_cfg) and `cw[WORK_ACT_0IDX]*=5.0` work-boost (module:734) — which the code author already flagged ("Investigate work_pos_weight", module:1070). This machinery lives in the LOCKED Step-4 04D/feature config. **All diagnostic artifacts now ruled out (metric, temp, checkpoint, mutex) — this is the genuine floor: a modest, shape-faithful, work-hot level bias rooted in locked Step-4.** Deliverable otherwise healthy (latch gone, bands diverge, weekends pass). Downstream note: weekday daytime residential (AT_HOME) occupancy may be modestly under-counted for BEM. **Options: (A) accept + document as known limitation; (B) add a bias-correction/calibration of the weekday work/home level. Retrain Step-4 is OFF (locked). Awaiting user decision. Step 6 NOT declared done; Step 7 NOT started.** |
| 2026-06-26 | **De-risk before C: backcast on the LOCKED Step-4 base (job 1006516, GPU 3m26s, read-only inference) — splits the bias: WORK = Step-6-added, HOME = inherited** | Opus. User chose "first C and B". Before committing a ~1.5-day Step-6 retrain, generalised `--backcast_ckpt` to accept an absolute path (backwards-compatible; bare names still resolve in models/; tagged output) and ran `--stage D1` on the locked Step-4 base `R5_lr1e4/checkpoints/best_model.pt` (inference only — base NOT modified, no lock violation; local `py_compile` PASS). **Result, weekday S1 vs the Step-6 checkpoints:** base MADwork **0.0853 (PASSES, JSwork 0.0009 near-perfect)** vs Step-6 0.169 (FAIL); base MADhome **0.1769 (FAILS, worse than Step-6 0.141)**; base WFH_RATE recon 0.705 (under) vs Step-6 0.817 (over) — straddling obs 0.759. **Reading:** the WORK over-prediction is ADDED by Step-6 progressive fine-tuning (base work is fine; Step-6 ~doubles the error) ⇒ softening work_pos_weight in a Step-6 retrain WOULD target it. BUT the HOME miss is present in BOTH (base even worse) ⇒ it is INHERITED from the locked Step-4 base and a Step-6 retrain CANNOT fix it; only B (post-hoc calibration) can. Since HOME is the BEM-critical residential channel and is inherited, **C is now a 1.5-day gamble that would mainly rebalance work↔home, not fix the channel that matters; B is the higher-value, lower-cost, needed-regardless fix.** Revised recommendation: **B first** (calibrate weekday home/work level on the deliverable), hold C as optional. Awaiting user decision. Step 6 NOT declared done; Step 7 NOT started. |
| 2026-06-26 | **B design — bias characterization (job 1006519, CPU): band reweight already fixed most of it; delta-method would OVER-correct → use target-anchored cap** | Opus, user chose B. Characterised per-slot weekday bias (backcast gen − obs 2022) + composition. **Mechanism of the bias:** the raw model collapses **78.7%** of weekday-employed into "full-office-day" (biz-hours work ≥0.9) vs observed **34.0%** — it loses the real mix (obs: 31% home-day / 30% partial / 34% full-office). Per-slot bias is pervasive: HOME −0.13…−0.20, WORK +0.06…+0.25, worst in evening/night (model leaves people AT WORK overnight). Day-means: HOME −0.1414, WORK +0.1694. **GOOD NEWS:** the existing post-hoc band reweight ALREADY corrects most of it — deliverable conservative full-office 50.1% (from 78.7%), mean biz-work 0.638 (from 0.906; obs 0.584). So the deliverable's RESIDUAL is modest: biz-hours work ~+5pp, an evening/night work tail (~+6pp), home day-mean under ~8–14pp. **KEY DESIGN FINDING:** the naive delta-method (subtract the backcast bias 0.169 from the deliverable) OVER-corrects — it drives weekday work to 0.077, BELOW even observed-2022 (0.151) — because the deliverable is already healthier than the backcast (band reweight + 2030 conditioning). So B must be **target-anchored, not bias-subtraction**. Recommended B: cap the deliverable weekday WORK at the observed-2022 per-slot profile (reassign excess work→home, min-dwell-respecting, then 04M), preserving each band's WFH-day share. Awaiting user confirm on the approach/target before implementing (affects publishable 2030 levels). Step 6 NOT declared done; Step 7 NOT started. |
| 2026-06-26 | **Calibration B implemented (`calibrate_weekday_work_2split.py`) — observed-2022-anchored work cap; v1 inflated WFH share → refined to non-biz-only (jobs 1006520→1006521)** | Opus, user confirmed "anchor to real 2022 data". B trims weekday-employed WORK→HOME from work-block tails to cap each band's per-slot work at the observed-2022 weekday-employed profile; flipped Work slots get a time-appropriate home activity (Sleep night / Passive-leisure day); non-employed + weekends untouched. **v1 (job 1006520) over-reached:** capping business-hours work too pushed conservative office-workers over the biz-home≥0.5 WFH-day threshold → WFH-day share inflated 0.174→0.222 (broke the 17.5% band definition); root cause = observed 2022 was a COVID year (~31% home-days) so its daytime work level encodes MORE telework than the conservative scenario. **Fix (job 1006521): trim NON-business-hours slots only** (1–10, 27–48); leave biz slots (11–26) to each band's WFH scenario. Since the WFH-day classifier is biz-hours-based, this preserves shares EXACTLY: 0.1738/0.3019/0.3800 unchanged. Result: night(40–48) work 0.13→**0.035** (obs ~0.02–0.03), evening decline realistic, biz hours untouched, bands differentiated (weekday-employed WORK day-mean 0.283/0.251/0.233; HOME 0.598/0.633/0.653, conservative home +8.4pp). Wrote `2030_synthetic_diaries_2split_calibrated.csv`. **Next: 04M min-dwell on calibrated diaries + verify night tail survives; then declare Step 6 done.** Step 7 NOT started. |
| 2026-06-26 | **04M min-dwell + final verification — STEP 6 DECLARED DONE** | Opus. 04M min-dwell on the calibrated deliverable (job 1006522): only 136 wrk30 + 5,241 hom30 slots changed (median transitions 2.0→2.0) → the calibration was already dwell-coherent, night-tail fix intact. Final weekday profile check (job 1006523) on `2030_synthetic_diaries_2split_calibrated_mindwell.csv`: conservative weekday NIGHT_END WORK 0.037 / HOME 0.916, MIDDAY WORK 0.397 (untouched), **VERDICT HEALTHY** (work releases in evening). **FINAL DELIVERABLE for Step 7 = `outputs_step6/2030_synthetic_diaries_2split_calibrated_mindwell.csv` (111,024 rows).** Full sign-off: degeneracy fixed (987039); bands diverge 17.0/29.2/38.8% WFH-day share + WD_AT_HOME diverges (Gate 4 PASS); backcast metric (profile-JS) + temp (0.8) fixes make the validation honest (shape excellent, weekends PASS, anti-copy PASS); weekday work-hot bias diagnosed (home-under INHERITED from locked Step-4, work-over+night-tail added by Step-6) and corrected by calibration B (non-biz work cap to observed-2022, WFH-day shares preserved exactly, BEM-clean). **Documented residual (accepted):** weekday business-hours home modestly under-counted = locked Step-4 inherited bias, scenario-appropriate for conservative's low-telework premise; un-fixable without retraining locked Step-4 (C ruled out). **STEP 6 DONE. Step 7 (BEM wiring) NOT started — no Step7_docs/ folder yet; confirm with user before starting; carry office_archetype_lookup.csv as prerequisite (OD-4).** |
| 2026-06-26 | **Calibration-C added (`3rdJ_06_calibrate_C_activity_weekend_2split.py`) — triggered by Step-7 2030 diagnostic** | Employee (Sonnet 4.6), LOCAL. Two Step-6 drifts identified in the Step-7 2030 run (diagnostic 2026-06-26): (1) sleep share collapsed 34.6%→22.8% (act30 un-calibrated); (2) weekend daytime home dropped 0.43 (no modeled driver). Calibration-C (same family as B) corrects both post-hoc without touching locked model files. Stage 1 restores weekend hom30 to observed-2022 per-stratum (Sat/Sun) targets via per-slot OUT↔HOME flips + 04M min-dwell. Stage 2 donor-resamples all strata act30 from the 2022 observed pool conditioned on (slot × state=WORK/HOME/OUT). wrk30 never modified; weekday hom30 never modified → WFH weekday gain exactly preserved. **Output: `outputs_step6/2030_synthetic_diaries_2split_calibrated_mindwell_C.csv` (111,024 rows) — new canonical 2030 deliverable for Step 7.** Step 7 re-run with `--year 2030 --deliverable <_C file>` completed; all 6 gates PASS; pre-calibration band files backed up to `outputs_step7/*_BAK_2026-06-26.csv`. |
| 2026-06-26 | **Calibration-C completed with weekend work-cap stage (Stage 0) — triggered by Step-7 validator E.4 FAIL** | Employee (Sonnet 4.6), LOCAL. Step-7 validator E.4 (WD > WE office presence) FAILED for all 9 arch/band combos in 2030: calibration-B capped weekday work only; weekend wrk30 was never capped, leaving WE wrk30 ~18.6% (observed ~6.6%). Stage 0 added to `3rdJ_06_calibrate_C_activity_weekend_2split.py` — per-slot trim-only cap (1->0 flips, seed=42) of weekend wrk30 to observed-2022 weekend per-slot mean; Saturday 18.7%->7.1%, Sunday 18.5%->6.1%. Calibration-C deliverable regenerated (111,024 rows); all Step-7 FAILs cleared (2022: 32/0/0, 2030: 43/0/0). Weekday wrk30 untouched; Stage 1/2 integrity checks preserved via wrk_orig re-snapshot post-Stage-0. |
