...
Exit codes:
    0  â€” simulation completed successfully
    1  â€” simulation failed (error written to stdout)
    2  â€” argument / setup error (bad paths, missing files)
"""
...
from eSim_bem_utils_3J import neighbourhood, config
...
def main() -> None:
    ...
    if result.get("status") == "ok":
        print(f"\nDone: {os.path.basename(idf_path)} â€” output: {result.get('output_dir')}", flush=True)
        sys.exit(0)
    else:
        print(f"\nFAILED: {os.path.basename(idf_path)} â€” {result.get('error')}", flush=True)
        sys.exit(1)
