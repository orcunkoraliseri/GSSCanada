...
## C.7 - The `V4-B4` 2J EUI extraction defect, and the argument for this study's immunity
...
The immunity argument. The present pipeline is verified immune to this defect because its EUI
values are read from hourly meter streams, never from the tabular demand-summary table
`calculate_eui()` reads. This is worth one sentence in this paper's own Limitations as a reproducibility
point: the same class of extraction defect exists in the codebase this project descends from, and
The present pipeline structurally does not route through the vulnerable function.
