...
# Superseded in part, 2026-08-06 night (additive; nothing above deleted)

## 1. "They cannot differ" is no longer true, by design

The Assembly section above says the two `fullSet/` files are written from one string and **cannot
diverge**. That was true when written. The user then ruled that the submission copy must carry no
notes added during the build, so `readySubmission.md` is now that same string put through one
deterministic transform. **The two files are intentionally different and their md5s no longer match.**

The 2J hazard the original design guarded against is still closed, and by the same argument: the
difference between the two files is **a function that runs on every build and prints a manifest of
every removal**, not an edit somebody made by hand. Re-running the assembler reproduces both exactly.

## 2. Three defects found after this report was signed off

- 🔴 **The strip deleted the `**Table 4.**` caption.** A `## Sources` section was dropped by running
  from its heading to the next heading, and the caption sat inside that span. **Every check passed.**
  The residue post-condition asks *did apparatus survive*, which is structurally blind to *did content
  disappear*. Found by the R1 read-through, a human-role reader, not by any check. Fixed two ways: a
  section drop now also stops at the first line that is plainly content again, and a **loss check**
  now counts every table and figure caption before and after the strip and names any that vanished.
  Both arms seen failing.
- 🔴 **The manuscript stated its own headline retail median twice, differently.** Chapter 5, the
  Abstract and the Discussion said **75.63 / 5.47 % below floor**; Chapter 7 and Table 7 said
  **75.4 / 5.7 %**. Both arithmetically self-consistent, which is why neither looked wrong alone.
  Re-derived from the frozen CSV: **75.6260**, **5.4675 %**. Chapter 7 and Table 7 corrected. No band
  moved, no verdict changed. *No check in this project compares a number in one chapter against the
  same number in another chapter.*
- **A broken cross-reference:** Results §5.1 pointed at "Section 5.2 (Figure 10)" when Figure 10 is
  the §5.1 longitudinal figure and §5.2's figure is Figure 7. Corrected.

## 3. The `n/r` count moved again, and the reason is the strip, not a recount

The 19 figure above counted `⚠ check source` marks in the source files. The submission copy renders
them as `n/r`. Two of the 19 sat inside sections that the strip removes, so the submission copy shows
fewer. **The source files still carry all 19.** Do not read the two numbers as a disagreement.

## 4. Table 6's Evidence column is still standing, still on purpose

It is the largest surviving block of apparatus and the only thing that keeps "additive by
construction" from being an unsupported assertion. Relocation spec written at
`3rdJ_table06_evidence_restructure.md`; not executed.
