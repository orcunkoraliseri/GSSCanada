...
[REF NEEDED: source for class-weighted cross-entropy and the matching logit adjustment]
...
with τ_res = 0.50, τ_off = 0.40 and τ_ret = 0.15. A slot with no candidate is assigned to no channel. <!-- Confirm the exact normalisation against the decode code before the numbers pass (the old text says "threshold-normalized argmax"). -->
