...
| OR-rule leak | `occACT==4 & occPRE==1` (online shopping), excluded from AT_RETAIL | rule frozen 2026-07-02; cross-tab still reported | project-chosen |
...
| Transformer (Regression) | Old-head (Head 1, Head 2) JS drift | ΔJS ≤ 0.002 bits vs the two-channel baseline | project-chosen |
...
## (c) Wiring + differentiation gates (the two-channel stage lesson gates)

Made mandatory because the two-channel stage People-field wiring bug (`Number_of_People_Schedule_Name`, not
