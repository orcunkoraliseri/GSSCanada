...
be separated for those cycles (Table 2, footnote 1). The full per-cycle codebook, listing the raw
variable and the mapped codes for each of the four cycles alongside the derivation rule itself, is
given in Table A2.
...
output-side probes mandatory before any campaign cell of this study is accepted, rather than leaving output-side
