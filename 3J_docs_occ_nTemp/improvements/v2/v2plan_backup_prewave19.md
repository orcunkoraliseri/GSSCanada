...
*Updated 2026-08-05. **34 of 45 done · 1 in progress · 5 ready to start · 5 blocked.** Nothing is
...
**V2-D4 — sync the scorer's `BENCH` to the decided bands.**
The Step-9 scorer holds a table called `BENCH`: the reference energy-intensity band each channel is
judged against, which is what turns a simulated building into a PASS or a FAIL. It has two halves.
**The provenance half is done** — we followed every band's citation back to its source, and the office
band's `src=` pointed at a directory that does not exist, meaning the number deciding 56 cells had no
traceable origin. **The values half was stuck** because we had not decided what the bands should be;
the WP-B decisions unblocked it. What is left: write the decided values and their new citations into
the scorer, then confirm the edit does not silently flip any gate we did not predict in advance — a
band change that quietly turns a FAIL into a PASS is exactly what this project refuses to do.

**⬜ READY TO START — unblocked, in the order I would do them**

| # | ID | Why it is next |
|---|---|---|
| 1 | **V2-D10** | Implement the decided per-object DHW resize (`LAUNDRY` K ≈ 7, the rest K = 1). |
| 2 | **V2-D9** | Decide `NECB-C-*` for retail. Only lever in weeks that could move `S9-EUI-retail`. |
| 3 | **V2-E2** | Runnable locally, no cluster wait. |
| 4 | **V2-E1** | 1 GPU job. Closes the last high finding, B-3. |
| 5 | **V2-E4** | 4 CPU jobs, `sbatch`, `-t 7-00:00:00`. |
...
| **V2-G3** | ✅ **UNBLOCKED 08-05** — WP-A, WP-B and C6 are all closed. Ready to write |
...
| 🔄 | **V2-D4** | Sync the scorer's `BENCH` to the decided bands | small code | Q7 | **PARTIAL** · provenance ✅ 08-04 (office `src=` resolved to nothing; new checker falsified 3/3) · **values half now UNBLOCKED by the WP-B decisions** |
...
| ⛔ | **V2-G3** | Write the consolidated limitations section | writing | the paper | WP-A ✅ WP-B ✅ → **unblocks when C6 lands** |
