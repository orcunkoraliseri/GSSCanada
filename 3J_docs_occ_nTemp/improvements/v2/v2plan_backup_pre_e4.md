...
*Updated 2026-08-05 (night). **40 done · 1 in progress (V2-E4) · 4 blocked.**
**The whole desk-work critical path is closed and every decision is taken.** What remains is one
compute job that needs an allocation call, the re-score, and the closing bookkeeping.*

```
DONE        ████████████████████████████████████████░░░░░░  40 / 45
IN PROGRESS █                                               1   ← V2-E4  (needs the user)
BLOCKED     ████                                            4
```
...
> a **V2-G3 limitation**. Carrying it as "partial" forever would put a permanently unreachable item
> in the queue. The board (`v2board.html`) and this panel are both at **40 / 1 / 0 / 4**, verified
> programmatically against the summary table.

**🔄 IN PROGRESS — V2-E4, the validator across seeds 0–4.** *Promoted on E1's partial closure.*
Convention adopted 2026-08-05 at the user's request: **the moment a task closes, its successor is
promoted into this block and onto the board**. If this block is ever empty while READY is not, the
board is **stale**. 🔴 **E4 needs the user before it can be submitted**, for two reasons: Speed is
reachable but saturated by the user's **own** arrays (~670 tasks pending at `AssocGrpCpuLimit`), so
submitting is an allocation decision; and **E1 just showed the shipped seed-3 checkpoint was already
lost**, which puts the feasibility of a 5-seed retrain in question. E4 also supplies the
independent-seed control **V2-E2** needs to judge Head-2 drift.

**⚠️ PARTIAL — V2-E1. B-3 is answered; its PR-AUC half is permanently blocked.** 🔴 **The shipped
Leg-3 seed-3 checkpoint does not exist on either machine** — Speed holds `warmup_checkpoint.pt`
alone and every `sweep/seed_3_*` dir is outputs-only. Free-running PR-AUC is **not recomputable** and
retraining would answer a different question. **This is the answer to the "ckpt hash needs cluster"
line V2-D5 left open: there is no checkpoint to hash.** → limitation for **V2-G3**.
**But B-3 itself was settled with no checkpoint at all**, by perturbing the pool and re-running the
...
| 🔄 | **V2-E4** | Validator across seeds 0–4; publish mean ± sd | 4 CPU jobs | B-7 | **IN PROGRESS 08-05 night** (promoted on E1's partial closure) · `sbatch`, single line, `-t 7-00:00:00` · 🔴 **needs the user**: Speed is reachable but saturated by the user's own arrays 1172484/1172485 (~670 pending at `AssocGrpCpuLimit`), so submitting is an allocation decision · **also now supplies the independent-seed control V2-E2 lacked for Head-2 drift** · ⚠️ **and E1 casts doubt on feasibility — seeds 0–4 require training runs, and the shipped seed-3 checkpoint was already lost** |
...
**Also corrected tonight: "Speed is down" was wrong.** Speed is reachable. `squeue` shows the queue
saturated by **the user's own array jobs 1172484 / 1172485**, ~670 tasks pending at
`AssocGrpCpuLimit`. The constraint is the account CPU limit, not availability. No job was submitted
into that backlog without the user present.
...
**Correction to a standing assumption: "Speed is down" was wrong.** Speed is reachable. `squeue`
shows it saturated by **the user's own arrays 1172484 / 1172485**, ~670 tasks pending at
`AssocGrpCpuLimit`. The constraint is the account CPU limit. **Nothing was submitted into that
backlog with the user away.**
