...
*Updated 2026-08-05 (night). **38 of 45 done · 1 in progress · 2 ready to start · 4 blocked.**
Nothing is waiting on the user any more. **The whole desk-work critical path is closed**, and with
**V2-D9 closed tonight the last decision is taken too** — WP-A, WP-B, WP-C, every WP-D decision and
the limitations section are written. **Everything still open is compute plus bookkeeping.***
...
**🔄 IN PROGRESS — V2-E2, the row-matched REG-1 / REG-2 comparison.** *Promoted on D9's closure.*
Convention adopted 2026-08-05 at the user's request: **the moment a task closes, its successor is
promoted into this block and onto the board**, so both always show what is being worked on rather
than only what is finished. If this block is ever empty while READY is not, the board is **stale** —
the last closure failed to promote its successor. E2 runs **locally** off `step4_val_meta.csv`, which
matters because Speed has been unavailable all day.
...
| # | ID | Why it is next |
|---|---|---|
| 1 | **V2-E1** | 1 GPU job. Closes the last high finding, B-3. |
| 2 | **V2-E4** | 4 CPU jobs, `sbatch`, `-t 7-00:00:00`. |
...
| ⬜ | **V2-E1** | Persist retail probabilities in 04E; recompute PR-AUC / F1 / RW8 | 1 GPU job ~40 min | **B-3**, C-4 | READY · the last high finding still needing compute |
| 🔄 | **V2-E2** | Row-matched REG-1 / REG-2 comparison | same job as E1 | C-2 | **IN PROGRESS 08-05 night** (promoted on D9's closure) · runnable **locally** (`step4_val_meta.csv`), no cluster wait — with Speed down this is the one that proceeds regardless |
...
| ⬜ | **V2-E4** | Validator across seeds 0–4; publish mean ± sd | 4 CPU jobs | B-7 | READY · `sbatch`, single line, `-t 7-00:00:00` |
