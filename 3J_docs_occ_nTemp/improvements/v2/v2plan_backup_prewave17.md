...
**V2-C3 — the retail occupant-density line in both master docs.**
Our documentation says a retail space holds roughly one person per 3.7 m², while the model uses one
per 25 m². For months that read as a 6.8× contradiction between the docs and the code, and it was
logged as a high-severity finding (B-11). **It is not a contradiction.** NECB states occupancy as
**3.72 people per 1000 ft²**, and converting that gives **24.97 m² per person** — the same quantity in
different units, copied into our text without its unit. So this is a **writing fix, not a code fix**:
correct the sentence in both master documents, state the unit explicitly, and record B-11 as retired.
**One real issue survives and goes into the same edit:** the model applies NECB's *office* density
(24.97) to retail zones, where NECB specifies **29.97** for retail sales, so our shops are about 20 %
more crowded than the code assumes.
...
| 🔄 | **V2-C3** | Retail occupant density line in both master docs | 20 min | B-11 | 🔴 **REOPENED 08-05** · it is a **unit-label error**, not a discrepancy — `25.0/3.7 = 6.76` is a conversion factor, so **B-11's defect does not exist**; real fix = retail uses office 24.97 where NECB gives **29.97** |
