...
| ID | Task | Cost | Closes | Status |
|---|---|---|---|---|
| **V2-A1** | Run the B-13 falsifier on the 2J converter | 1 script, minutes | B-13 | **DONE** 08-04 · B-13 withdrawn |
...
| **V2-C3** | Retail occupant density ~3.7 → parsed 25.0 m²/person | 20 min | B-11 | **DONE** 08-04 |
| **V2-C4** | The 0.95 retail peak — restate or re-source | 30 min | B-11, B-5 | **DONE** 08-04 (labelled; re-source owed) · 08-05: re-source **not answerable in-repo** — the tower IDF carries `NECB-A-*` only, no retail schedule set exists to read → routed to **V2-F7** |
...
| **V2-F6** | Walk the `RV05` retrieval route ourselves: one prototype ZIP → `.table.htm` → Total Site Energy | 30 min | settles the vintage mismatch; falsifies or confirms `RV05` Section B | **OPEN** · prediction pre-registered 08-05 · **do this first, it does not wait on V06** |
| **V2-F7** | `V06` — NECB retail schedule table + Canadian archetype artefact existence | prompt authored; user runs it | V2-C4's owed re-source; terminates the Canadian-archetype line | **AUTHORED** 08-05 · `deepResearch_Resources/V06_necb_schedules_and_canadian_archetypes.md` · **EUI values forbidden outright**; type-A transcription is the built-in falsifier |
