...
*Updated 2026-08-05. **36 of 45 done · 1 in progress · 4 ready to start · 4 blocked.** Nothing is
waiting on the user any more. **The whole desk-work critical path is now closed** — WP-A, WP-B, WP-C,
the WP-D decisions and the limitations section are all written. **Everything still open is either a
compute job or one code change.***

```
DONE        ████████████████████████████████████░░░░░░░░░░  36 / 45
IN PROGRESS █                                               1   ← V2-D10
READY       ████                                            4
BLOCKED     ████                                            4
```

**🔄 IN PROGRESS — V2-D10, the per-object DHW resize.** *On deck, not started.* Convention adopted
2026-08-05 at the user's request: **the moment a task closes, its successor is promoted into this
block and onto the board**, so both always show what is being worked on rather than only what is
finished. If this block is ever empty while READY is not, the board is **stale** — the last closure
failed to promote its successor.

> **Pre-registration is owed before any run.** The discriminator is **`LAUNDRY`'s own slope leaving
> −0.98**, *not* the aggregate elasticity. Share-reweighting moves the aggregate on its own — at
> K = 6 the aggregate went to 0.334 while every heater's individual slope stayed put — so an
> aggregate-only check is a gate that cannot fail.

**⬜ READY TO START — unblocked, in the order I would do them**

| # | ID | Why it is next |
|---|---|---|
| 1 | **V2-D9** | Decide `NECB-C-*` for retail. Only lever in weeks that could move `S9-EUI-retail` — and D4 showed the gap is **5.7 %**, not the 0.15 % previously assumed, so the decision is bigger than it looked. |
| 2 | **V2-E2** | Runnable locally, no cluster wait. |
| 3 | **V2-E1** | 1 GPU job. Closes the last high finding, B-3. |
| 4 | **V2-E4** | 4 CPU jobs, `sbatch`, `-t 7-00:00:00`. |
...
| ⬜ | **V2-D9** | Decide whether to load `NECB-C-*` for the retail channel | decision + small code | V2-F8, `S9-EUI-retail` | **NEW 08-05** · `grep -c "NECB-C-" = 0` today · only lever in weeks that could move `S9-EUI-retail` (fails by 0.06–0.23 %) · ⚠️ retail lighting is frozen — **predict the direction first** |
| 🔄 | **V2-D10** | Implement the per-object DHW resize decided in V2-B4 | small code + 1 run | B4 | **NEW 08-05** · `resize_idf()`: `LAUNDRY` K ≈ 7, rest K = 1 · 🔴 **pre-register first: the discriminator is `LAUNDRY`'s own slope leaving −0.98, NOT the aggregate elasticity** |
