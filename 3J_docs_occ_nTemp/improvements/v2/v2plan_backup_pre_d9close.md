...
*Updated 2026-08-05 (evening). **37 of 45 done · 1 in progress · 3 ready to start · 4 blocked.**
Nothing is waiting on the user any more. **The whole desk-work critical path is now closed** — WP-A,
WP-B, WP-C, the WP-D decisions and the limitations section are all written. **V2-D10 closed this
evening: the last code change before the re-score is done.** Everything still open is compute, plus
one decision.*
...
**🔄 IN PROGRESS — V2-D9, load `NECB-C-*` for the retail channel, or not.** *Promoted on D10's
closure.* Convention adopted 2026-08-05 at the user's request: **the moment a task closes, its
successor is promoted into this block and onto the board**, so both always show what is being worked
on rather than only what is finished. If this block is ever empty while READY is not, the board is
**stale** — the last closure failed to promote its successor.

> **What makes D9 harder than it looked.** `grep -c NECB-C-` is **0** today, so this is the only lever
> in weeks that could move `S9-EUI-retail`. V2-D4's measurement corrected the size of the target: the
> gap is **5.7 %** — retail median 75.4 against a floor of 80, with **44 of 56** cells under — not the
> 0.06–0.23 % single-cell crossing previously assumed. Retail lighting is **frozen**, so the direction
> must be predicted before the schedules are loaded; a lever that moves the median the wrong way is a
> result, not a setback.

**✅ CLOSED THIS EVENING — V2-D10, the per-object DHW resize.** D1–D5 PASS, **D6 FAIL as
...
| # | ID | Why it is next |
|---|---|---|
| 1 | **V2-E2** | Runnable locally, no cluster wait. |
| 2 | **V2-E1** | 1 GPU job. Closes the last high finding, B-3. |
| 3 | **V2-E4** | 4 CPU jobs, `sbatch`, `-t 7-00:00:00`. |
...
| 🔄 | **V2-D9** | Decide whether to load `NECB-C-*` for the retail channel | decision + small code | V2-F8, `S9-EUI-retail` | **IN PROGRESS 08-05 eve** (promoted on D10's closure) · `grep -c "NECB-C-" = 0` today · only lever in weeks that could move `S9-EUI-retail` · ⚠️ **CORRECTED by V2-D4:** the gap is **5.7 %** (retail median 75.4 vs a floor of 80, **44/56** cells under), *not* the 0.06–0.23 % single-cell crossing · retail lighting is frozen — **predict the direction first** |
...
| ⬜ | **V2-E2** | Row-matched REG-1 / REG-2 comparison | same job as E1 | C-2 | READY 08-04 · runnable **locally** (`step4_val_meta.csv`), no cluster wait |
