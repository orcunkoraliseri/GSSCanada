#!/bin/bash
# 3J Leg-3 Step 8E -- aggregate ARM E (T9-13 DHW volume scaling), plus the P1 sweep.
#
# Built from agg_armC.sh. Two differences, both deliberate:
#  1. the campaign dir is DISCOVERED, not hard-coded, and the job stops if it does not resolve to
#     exactly one directory (the 2026-07-31 pre-flight 1170768 died on a hand-written glob that
#     assumed one level too few -- and the older validate script had the same bug and never noticed
#     because it asserted nothing).
#  2. it runs the P1 shape-preservation sweep over all 56 provenance files here, in the job, rather
#     than leaving 56 files to be read one at a time afterwards.
#SBATCH --job-name=3J_L3_aggE
#SBATCH -p ps
#SBATCH --cpus-per-task=4
#SBATCH --mem=16G
#SBATCH -t 7-00:00:00
#SBATCH --output=/speed-scratch/o_iseri/step8_4split/campaign/logs/aggE_%j.out
...
echo "### 0. resolve the arm E campaign directory"
NDIR=$(ls -d $CAMP/out_E_dhwvol/campaign_*/ 2>/dev/null | wc -l)
if [ "$NDIR" != "1" ]; then
  echo "FATAL: expected exactly 1 campaign dir under out_E_dhwvol, found $NDIR"
  ls -d $CAMP/out_E_dhwvol/*/ 2>/dev/null
  exit 1
fi
CDIR=$(ls -d $CAMP/out_E_dhwvol/campaign_*/ | head -1)
CDIR=${CDIR%/}
echo "  campaign dir: $CDIR"
echo "  expected hash: campaign_56d6e324 (injector md5 prefix)"
case "$CDIR" in
  *campaign_56d6e324) echo "  hash OK" ;;
  *) echo "FATAL: campaign hash is not 56d6e324 -- a different injector produced these runs"; exit 1 ;;
esac
...
echo "### 2. P1 sweep -- shape preservation across ALL 56 cells, not just the smoke"
...
# FIX 2026-08-02 (open item 4). The old line asserted `n_audited == 47` for all 56 cells. The
# actual distribution is 47 (26 cells), 71 (20), 31 (6), 0 (4 Default_NECB controls) -- the count
# is a function of the GEOMETRY (how many WaterUse:Equipment objects the IDF has) and of WHICH
# channels the scenario requested, not a constant. A check that is wrong for 30 of 56 cells tells
# you nothing on any of them.
#
# What IS invariant, and is therefore what is asserted: two cells with the SAME geometry and the
# SAME channels_requested must audit the SAME number of objects. Anything else is a real anomaly.
# This can fail; the old one could only be ignored.
TBL=$CAMP/logs/aggE_naudited_$SLURM_JOB_ID.txt
: > $TBL
for D in $CDIR/*/; do
  CELL=$(basename "${D%/}")
...
echo "  distinct MXU_*_DHWv2_* schedule names created, deduped across all 56 cells:"
grep -h "^t9_13_derived_name " $CDIR/*/injected.idf.provenance.txt \
  | sed 's/^t9_13_derived_name //' | sed 's/_r[0-9]\{4\}w[0-9]\{4\}_.*$//' | sort -u

echo "### 2d. D9 -- per-day-type fidelity, read back from each saved IDF (FINDING 9 guard)"
# D9 rides on the same provenance line as D7. Its ABSENCE is the thing to watch: a cell produced by
# an injector older than md5 233932d7 has no n_d9= field at all, and a missing gate must never read
# as a passing one.
...
$PY -u 3rdJ_08E_aggregate_4split.py \
    --campaign-dir "$CDIR" \
    --outdir $CAMP/agg_E_dhwvol \
    --eplus-idd "$EPLUS_IDD"
RC=$?
echo "  aggregate arm E exit=$RC : $(date)"
ls -la $CAMP/agg_E_dhwvol
exit $RC
