#!/usr/bin/env python3
"""
P10R SIBLING (2026-09-22) of 3rdJ_06_calibrate_C_4split.py (md5 ffce34e6..., left untouched).
ONLY CHANGE: Stages C0 (weekend work), C1 (weekend home) and RETAIL now take LABOUR-FORCE-STRATIFIED
targets, the way Stage B already conditions on LFTAG: for LFTAG in (1, 2), the OBS2022 rows with that
LFTAG set the target applied to the 2030 rows with the same LFTAG. LFTAG 3 / NaN have no anchor (the
census frame holds no one outside the labour force) and are left UNCALIBRATED by these three stages.
Everything else (Stage B, global mindwell, C1's local mindwell over all weekend rows, C2, the mutex
guard, seed 42, the pooled Sat+Sun weekend target) is unchanged. No weekday home target (manager
ruling, 3J_IMP_execution_2026-09-22.md §P10R). Run with `--out_tag v3` -> ..._C_v3.csv.
Why: writing/implementation/IMP/P10_2030_level_check.md §3 and §7 (Option R).
...
LF_STRATA = [1, 2]   # [P10R] labour-force strata raked in C0/C1/RETAIL; LFTAG 3/NaN left uncalibrated
WEEKEND_STRATA = [2, 3]
WEEKDAY_STRATA = [1]
SMALL_CELL = 20
...
def run_stage_C0(out: pd.DataFrame, obs22: pd.DataFrame, rng) -> None:
    ...
    # [P10R] one pooled Sat+Sun target PER LABOUR-FORCE STRATUM, applied to the same LFTAG in 2030
    for lf in LF_STRATA:
      obs_lf = obs22[obs22["LFTAG"] == lf]
      obs_we_pooled = obs_lf[obs_lf["DDAY_STRATA"].isin(WEEKEND_STRATA)]
      ...
      if n_we_pooled > 0:
          ...
          n_sat = int((obs_lf["DDAY_STRATA"] == 2).sum())
          n_sun = int((obs_lf["DDAY_STRATA"] == 3).sum())
          print(f"  [P10R LFTAG={lf}] [POOLED WEEKEND TARGET] Sat+Sun n={n_we_pooled:,} (Sat={n_sat:,}, Sun={n_sun:,})  "
                f"day-mean={target_wrk_pooled.mean():.4f}  SE={se_pooled * 100:.2f}pp")
      else:
          print(f"  [P10R LFTAG={lf}] [POOLED WEEKEND TARGET] no observed weekend rows available")
      for stratum in WEEKEND_STRATA:
        d30_mask = ((out["DDAY_STRATA"] == stratum) & (out["LFTAG"] == lf)).values
        n = int(d30_mask.sum())
        ...
        obs_own = obs_lf[obs_lf["DDAY_STRATA"] == stratum]
        own_mean = float(obs_own[WRK].values.mean()) if len(obs_own) > 0 else float("nan")
        gap_vs_own_pp = (rate_after - own_mean) * 100.0
        print(f"  LFTAG={lf} Stratum {stratum} ({lbl}): n={n:,}  DOWN(1->0)={n_down:,}  UP(0->1)={n_up:,}  "
...
def run_stage_C1(out: pd.DataFrame, obs22: pd.DataFrame, rng) -> None:
    ...
    # [P10R] one pooled Sat+Sun HOME target PER LABOUR-FORCE STRATUM, applied to the same LFTAG in 2030
    for lf in LF_STRATA:
      obs_lf = obs22[obs22["LFTAG"] == lf]
      obs_we_pooled = obs_lf[obs_lf["DDAY_STRATA"].isin(WEEKEND_STRATA)]
      n_we_pooled = len(obs_we_pooled)
      ...
      if n_we_pooled > 0:
          ...
          n_sat = int((obs_lf["DDAY_STRATA"] == 2).sum())
          n_sun = int((obs_lf["DDAY_STRATA"] == 3).sum())
          print(f"  [P10R LFTAG={lf}] [POOLED WEEKEND HOME TARGET] Sat+Sun n={n_we_pooled:,} (Sat={n_sat:,}, Sun={n_sun:,})  "
                f"day-mean={target_hom_pooled.mean():.4f}  SE={se_pooled * 100:.2f}pp")
      else:
          print(f"  [P10R LFTAG={lf}] [POOLED WEEKEND HOME TARGET] no observed weekend rows available")
      ...
      for stratum in WEEKEND_STRATA:
        ...
        gap_vs_own_pp = (rate_after - own_mean) * 100.0
        print(f"  LFTAG={lf} Stratum {stratum} ({lbl}): n={n:,}  UP(OUT->HOME)={n_up:,}  "
...
def run_stage_retail(out: pd.DataFrame, obs22: pd.DataFrame, rng, lever_value: float, lever_name: str) -> None:
    ...
    total_up = total_dn = 0
    # [P10R] per-day-type retail target PER LABOUR-FORCE STRATUM (LFTAG 3/NaN left uncalibrated)
    for lf, stratum in [(lf, st) for lf in LF_STRATA for st in (1, 2, 3)]:
        obs_str = obs22[(obs22["DDAY_STRATA"] == stratum) & (obs22["LFTAG"] == lf)]
        d30_mask = ((out["DDAY_STRATA"] == stratum) & (out["LFTAG"] == lf)).values
        n = int(d30_mask.sum())
        print(f"  [P10R LFTAG={lf}] stratum {stratum}: obs22 n={len(obs_str):,}  2030 n={n:,}")
        ...
        print(f"  LFTAG={lf} Stratum {stratum} ({lbl}): n={n:,}  UP(OUT->RETAIL)={n_up:,}  "
