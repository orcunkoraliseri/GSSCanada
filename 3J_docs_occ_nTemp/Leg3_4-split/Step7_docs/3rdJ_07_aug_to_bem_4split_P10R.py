"""P10R SIBLING (2026-09-22) of 3rdJ_07_aug_to_bem_4split.py (md5 fa8bb8a8..., left untouched).
Why: writing/implementation/IMP/P10_2030_level_check.md §7 Option R; state in IMP/P10R_fix.md.
CHANGES, and only these:
  * 2030 source = ..._C_v3.csv (LFTAG-stratified weekend/retail rake), md5 pinned below.
  * OUT_DIR = outputs_step7_P10R/ (the frozen outputs_step7/ is never written).
  * --year 2022: residential/office/retail are built on the STOCK frame with 2022 REAL diaries,
    `08A.demo_assemble(stock, stock[CYCLE_YEAR==2022 & IS_SYNTHETIC==0])` -- the same function the
    Y2005/10/15 products use -- instead of the all-cycle stock. Step-7's own conventions kept:
    residential = convert(complete_day_types(frame)); office/retail on the non-completed frame.
  * --year 2030: residential per band = demo_assemble_2030(stock, band pool) (the matcher the office
    product already uses) instead of the unmatched assemble_2030 draw. Office and retail builders
    unchanged (build_office_2030_product / build_retail_product_2030), fed _C_v3.
  * Hotel products are NOT rebuilt: byte-copied from outputs_step7/ with their frozen md5 asserted.
  * --registry: writes outputs_step7_P10R/P10R_products_registry.json {file name: md5}.
...
D2030 = (LEG3 / "Step6_docs" / "outputs_step6"
         ...
         / "2030_synthetic_diaries_4split_calibrated_mindwell_C_v3.csv")   # [P10R]
D2030_EXPECTED_MD5 = "6ee73094bc076955baef52eec51e8764"                  # [P10R] _C_v3
D2030_PREDECESSOR_MD5 = "5aa74f44cd09a7afa9fa5418864956ed"   # [P10R] _C_v2, the frozen arm's source
...
D2030_ALLOWED_STEMS = {
    ...
    "2030_synthetic_diaries_4split_calibrated_mindwell_C_v3",   # [P10R]
}
...
FROZEN_HOTEL_MD5 = {
    ...
    "hotel_schedule_multiplier_2030_central.csv": "4b3d3a4603cc0cccc6a1bf42139d69ee",
    "hotel_schedule_multiplier_2030_opt.csv":     "e0ab6c86fbe3c0ea2475f8cada6d1bd6",
}
STEP8A_PATH = LEG3 / "Step8_docs" / "3rdJ_08A_gen_historical_products_4split.py"


def _copy_frozen_hotel(name):
    """[P10R] byte-copy one frozen hotel product into OUT_DIR; refuse on any md5 mismatch."""
    src, dst = FROZEN_S7 / name, OUT_DIR / name
    assert _md5(src) == FROZEN_HOTEL_MD5[name], f"frozen hotel {name} md5 moved: {_md5(src)}"
    shutil.copy2(src, dst)
    assert _md5(dst) == FROZEN_HOTEL_MD5[name], f"copied hotel {name} md5 differs"
    print(f"  [P10R] hotel {name}: byte-copied from outputs_step7 (md5 {FROZEN_HOTEL_MD5[name][:8]}, unchanged)",
          flush=True)
    return dst
...
def _load_08a():
    ...
    spec = _ilu.spec_from_file_location("_p10r_step8a", STEP8A_PATH)
...
def cmd_year_2022():
    ...
    # ---- [P10R] 2022 REAL diaries onto the stock frame (08A.demo_assemble, as Y2005/10/15) ----
    pool22 = stock[(stock["CYCLE_YEAR"] == 2022) & (stock["IS_SYNTHETIC"] == 0)].copy()
    print(f"\n[P10R] pool = stock CYCLE_YEAR==2022 & IS_SYNTHETIC==0: {len(pool22):,} rows "
          f"(of {len(stock):,}); assembling with 08A.demo_assemble", flush=True)
    check_mutex(pool22, "2022 real pool (pre-assembly)")
    h08a = _load_08a()
    stock = h08a.demo_assemble(stock, pool22)      # same rows/columns as stock, 2022-real diaries
    check_mutex(stock, "2022 assembled (P10R frame)")
    ...
    # ---- Hotel ----
    print("\n[Hotel] [P10R] unchanged -- byte copy of the frozen 2022 hotel product", flush=True)
    out_hot = _copy_frozen_hotel("hotel_schedule_multiplier_2022.csv")
    run_hotel_gates(pd.read_csv(out_hot), label="2022 (frozen copy)")
...
def cmd_year_2030(bundle, sens=None, deliverable=None):
    ...
    if "residential_office" in chans:
        for blabel, office_band in office_states:
            ...
            # [P10R] matched assembly (AGEGRP/SEX/LFTAG/NOCS + DDAY_STRATA) instead of assemble_2030
            _stock = pd.read_csv(AUG, low_memory=False)
            df_band = demo_assemble_2030(_stock, d30[d30["BAND"] == office_band].copy())
            check_mutex(df_band, f"2030 assembled ({office_band}, P10R matched)")
...
def main():
    ...
    grp.add_argument("--registry", action="store_true",
                     help="[P10R] write outputs_step7_P10R/P10R_products_registry.json {csv name: md5}")
    ...
    if args.audit:
        ...
        reg = {p.name: _md5(p) for p in sorted(OUT_DIR.glob("*.csv"))}
        out = OUT_DIR / "P10R_products_registry.json"
        out.write_text(_json.dumps(reg, indent=2), encoding="utf-8")
        for k, v in reg.items():
            print(f"  {v}  {k}", flush=True)
        print(f"  WROTE {out} ({len(reg)} products)", flush=True)
    elif args.year == "2022":
        cmd_year_2022()
    elif args.year == "2030":
