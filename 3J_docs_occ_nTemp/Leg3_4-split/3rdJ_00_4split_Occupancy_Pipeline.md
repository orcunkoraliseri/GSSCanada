...
| Retail (⚠️ Leg 3) | People: `0.95 × shape_c_d(t)` in customer hours (peak-normalized, dr_L3-06 — see blockquotes below); staff-only slots (baseline ≤ 0.10) keep the baseline; Lights/Plug per the Step-9 rules (opening-hours / staff) | NECB retail density (~~~3.7 m²/person~~ → 🔴 **25.0 m²/person**, parsed from the injected IDF; **not** an intended retail density — see V2-E3 and the blockquote below) — **do not scale the count** |
...
> ~~3.7 m²/person~~.** Parsed directly from the injected IDF, self-verified for this correction:
