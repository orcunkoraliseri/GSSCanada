# 3J manager prompt - THE standing handoff, last updated 2026-08-09 (docx cleanup + references round)
...
## Where the paper stands, 2026-08-09 (end of the docx cleanup + references round)
...
Current state: **1,325 lines**, 22 captions. `f3` 4 PASS / 1 FAIL (**correct, do not modify f3**) ·
`f4` 7 PASS / 0 FAIL (31 files, 22 exhibits) · `f5` 7 PASS / 0 FAIL · `f6` 5 PASS / 0 FAIL. Every
figure is 300 or 600 dpi with a vector PDF. The line count dropped 1,467 to 1,325 because 59 thematic
breaks and six apparatus notes came out, not because anything was cut.

**There is a rendered submission package:**
...
## Open BUILD NOTE 1 of 2: Table A2 ships unlabelled and uncited
...
**Note, 2026-08-09:** the Table A1/A2 placeholder moved from `Chapter_08_Conclusion.md:13` to
`Chapter_10_Supplementary.md:3` when Chapter 08 was split. The defect and both options are unchanged.

---

## Open BUILD NOTE 2 of 2: two references have never been opened
...
### Settled 2026-08-09 in the docx cleanup round
...
**2J is cross-cited, and it had been missing from its own successor.** §1.4 describes 2J's abstract
almost word for word while citing the JBPS paper and the eSim companion instead. 2J is now
`Iseri and Hachem-Vermette (under review b)` and is cited at §1.4, §2.1, §3.6, §7.F and the reference
list; the JBPS entry became `under review a`.
⚠ **Left for the authors:** §1.4 opens "Leg-1, published as the second journal in this line (2J)",
which reads as though Leg-1 and 2J are the same paper. The citations are right either way.

**Line spacing single**, see the reference-doc warning above.

---

## The work that is left, in the order I would do it

1. **Settle the Table A2 label** (open BUILD NOTE 1 above). Then rebuild the .md, the .docx, and
   re-run `f4`.
2. **Author a `V<NN>` prompt for the Kurin and Menon citations** (open BUILD NOTE 2 above) so the
   author can run it externally. Two entries, four fields each. Do not search from the session.
3. **Fill the cover letter's placeholders**: handling editor's name and submission date, in
...
4. **Write the three binding commitments into the cover letter and abstract** if they are not already
   there - reconfirming B&E is what made them binding. Check, do not assume.
5. **The generative-AI declaration.** RV10 item 18 gives Elsevier's prescribed wording; it goes in a
...
6. **Deferred from earlier rounds, still deferred:** N7 (`f3`'s C2 failure list; **do not relax C2** -
