...
**Last updated: 2026-09-25 ~03:00 UTC, "Stage 2 done, three helpers running, author asleep" round.**
Predecessor: `archive/RESUME.2026-09-25_pre_0108utc_status.md` (older: `archive/RESUME.2026-09-24_pre_local_relaunch.md`, `archive/RESUME.2026-09-23_pre_cancel.md`, `archive/RESUME.2026-09-23_pre_hold_return.md`).

> 🟢 **UPDATE 2026-09-25 ~03:00 UTC - READ THIS BOX FIRST.** Author asleep: "continue until the end, and update the
> manager prompt and artifact". Board v21.
> - **Done:** Stage 2b numbers (139/143 by agent, rewrite_log "Stage 2b") + Stage 2c by manager (rewrite_log "Stage 2c"):
>   4 leftover markers fixed, meaning-changed sentences rewritten (retail now PASS on the median rule, sign flips),
>   V3c written in (night presence is a property of the office-hours control; Highlight 2 is now office presence),
>   V4 limitation sentence, 10_Declarations in 2J form, Table A.1 split, self-citations as 2J ("companion journal
>   manuscript, under review", in-prep refs removed), Widen reference replaced with the 2J Crossref-checked one.
>   Zero ⟦P10R⟧ markers, zero en/em dashes in chapters_v2.
> - **Author images:** Figure 1 installed (`writing/submission/figures/Figure_01_pipeline_4split.png`, 4488 px, checked OK).
>   Graphical abstract NOT OK: stray diagonal line across the left panel + old hotel top chart. Prompt updated to office
>   (`writing/submission/figures/Prompts_Images_v4/graphicalAbstract.md`); author must regenerate. Their tool redrew both
>   with scripts `writing/figures/generate_fig01_framework.py`, `generate_graphical_abstract.py` (do not edit them: images are the author's).
> - **Author research reports RV11-RV14** saved in `deepResearch_Resources/`. Vetting agent running -> writes
>   `deepResearch_Resources/VETTING_RV11_RV14_2026-09-25.md`. Apply only TRUSTED-2J actions; CANDIDATE items go to the author.
> - **Running:** (1) vetting agent (above); (2) P9 second pass agent (02, 07-09, SI, writing/tables, data figures ->
>   `IMP/data/P10R/P9_second_pass_table.csv`, rewrite_log "P9 second pass"); (3) V3a repeat runs,
>   `Leg3_4-split/Step8_docs/campaign_local_P10R_V3a/_logs/driver.log`, 10/40 at 02:50 UTC, ~25 min each, 0 failed.
> - **Then:** rewrite any meaning-changed sentences the P9 agent lists -> apply vetting actions -> V3a sentence (§11 of
>   `IMP/V3_design_and_runs.md`) -> rebuild both Word files (check media md5s) -> P14 cover-letter placeholders ->
>   V3-O2 cross-check -> closure (progress log, this file, board, memory).
> - **Status words:** 3J text calls both companion papers "in revision" (P12 facts: neither is under review). Change to "under review" only if that is true at submission.
>
