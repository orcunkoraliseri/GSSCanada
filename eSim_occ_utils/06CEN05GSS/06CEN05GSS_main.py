"""
06CEN05GSS Main Controller

Unified controller for the occupancy modeling pipeline.
Controls all steps from Census/GSS alignment through BEM output generation.
...
Usage:
    python 06CEN05GSS_main.py              # Interactive menu
    python 06CEN05GSS_main.py --sample 5   # Set sample percentage
"""
...
DEFAULT_SAMPLE_PCT = 5  # Default sample percentage for pipeline
...
def print_header() -> None:
    ...
    print("\n" + "=" * 60)
    print("  06CEN05GSS OCCUPANCY MODELING PIPELINE")
    print("  Census 2006 + GSS 2005 Integration")
    print("=" * 60)
    ...
    try:
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                "alignment",
                Path(__file__).parent / "06CEN05GSS_alignment.py"
            )
...
def run_profile_matching(sample_pct: int) -> bool:
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "profile_matcher",
            Path(__file__).parent / "06CEN05GSS_ProfileMatcher.py"
        )
...
def run_hh_aggregation(sample_pct: int) -> bool:
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "hh_aggregation",
            Path(__file__).parent / "06CEN05GSS_HH_aggregation.py"
        )
...
def run_bem_conversion(sample_pct: int) -> bool:
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "bem_converter",
            Path(__file__).parent / "06CEN05GSS_occToBEM.py"
        )
...
def run_gss_reader() -> None:
    """Run GSS header reader."""
    print("\n" + "=" * 50)
    print("  GSS HEADER READER")
    print("=" * 50)
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "gss_reader",
            Path(__file__).parent.parent / "gss_reader.py"
        )
        gss_reader = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(gss_reader)
        gss_reader.main()
    except Exception as e:
        print(f"  ❌ GSS reader failed: {e}")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="06CEN05GSS Occupancy Modeling Pipeline Controller"
    )
    parser.add_argument(
        "--sample",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Run specific option directly (1-7)"
    )
