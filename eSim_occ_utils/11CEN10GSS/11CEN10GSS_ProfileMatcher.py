"""
11CEN10GSS Profile Matcher Module

Matches Census 2011 agents with GSS 2010 schedules based on demographic profiles.
...
class MatchProfiler:
    ...
    Matching Tiers:
        ...
        Tier 2: Core Demographics (HHSIZE, AGEGRP, SEX, MARSTH, LFTAG, PR)
        Tier 3: Key Constraints (HHSIZE, AGEGRP, SEX)
...
def generate_full_expansion(
    ...
    carry_vars = [
        "HHSIZE", "AGEGRP", "SEX", "MARSTH", "PR", "TOTINC",
        "KOL", "LFTAG", "CMA",
        "DTYPE", "BEDRM", "ROOM", "CONDO", "REPAIR"
    ]
...
def validate_matching_quality(
    ...
    """
    Validates profile matching quality: tier distribution and episode sanity check.
...
def main(sample_pct: float = 10.0) -> None:
    """
    Entry point for profile matching.

    Args:
        sample_pct: Percentage of data to sample (1-100). Default 10.
    """
    import sys
    sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent.parent))
    from occ_config import BASE_DIR

    ALIGNED_DIR = BASE_DIR / "Outputs_11CEN10GSS" / "alignment"
    OUTPUT_DIR = BASE_DIR / "Outputs_11CEN10GSS" / "ProfileMatching"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    aligned_census = ALIGNED_DIR / "Aligned_Census_2010.csv"
    aligned_gss = ALIGNED_DIR / "Aligned_GSS_2010.csv"

    print("=" * 60)
    print("  11CEN10GSS PROFILE MATCHING")
    print("=" * 60)
    ...
    matched_keys_path = OUTPUT_DIR / f"11CEN10GSS_Matched_Keys{suffix}.csv"
    ...
    expanded_path = OUTPUT_DIR / f"11CEN10GSS_Full_Schedules{suffix}.csv"
    generate_full_expansion(df_matched, expander, expanded_path)

    # --- 7. Validate Matching Quality ---
    print("\n--- Step 7: Running Validation ---")
    validation_path = OUTPUT_DIR / f"11CEN10GSS_Validation{suffix}.txt"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Profile Matching: Census 2011 + GSS 2010")
    parser.add_argument(
        "--sample",
        type=float,
        default=10,
