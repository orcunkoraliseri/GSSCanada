#!/bin/bash
#SBATCH --job-name=wp10_stage4_default_RC6
#SBATCH --account=chachemv
#SBATCH -p ps
#SBATCH -c 1
#SBATCH --mem=24G
#SBATCH -t 7-00:00:00
#SBATCH --output=/speed-scratch/o_iseri/1J_rerun/logs/wp10_stage4_default_RC6_%j.out

# WP10 Stage 4c: Default (--iter-count 1) task for NUS_RC6, on the RE-STAGED
...
echo "=== Running run_batch_hpc.py NUS_RC6 (Default, --iter-count 1) ==="
python "$CODE/eSim_bem_utils/run_batch_hpc.py" \
    --idf        "$CODE/BEM_Setup/Neighbourhoods/NUS_RC6.idf" \
    --region     Quebec \
    --sim-mode   standard \
    --iter-count 1 \
...
EXIT_CODE=$?
echo "Done: NUS_RC6 Stage4 Default (exit code: $EXIT_CODE)"
exit $EXIT_CODE
