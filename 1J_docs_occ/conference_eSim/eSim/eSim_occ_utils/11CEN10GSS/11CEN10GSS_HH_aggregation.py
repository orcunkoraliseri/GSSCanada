"""
11CEN10GSS Household Aggregation Module
...
class HouseholdAggregator:
    ...
    def process_all(self, df_expanded: pd.DataFrame) -> pd.DataFrame:
        ...
        and merges aggregation back to individual grids with static metadata.
...
def main(sample_pct: int = 10, skip_validation: bool = False) -> None:
    ...
    # Input from ProfileMatcher (no DTYPE expansion step for 11CEN10GSS)
    INPUT_DIR = BASE_DIR / "Outputs_11CEN10GSS" / "ProfileMatching"
    OUTPUT_DIR = BASE_DIR / "Outputs_11CEN10GSS" / "HH_aggregation"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    suffix = f"_sample{sample_pct}pct" if sample_pct < 100 else ""
    expanded_file = INPUT_DIR / f"11CEN10GSS_Full_Schedules{suffix}.csv"

    output_file = OUTPUT_DIR / f"11CEN10GSS_Full_Aggregated{suffix}.csv"
    report_path = OUTPUT_DIR / f"11CEN10GSS_Validation_HH{suffix}.txt"
    plot_path = OUTPUT_DIR / f"11CEN10GSS_Validation_Plot{suffix}.png"

    print("=" * 60)
    print("  11CEN10GSS HOUSEHOLD AGGREGATION")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Household Aggregation: Census 2011 + GSS 2010"
    )
    parser.add_argument(
        "--sample",
        type=int,
