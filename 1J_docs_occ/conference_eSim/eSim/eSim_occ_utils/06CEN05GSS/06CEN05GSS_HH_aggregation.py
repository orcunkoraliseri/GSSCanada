"""
06CEN05GSS Household Aggregation Module
...
Pipeline:
1. Load expanded schedules from ProfileMatcher
...
class HouseholdAggregator:
    ...
    def process_all(self, df_expanded: pd.DataFrame) -> pd.DataFrame:
        ...
        Args:
            df_expanded: Expanded schedules DataFrame from ProfileMatcher.
    ...
    def _aggregate_household(self, people_grids: list) -> pd.DataFrame:
        """
...
def validate_household_aggregation(
    ...
    if report_path:
        try:
            ...
            print(f"   ❌ Error eSim_writing report file: {e}")
...
def main(sample_pct: int = 25, skip_validation: bool = False) -> None:
    """
    ...
    INPUT_DIR = BASE_DIR / "Outputs_06CEN05GSS" / "ProfileMatching"
    OUTPUT_DIR = BASE_DIR / "Outputs_06CEN05GSS" / "HH_aggregation"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    
    # Input file from ProfileMatcher
    suffix = f"_sample{sample_pct}pct" if sample_pct < 100 else ""
    expanded_file = INPUT_DIR / f"06CEN05GSS_Full_Schedules{suffix}.csv"
    
    # Output files
    output_file = OUTPUT_DIR / f"06CEN05GSS_Full_Aggregated{suffix}.csv"
    report_path = OUTPUT_DIR / f"06CEN05GSS_Validation_HH{suffix}.txt"
    plot_path = OUTPUT_DIR / f"06CEN05GSS_Validation_Plot{suffix}.png"
    
    print("=" * 60)
    print("  06CEN05GSS HOUSEHOLD AGGREGATION")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Household Aggregation: Census 2006 + GSS 2005"
    )
    parser.add_argument(
        "--sample",
        type=int,
        default=25,
        help="Sample percentage used in ProfileMatcher (default: 25)"
    )
    parser.add_argument(
        "--skip-validation",
        action="store_true",
