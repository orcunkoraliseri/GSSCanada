#!/bin/bash
#SBATCH --job-name=4J_null_structure
#SBATCH --partition=ps
#SBATCH --mem=16G
#SBATCH --time=7-00:00:00
#SBATCH --output=/speed-scratch/o_iseri/4J_null_structure_%j.out

# Step 3, D-S3-4 / D-S3-5: null LOC / COP run-structure measurement.
# Copied from the shipped /speed-scratch/o_iseri/4thJ_act2_setup_and_run.sh template per the
# cluster rule: submit through a shipped .sh launcher, never a hand-rolled `sbatch --wrap`.
...
"$ENVDIR/bin/python" -u 4thJ_null_structure.py
