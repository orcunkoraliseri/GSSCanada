...
## EXECUTION RECORD — 2026-08-22 (evening)

All four directives are under way. `prereg.md` md5 `e4243e07cdd80c9c846b91f40e3e8c45` unchanged
throughout.

### Directive 1 — `envs/step7` — 🟢 DONE

Built with `vllm` 0.27.1, `xgrammar` 0.2.3 and `bitsandbytes`. `envs/step4` re-verified after every
fix and is untouched (torch `2.5.1+cu121`). It took three additive fixes to serve a token
(`FINDING 79`): `flashinfer` uninstalled, `VLLM_USE_FLASHINFER_SAMPLER=0` for the unguarded second
import path, and `enforce_eager` because the two envs share an interpreter and dynamo refuses to trace
a stdlib it did not hash. 🔴 Every Step 7 throughput figure is therefore **eager-mode** and a floor.

### Directive 2 — `G7.10` — 🟢 DONE, PASS

10,000 strings, 5,000 valid and 5,000 deliberately malformed across 19 labels, **0 disagreements**
between the hand-written oracle and XGrammar 0.2.3. `outputs_step7/g710_oracle_agreement.json`.

### Directive 3 — the pilot rehearsal — 🟢 DONE, and it earned its keep six times over

**Generation.** Six jobs, `N = 600` per fold per arm (1286195–1286200). Constrained **600/600 valid**
on all three folds; unconstrained **16 / 44 / 24 of 600**.

**Step 7 verification.** `tools/4thJ_gates_step7.py` built and run, `G7.1`–`G7.13`, plus an
eleven-run perturbation battery. 12 PASS / 15 FAIL over scored gates.

**Step 6.3–6.5.** 6.3 was already satisfied by the three Leg-4 folds. For 6.4, `G6.4` and `G6.1` had
**no implementation path at all** — nothing mapped our 158 activity codes onto Eurostat's `acl00`
aggregates, so neither the headline gate nor its FAIL criterion could be computed. Both now exist,
calibrated on the real corpus at 9 PASS / 0 FAIL, and `G6.2`/`G6.3` are scored on the same metric.
🔴 6.5, the privacy audit, is **not started**.

**What the rehearsal caught, which is the whole reason for it:**

| | found | would have cost |
|---|---|---|
| `FINDING 80` | vLLM was handed the whole-record grammar and masks only the completion | 0 of 16 valid; the whole campaign |
| `FINDING 81`/`83` | `G7.4` is not enforced — no household-indexed `COP` variants exist | a "passing" gate that was never wired in |
| `FINDING 82` | `G7.6` cannot fall unless `G7.2` has | a third confirmation counted as a measurement |
| `FINDING 84` | four crosswalk errors, three of them one-directional and country-correlated | `G6.4` reporting a 31 % travel error that belonged to the crosswalk |
| `FINDING 85` | the pre-registered "strongest" null is the weakest in 6 of 9 cells | discovering it after a reportable result existed |
| coverage clause | the first `G7.2` perturbation used `999`, which **is** a legal code | a gate never seen failing, recorded as seen failing |

Five open decisions were raised and none was decided here: `D-S7-4`, `D-S7-5`, `D-S6-8`, `D-S6-9`, and
the `G7.4` enforcement question inside `D-S7-5`.

### Directive 4 — Leg 5 — 🟡 SUBMITTED, QUEUED

`tools/4thJ_step4_leg5_fold.sh` written and fold `es` submitted as **job 1286209** on
`gpu:nvidia_a100_7g.80gb:1`. `--run-type primary`, so LoRA in bf16 — **no quantisation, therefore no
`bitsandbytes` in `envs/step4`**, which stays exactly as directive 1 required. Effective batch held at
16 (2 × 8), identical to every Leg-4 fold. Epochs from `4thJ_step4_thresholds.py:EPOCHS_LEG5` = 3.
🔴 `--out` is redirected to `diagnostics_leg5/` because `4thJ_step4_diagnostics.py:413` puts no leg in
its filename and would have silently overwritten the Leg-4 diagnostics.

Work item 7.2, the throughput comparison, is **job 1286208** on the same instance. Both are `PENDING
(Resources)` — the node advertises one `7g.80gb` and nine `2g.20gb` slices of the same physical GPU,
so any 2 GB slice in use blocks the whole one.

🔴 **Only fold `es` is submitted.** `uk` and `it` wait until the `es` fold's Step 4 gates have been
read, per directive 4's own order: *"verify Step 4 gates, and execute the full generation campaign."*

### 🔴 What the campaign must be sized for, now that it is measurable

`V7.a` refuses to score `G7.7`/`G7.8` below **10 strata carrying 100 records**. From the real
100,000-person prefix pools (228 strata per fold) that needs **N ≈ 5,115 / 5,203 / 4,850**. The
`N = 600` rehearsal cannot reach it and never could. `G7.9`'s rejection-sampled control is harder
still and becomes affordable only to the extent Leg 5 lifts `G7.5` above the pilot's 2.67 %.
