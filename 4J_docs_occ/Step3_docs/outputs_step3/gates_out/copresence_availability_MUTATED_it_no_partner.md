...
| it | recorded | not recorded | recorded | recorded | recorded | recorded |
...
| it | `cop_extra_it_madre` | not recorded | The mother component of the `cop_parent` OR. |
| it | `cop_extra_it_padre` | not recorded | The father component of the `cop_parent` OR. |
| it | `cop_extra_it_siblings` | not recorded | "With siblings." A genuine Italian extra — no other country fields a siblings co-presence flag. |
...
| it | 343 | not recorded | "Care of domestic/pet animals" — the explicit contrast with code 342 ("da cortile/allevamento", yard/farmyard animals, included) implies 343 is the indoor/pet case; rejected on that contrast. |
| it | 351 | not recorded | "Construction and renovation of own dwelling" — general, no "esterne" qualifier; ambiguous indoor/outdoor, same reasoning as Spain's 351. |
| it | 352 | not recorded | The label explicitly says "**nella** propria abitazione" (**inside** one's own dwelling) — this is explicitly indoor, not merely unclear, and is excluded on that stronger basis. |
| it | 353 | not recorded | Furniture/household-goods repair — ordinarily an indoor activity; no outdoor qualifier in the label. |
| it | 354 | not recorded | "Vehicle maintenance and repair" — same reasoning as Spain's 354. |
