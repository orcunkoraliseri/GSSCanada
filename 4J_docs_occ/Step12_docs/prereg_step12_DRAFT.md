# Step 12 — no-core real-stock UBEM. Pre-registration. **DRAFT.**
...
opened for writing by this document or any Step 12 tool. This draft becomes citable only when the
owner freezes it and a sidecar md5 is added; until then nothing here may be quoted as a
pre-registered bar.

**Written** 2026-09-03, box 4 of `IMP/Prompt/4thJ_imp_nocore_execution_2026-09-03.md`.
...
* **Population**: the no-core layout of the four districts (Madrid, Lyon, London, Bologna),
  ...
  fallback (§3.2 of `4thJ_12_nocoreRealStock.md`). France (Lyon) is a physical baseline, never a
  4J denominator.
...
* **Gate series**: `G12.x`, this document's companion `4thJ_12_nocoreRealStock_val.md`.
...
3. The owner must pin `ENGINE_DIGEST_PIN` in `tools/4thJ_step12_preflight.py`.
