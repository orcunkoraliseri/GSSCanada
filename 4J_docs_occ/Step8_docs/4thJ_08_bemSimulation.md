...
🔴🔴 **EVERYTHING IN THIS BOX IS SUPERSEDED. `FINDING 141` / `D-S9-3`(a): the three
campaigns above simulated occupancy FOUR HOURS EARLY, and they were re-emitted and re-run on
2026-08-26. Read `Step8_docs/docs/2026-08-26_D-S9-3a_the-rotated-re-run.md` before quoting any number
in this section.** On the rotated campaign at `f = 1.00`: `FINDING 143` the **peak** effect is
**+2.7145 / +0.0393 / −0.6332 %** against diary spreads of 4.9837 / 2.3797 / 1.5959 — ratios
**0.54 / 0.02 / 0.40**, so `FINDING 134`'s test now fells the peak claim too, and in Italy the sign
flips. `FINDING 144` every **annual** median is negative, **−1.5100 / −0.3605 / −0.4178 %**
(was +1.8184 / −0.0357 / +0.0590), monotone in `f`, ratio to spread 0.80–1.44 — the sign of the
annual channel was a statement about the clock. 🔴 **At the top of the pre-registered sweep no
occupancy claim survives on either channel.** `FINDING 145`: the diurnal-shift sentence above is
**withdrawn** (rotated it is `es` 6→7 at `f ≥ 0.50`, `uk` never, `it` 6→7 at `f = 1.00`
only); the annual peak **hour** still never moves, and the **class ordering survives** — `AB` largest
in every fold (`es` +3.46 / `uk` +1.04 / `it` +0.50 % at `f = 1.00`), which is the one occupancy
statement this campaign still supports. ⚪ The `es` class figures quoted above (+10.13 / +4.29)
are **not** what `prerotation/agg_by_class.csv` carries at any `f` (it says 7.45 / 3.89); flagged, not
propagated.
