...
```
Create a flat vector process diagram, LANDSCAPE orientation, about 2400 x 1000 pixels, on a white
background, in one clean sans-serif family. The picture is wide and short, meant to be read across a
full journal page width or across a slide.
...
important formatting rule here. Earlier generations rotated them anyway: if a label does not fit
horizontally inside its band, widen the band or set the label above it, but never turn it on its side.

At the very top, centred, exactly two lines of text with nothing above them:
  From Harmonised Time-Use Surveys to Simulated Building Energy
  The twelve steps of the cross-national occupancy pipeline

STRUCTURE, AND THIS IS THE CHANGE FROM EVERY EARLIER VERSION: THE FLOW RUNS LEFT TO RIGHT, NOT TOP TO
BOTTOM. Across the middle of the picture runs a single horizontal ROW of twelve rounded rectangular
cards, numbered 0 at the far left to 11 at the far right, each joined to the next by one short arrow
pointing RIGHT. Along the TOP edge, five wide tinted rounded bands sit above the cards they group, each
band spanning exactly the cards named below it. Along the BOTTOM edge, seven small pale tiles each
connect UP to the cards they guard by one thin vertical line. Those tier lines are the only vertical
connectors in the picture; every other arrow points right.

If twelve cards in one row would make the card body type smaller than 8 pt at 2400 pixels wide, wrap to
TWO ROWS OF SIX instead: cards 0 to 5 on the upper row reading left to right, cards 6 to 11 on the lower
row also reading left to right, with a single return arrow carrying the eye from the end of the upper row
to the start of the lower one. Never snake the row back on itself, and never let any arrow point left
except that one return arrow. With two rows, the five tinted bands still sit along the top and the seven
tiles still sit along the bottom.
...
There is NO GREEN and NO RED anywhere in this image. Green reads as pass and red as fail, and this figure
scores nothing.
...
digit, then a bold title, then one to three body lines in smaller regular type, then a small chip at the
bottom of the card. Cards are equal width and equal height. The chips
...
card or cards it guards:
  pre-declared gate batteries   joined by ONE bracket that spans cards 1, 2, 3, 4 and 5 together.
                                The bracket starts at the LEFT edge of card 1 and ENDS AT THE RIGHT
...
labels, and the dashed-box line. Nothing else. In particular:
  ...
  the reported folds are not yet trained; the no-core engine does not exist yet.
  The last two were true when this prompt was first written and the manuscript now contradicts both:
  all three folds are trained, generated and scored, and Step 11 is complete.
...
Use no green and no red anywhere; the palette above contains neither. Every line must stay legible when
the picture is printed at full page width. Write NO explanatory note, verdict sentence, footnote or
caption inside the image, and in particular put NO band of small italic text along the bottom: the only
text in the picture is the list under QUOTED TEXT. Render at the largest pixel size available, at least
2400 by 1000, and return a PNG rather than a JPEG: this is line art, and JPEG compression frays the
