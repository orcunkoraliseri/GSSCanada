...
c_model = '#3E6B99'      # Slate steel blue
c_null = '#D19C65'       # Muted ochre
hatch_null = '///'
...
for i, (fold, band, model_val, null_val, is_closest) in enumerate(data_rev):
    ...
    # Upper bar: Model
    ax.barh(y + bar_height/2, model_val, height=bar_height, color=c_model, edgecolor='#23405E',
            label='Fine-tuned language model' if i == len(data_rev)-1 else "")
    # Lower bar: Null
    ax.barh(y - bar_height/2, null_val, height=bar_height, color=c_null, edgecolor='#63421C', hatch=hatch_null,
...
# Note (a) positioned in bottom right where x in [42, 80] is open
note_a = "The null is real diaries from the other two countries, reweighted by iterative proportional fitting\nonto the held-out country's own published marginals. It is not a naive or random baseline."
ax.text(0.99, 0.22, note_a, transform=ax.transAxes, va='top', ha='right', fontsize=7.5, style='italic', color='#444444',
        bbox=dict(boxstyle='square,pad=0.4', facecolor='#FDFDFD', edgecolor='#E0E0E0', lw=0.7))

# Note (b) near axis bottom left
note_b = "The bar was fixed and md5-locked before training and was never moved."
ax.text(0.01, -0.11, note_b, transform=ax.transAxes, va='top', ha='left', fontsize=7.5, style='italic', color='#555555')

# Verdict line beneath chart
verdict_text = "Nine cells. Nine losses. The null is closer to the held-out country's published tables in every band of every fold."
plt.figtext(0.5, 0.02, verdict_text, ha='center', va='bottom', fontsize=10, fontweight='bold', color='#111111')

fig.suptitle("Figure 3: Time-Budget Mean Absolute Error Across Nine Cells", fontsize=11.5, fontweight='bold', y=0.97)

plt.subplots_adjust(left=0.22, right=0.96, top=0.91, bottom=0.16)
