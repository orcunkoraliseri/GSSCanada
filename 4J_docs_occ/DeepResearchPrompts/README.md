...
**Wave 14 — venue positioning re-run for the null result (`L33`), written 2026-09-16, RETURNED and VETTED 2026-09-16.**
🔴 **Verdict: `VETTING_RL33.md`. Accepted in full on all four items, all twenty-three findings, and its primary/secondary recommendation.** Re-ran `L14`'s question against the pre-registered null result. Proved that applied AI venues (*ESWA*, *Applied Intelligence*) possess severe pro-innovation/SOTA-chasing bias that rejects applied papers where models fail to beat baselines; *Software X* was decisively eliminated on aims and scope (mandates original reusable software under open licenses, max 6 pages, whereas Paper 4 is an 18,000-word empirical findings paper with restricted weights); *Patterns* and *Scientific Reports* impose $2,500 to $6,000 USD out-of-pocket APC costs under CRKN exclusions. **`RL14`'s recommendation firmly stands:** primary target *Energy and Buildings* (Elsevier, 100% APC waived under CRKN, 3.8 weeks turnaround), secondary *Building and Environment* (Elsevier, 100% APC waived, 3.1 weeks turnaround), with framing refocused on pre-registered benchmarking and methodology defense (Hong and Li, 2026). *Data-Centric Engineering* (Cambridge) remains the only viable fallback if an AI shelf is unconditionally required. Directly supersedes `RL14`.

**Wave 13 — the reference-repair round (`L32`), written 2026-09-16, RETURNED and VETTED 2026-09-16.**
🔴 **Verdict: `VETTING_RL32.md`. Accepted in full.** Verified 4 of 5 missing citations via CrossRef/DataCite (Loga 2016 for TABULA, Crawley 2001 for EnergyPlus, Shokri 2017 for membership inference, Hu 2021/2022 for LoRA); rejected Wilke 2013 as redundant with Richardson and Widen. Confirmed Vosoughkhosravi 2023 was a chimeric blend of a 2022 LEED paper and a real 2023 ATUS review (repaired to true ATUS review). Exposed the preamble sentence as an overclaim (only 4 Step 9 rows logged on disk). Fully closed the warning block with official metadata for TABULA, Spain INE 2011, UK SN 8128, Italy ISTAT 2016, Eurostat HETUS 2008, and Iseri et al. (2026). Ready for direct insertion into `4J_manuscript_submission.md`.
