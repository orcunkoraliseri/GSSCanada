...
| **1287234** | 7.4 base arm, **es** | 🟡 RUNNING, 12:54 elapsed at last check — **the only thing 7.4 waits on** |
| **1287378** | Step 4 ceiling, **es** — the REAL arm | 🟡 RUNNING on speed-40 |

🟢 **Two of three base arms are already fetched and byte-verified** into
`Step7_docs/outputs_step7/`: `generated_leg5_uk_base.jsonl` **42,049,522 B** and
`generated_leg5_it_base.jsonl` **58,976,527 B**, both exact matches for the remote sizes, plus their
`_summary.json`. **Only `es` remains to scp.**

#### A. When 1287234 lands — the 7.4 three-arm table
