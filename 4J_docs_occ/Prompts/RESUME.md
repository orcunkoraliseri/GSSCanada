...
### 🟡 WHAT IS RUNNING, AND WHAT TO DO WHEN IT LANDS

| jobs | what | when it lands |
|---|---|---|
| **1287231 / 1287232 / 1287233** | 7.5 rejection-sampled control at parity — **es 75,531 / uk 16,795 / it 48,809** draws, sized from the gate's own `implied_draws_for_parity` | re-run `4thJ_gates_step7.py`; read `G7.9`, and `V7.a`/`G7.7`/`G7.8` on **uk**, which failed at 5,200 with only **9** strata carrying 100 records against a floor of 10 |
| **1287234 / 1287235 / 1287236** | 7.4 untuned-base arm, `--base-only`, 16,795 draws/fold. ⚪ Slow by measurement, not by accident: the untuned 7 B runs to `max_new_tokens` on nearly every prompt, ETA ~21 h/fold | run `tools/4thJ_step7_firing_rate_report.py` for the complete three-arm `firing_rate_by_stratum.csv` |
| **1287240** | ceiling **refusal control**, `PYTHONPATH` deliberately unset | 🔴 it **must** stop at the optimiser with *"REFUSING rather than falling back to 32-bit AdamW"*. **A clean exit means the guard does nothing and the real arm must not be trusted.** Then `sbatch 4thJ_step4_ceiling_fold.sh es` |
