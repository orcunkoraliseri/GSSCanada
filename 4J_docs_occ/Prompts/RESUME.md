...
| **1287378** | Step 4 ceiling, **es** — the REAL arm | 🟡 RUNNING on speed-40, **1:17:52** at 08:17 EDT |
...
🟢 The refusal control already passed (job `1287240`: *"REFUSING rather than falling back to
32-bit AdamW"*, exit 1). `bitsandbytes 0.50.1` imports against **`torch 2.5.1+cu121`**;
`AdamW8bit` was imported but **not exercised** (no card on the probe node) — **the ceiling run is
what exercises it**, and it fails at optimiser construction if the binary is wrong.

#### C. Then

Board, Step 7 progress log, this file. 🔴 The manuscript stays out of scope until 7.4 lands.
