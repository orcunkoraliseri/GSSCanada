...
def analyze_tus_stats():
    ...
    base_dir = "Occupancy/Outputs"
    
    # We need to find where the actual TUS data is stored. 
    # Based on previous context, it seems to be in subfolders like Occupancy/Outputs_05CEN05GSS/ ...
    ...
    for year in years:
        ...
        search_pattern = f"Occupancy/Outputs_*/*{year}*.csv"
        ...
        if '2005' in year:
            search_path = "Occupancy/Outputs_06CEN05GSS/HH_aggregation/06CEN05GSS_Full_Aggregated_sample5pct.csv"
        elif '2015' in year:
            search_path = "Occupancy/Outputs_16CEN15GSS/HH_aggregation/16CEN15GSS_Full_Aggregated_sample5pct.csv"
        elif '2025' in year:
            # Assuming similar structure for 2025
            search_path = "Occupancy/Outputs_24CEN25GSS/HH_aggregation/24CEN25GSS_Full_Aggregated_sample5pct.csv"
            
        if not os.path.exists(search_path):
            # Try finding via glob if exact path is wrong
            potential_files = glob.glob(f"Occupancy/Outputs_*{year}*/{year}*_Residential_Occupancy.csv")
            if potential_files:
                search_path = potential_files[0]
            else:
                # Try broader search
                potential_files = glob.glob(f"Occupancy/Outputs_*{year}*/*.csv")
