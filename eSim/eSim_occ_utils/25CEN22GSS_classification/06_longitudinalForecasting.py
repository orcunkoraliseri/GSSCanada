...
def _train_one_epoch_weighted(
    ...
    stratum_weights: dict | None = None,
) -> float:
    ...
    stratum_weights e.g. {1:0.6, 2:1.2, 3:1.2} up-weights Sat/Sun rows on act_loss."""
    model.train()
    total, n = 0.0, 0
    for batch in loader:
        batch = {k: v.to(device) for k, v in batch.items()}
...
def run_substage_b(args, df: pd.DataFrame, device: torch.device) -> None:
    ...
    def _fine_tune(model: nn.Module, train_df, val_df, rw: dict, hom_weight_2022: float = 1.0,
        ...
        for epoch in range(1, max_epochs + 1):
            tr_loss = _train_one_epoch_weighted(
                model, train_loader, optimizer, device, rw, hom_weight_2022, stratum_weights
            )
            val_js  = _val_js(model, val_loader, device)
    ...
    # ────────────────────────────────────────────────────────────────────
    # Weekend up-weighting applied to all fine-tuning phases (Sat/Sun 2:1 vs WD).
    # Mirrors Sub-stage C v2 fix; addresses TFT Phase2 Sat gate breach (0.2040 vs 0.20).
    _B_STRATUM_W = {1: 0.6, 2: 1.2, 3: 1.2}

    # Phase 2: 2005+2010 → W_2010_ft
    # ────────────────────────────────────────────────────────────────────
    print("\n--- Phase 2: 2005+2010 → W_2010_ft (weekend up-weighted) ---")
    ...
    # ────────────────────────────────────────────────────────────────────
    print("\n--- Phase 3: 2005+2010+2015 → W_2015_ft (weekend up-weighted) ---")
    ...
    # ────────────────────────────────────────────────────────────────────
    print("\n--- Phase 4: all 4 cycles → W_2022_ft (weekend up-weighted) ---")
    if hom_boost != 1.0:
...
# ── Sub-stage B4 — Phase 4 resume (load W_2015_ft.pt and finish only Phase 4) ─

def run_substage_b4(args, df: pd.DataFrame, device: torch.device) -> None:
    """Resume Sub-stage B at Phase 4. Requires W_2015_ft.pt + DRIFT_MATRIX_1015/1522
    already on disk (saved by a prior B run that TIMEOUT'd in Phase 4).
    AT_HOME hom_boost is fixed to 1.0 because the prior B run's Phase 3
    AT_HOME_CHECK already PASSED (suppression < 5pp); change manually if rerunning
    in a regime where Phase 3 suppression exceeded the gate."""
    print("\n" + "=" * 60)
    print("SUB-STAGE B4 — Phase 4 resume from W_2015_ft.pt")
    print("=" * 60)
    ...
    def _load_ckpt(path: Path, label: str):
        if not path.exists():
            raise FileNotFoundError(f"{label} not found at {path}. Run Sub-stage B Phase 1-3 first.")
        ckpt = torch.load(path, map_location=device)
        m    = ConditionalTransformer(ckpt["config"]).to(device)
        m.load_state_dict(ckpt["model_state"])
        print(f"Loaded {label} (val_JS={ckpt['val_js']:.4f})")
    ...
    def _build_pool(cycles: list) -> pd.DataFrame:
        sub = df[df["CYCLE_YEAR"].isin(cycles)].copy()
        if args.smoke:
            sub = (sub.groupby("CYCLE_YEAR", group_keys=False)
                       .apply(lambda x: x.sample(frac=0.05, random_state=42))
                       .reset_index(drop=True))
        return sub

    def _split_70_20(pool: pd.DataFrame):
        tr, temp = train_test_split(pool, test_size=0.30, stratify=pool["DDAY_STRATA"], random_state=42)
        va, _    = train_test_split(temp, test_size=0.333, stratify=temp["DDAY_STRATA"], random_state=42)
        return tr, va

    def _fine_tune(model: nn.Module, train_df, val_df, rw: dict,
                   hom_weight_2022: float = 1.0, stratum_weights: dict | None = None):
        train_ds     = DiaryDataset(train_df)
        val_ds       = DiaryDataset(val_df)
        train_loader = DataLoader(train_ds, batch_size=bs, shuffle=True,  num_workers=0, drop_last=True)
        val_loader   = DataLoader(val_ds,   batch_size=bs, shuffle=False, num_workers=0)
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            print(f"  {epoch:6d}  {tr_loss:12.4f}  {val_js:8.4f}")
            if val_js < best_js:
                best_js, best_state, wait = val_js, {k: v.clone() for k, v in model.state_dict().items()}, 0
            else:
                wait += 1
                if wait >= patience:
                    print(f"  Early stop at epoch {epoch} (patience={patience})")
    ...
    print("\n--- Phase 4: all 4 cycles → W_2022_ft (weekend up-weighted, resumed) ---")
    print(f"  hom_weight_2022={hom_boost} (Phase 3 AT_HOME_CHECK PASSED in prior run)")
    model, cfg = _load_ckpt(MODELS_DIR / "W_2015_ft.pt", "W_2015_ft.pt")
...
def run_substage_c(args, df: pd.DataFrame, device: torch.device) -> None:
    ...
    # Weekend up-weighting — escalated 2026-05-20 after C v3 → D Phase i (job 933874)
    # still failed weekend gate (Sat JS=0.1691, Sun JS=0.1638 vs <0.10). Original 1:2:2
    # ratio insufficient; bumping to ~1:3:3 (WD:Sat:Sun), re-normalized to mean=1.0 so
    # total loss magnitude stays consistent.
    stratum_weights = {1: 0.5, 2: 1.4, 3: 1.4}
...
# ── 2030 drift summary helper ─────────────────────────────────────────────────

def _compute_2030_drift_summary(df_2030: pd.DataFrame) -> None:
    """
    Compute per-activity × per-stratum JS divergence between the model's
    2022 reconstruction (reconstructed_2022_diaries.csv) and the 2030
    synthetic diaries (df_2030). Saves 2030_drift_summary.csv.

    Columns: activity, strata, prop_2022, prop_2030, js_divergence, direction
    """
    recon_path = FORECAST_DIR / "reconstructed_2022_diaries.csv"
    if not recon_path.exists():
        print(f"  [WARN] {recon_path} not found — skipping 2030_drift_summary.csv")
        return

    df_2022 = pd.read_csv(recon_path)
    eps      = 1e-10
    records  = []

    for strata_val in [1, 2, 3]:
        sub22 = df_2022[df_2022["DDAY_STRATA"] == strata_val]
        sub30 = df_2030[df_2030["DDAY_STRATA"] == strata_val]
        if len(sub22) == 0 or len(sub30) == 0:
            continue

        acts22 = sub22[ACT_COLS].values.flatten().astype(int)
        acts30 = sub30[ACT_COLS].values.flatten().astype(int)

        # act values are 1..14 in reconstructed CSVs
        p22 = np.bincount(acts22, minlength=15)[1:].astype(np.float64)
        p30 = np.bincount(acts30, minlength=15)[1:].astype(np.float64)
        p22 /= max(p22.sum(), 1)
        p30 /= max(p30.sum(), 1)

        label = {1: "WD", 2: "Sat", 3: "Sun"}[strata_val]
        for act_idx in range(N_ACT):
            q, r = float(p22[act_idx]) + eps, float(p30[act_idx]) + eps
            m    = (q + r) / 2.0
            js   = 0.5 * q * np.log(q / m) + 0.5 * r * np.log(r / m)
            direction = "up" if r > q else ("down" if r < q else "stable")
            records.append({
                "activity":    act_idx + 1,
                "strata":      strata_val,
                "strata_label": label,
                "prop_2022":   round(float(p22[act_idx]), 6),
                "prop_2030":   round(float(p30[act_idx]), 6),
                "js_divergence": round(float(js), 6),
                "direction":   direction,
            })

    summary = pd.DataFrame(records)
    out_path = FORECAST_DIR / "2030_drift_summary.csv"
    summary.to_csv(out_path, index=False)
    print(f"Saved 2030_drift_summary.csv ({len(summary)} rows) to {out_path}")

    # Print top 5 shifting activities overall
    top5 = summary.groupby("activity")["js_divergence"].mean().nlargest(5)
    print("  Top 5 shifting activities (mean JS across strata):")
    for act, js in top5.items():
        print(f"    act={act}  mean_JS={js:.5f}")


# ── Sub-stage D — Phase ii: 2030 Forward Forecast ────────────────────────────

def run_substage_d_phaseii(args, device: torch.device) -> None:
    """
    Phase ii — 2030 Forward Forecast.

    Loads scenario_2030_features.csv (37,008 rows, 2030 AGEGRP-shifted demographics
    with 2022 diary slots as encoder seed). Runs W_pooled_2030 in inference mode
    with TrendEncoder forward projection on [DRIFT_1015, DRIFT_1522, zeros] to
    generate 2030 synthetic diaries.

    Output: 0_Occupancy/Outputs_21CEN22GSS/forecast_2030/2030_synthetic_diaries.csv
    Gate:   [SUBSTAGE_D_PHASE_II_COMPLETE] on success.
    """
    print("\n" + "=" * 60)
    print("SUB-STAGE D — Phase ii: 2030 Forward Forecast")
    print("=" * 60)

    # ── Load W_pooled_2030 ────────────────────────────────────────────────────
    ckpt_path = MODELS_DIR / "W_pooled_2030.pt"
    if not ckpt_path.exists():
        raise FileNotFoundError(f"W_pooled_2030.pt not found. Run Sub-stage C first.")
    ckpt  = torch.load(ckpt_path, map_location=device)
    cfg   = ckpt["config"]
    model = ConditionalTransformer(cfg).to(device)
    strict_load = not args.smoke
    ...
    if missing or unexpected:
        ...
        print(f"  [WARN] {msg} (smoke mode, strict=False)")
    model.eval()
    print(f"Loaded W_pooled_2030.pt (val_JS={ckpt['val_js']:.4f})")

    # ── Load TrendEncoder + run forward projection ─────────────────────────────
    enc_path = MODELS_DIR / "trend_encoder_2030.pt"
    if not enc_path.exists():
        raise FileNotFoundError(f"trend_encoder_2030.pt not found. Run Sub-stage C first.")
    enc_ckpt   = torch.load(enc_path, map_location=device)
    drift_dim  = enc_ckpt["drift_dim"]
    trend_enc  = TrendEncoder(drift_dim).to(device)
    trend_enc.load_state_dict(enc_ckpt["encoder_state"])
    trend_enc.eval()
    print(f"Loaded trend_encoder_2030.pt (drift_dim={drift_dim})")

    # Forward projection: [DRIFT_1015, DRIFT_1522, zeros] → predicted 2030 drift
    dm_1015_path = FORECAST_DIR / "DRIFT_MATRIX_1015.csv"
    dm_1522_path = FORECAST_DIR / "DRIFT_MATRIX_1522.csv"
    if dm_1015_path.exists() and dm_1522_path.exists():
        vec_1015 = _load_drift_vec(dm_1015_path)
        vec_1522 = _load_drift_vec(dm_1522_path)
    else:
        print("  [WARN] DRIFT_MATRIX_1015 / 1522 not found — using zeros for forward projection.")
        vec_1015 = torch.zeros(drift_dim)
        vec_1522 = torch.zeros(drift_dim)

    forward_seq = torch.stack(
        [vec_1015, vec_1522, torch.zeros(drift_dim)], dim=0
    ).unsqueeze(0).to(device)   # (1, 3, drift_dim)

    with torch.no_grad():
        proj_2030 = trend_enc(forward_seq)  # (1, drift_dim)
    print(f"TrendEncoder 2030 projection computed (norm={proj_2030.norm().item():.4f})")

    # ── Load scenario_2030_features.csv ───────────────────────────────────────
    sc_path = INPUTS_DIR / "scenario_2030_features.csv"
    if not sc_path.exists():
        raise FileNotFoundError(
            f"scenario_2030_features.csv not found at {sc_path}. "
            f"Run assemble_scenario_2030.py first."
        )
    sc_df = pd.read_csv(sc_path)
    print(f"Scenario file: {len(sc_df):,} rows × {sc_df.shape[1]} cols")

    if args.smoke:
        sc_df = (
            sc_df.groupby("DDAY_STRATA", group_keys=False)
                 .apply(lambda x: x.sample(frac=0.05, random_state=42))
                 .reset_index(drop=True)
        )
        print(f"  [SMOKE] sampled to {len(sc_df):,} rows")

    # Override CYCLE_YEAR to 2022 for tensor building (no 2030 cycle embedding)
    sc_df = sc_df.copy()
    sc_df["CYCLE_YEAR"] = 2022

    # ── Per-stratum inference ─────────────────────────────────────────────────
    BS = 256
    out_records: list = []
    at_home_wd_vals: list = []

    for strata_val in [1, 2, 3]:
        sub = sc_df[sc_df["DDAY_STRATA"] == strata_val].reset_index(drop=True)
        if len(sub) == 0:
            continue
        label = {1: "WD", 2: "Sat", 3: "Sun"}[strata_val]
        print(f"  Stratum {strata_val} ({label}): {len(sub):,} rows")
        ...
        with torch.no_grad():
            for b0 in range(0, len(sub), BS):
                ...
                out = model(batch)
                gen_acts_all.append(out["act_logits"].argmax(dim=-1).cpu().numpy())
                gen_hom_all.append(
                    (torch.sigmoid(out["home_logits"]) > 0.5).cpu().numpy().astype(np.int64)
                )

        gen_acts = np.concatenate(gen_acts_all, axis=0)  # (N, 48) ints 0..13
        gen_hom  = np.concatenate(gen_hom_all,  axis=0)  # (N, 48) ints 0/1

        if strata_val == 1:
            at_home_wd_vals.append(float(gen_hom.mean()))

        gen_acts_out = (gen_acts + 1).astype(np.int64)  # back to 1..14
        occ_ids = sub["occID"].values if "occID" in sub.columns else np.arange(len(sub))
        for i in range(len(sub)):
            rec = {
                "occID":       occ_ids[i],
            ...
            for j in range(48):
                rec[ACT_COLS[j]] = int(gen_acts_out[i, j])
                rec[HOM_COLS[j]] = int(gen_hom[i, j])
            out_records.append(rec)

    # ── Save 2030_synthetic_diaries.csv ───────────────────────────────────────
    out_df   = pd.DataFrame(out_records)
    out_path = FORECAST_DIR / "2030_synthetic_diaries.csv"
    out_df.to_csv(out_path, index=False)
    print(f"\nSaved 2030_synthetic_diaries.csv ({len(out_df):,} rows) to {out_path}")

    # ── Compute 2030_drift_summary.csv ────────────────────────────────────────
    _compute_2030_drift_summary(out_df)

    # ── Gate evaluation ───────────────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("Phase ii Gate Evaluation")
    print("=" * 60)

    row_ok = abs(len(out_df) - 37_008) <= 100
    print(f"  Row count: {len(out_df):,}  ({'PASS' if row_ok else 'FAIL'}, expected ~37,008)")

    if at_home_wd_vals:
        at_home_wd = float(np.mean(at_home_wd_vals)) * 100
        at_ok = 65.0 <= at_home_wd <= 90.0
        print(f"  WD AT_HOME: {at_home_wd:.1f}%  ({'PASS' if at_ok else 'WARN'}, expected 65–90%)")

    # Night slots (slots 1–8, 04:00–07:59) — sanity check, not a hard gate
    night_cols = ACT_COLS[:8]
    if all(c in out_df.columns for c in night_cols):
        night_acts = out_df[night_cols].values.flatten()
        act_counts = np.bincount(night_acts, minlength=15)[1:]  # 1-indexed
        top_act    = int(np.argmax(act_counts)) + 1
        top_pct    = act_counts[top_act - 1] / max(night_acts.size, 1) * 100
        print(f"  Night slot dominant activity: act={top_act} ({top_pct:.1f}% of slots 1–8)")

    print(f"\n[SUBSTAGE_D_PHASE_II_COMPLETE]")
...
def run_all(args, df: pd.DataFrame, device: torch.device) -> None:
    ...
    run_substage_d_phaseii(args, device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stage",
                        choices=["A", "B", "B4", "C", "D", "D2", "all"], default="all",
                        help="Sub-stage to run (B4=Phase 4 resume from W_2015_ft.pt, D=Phase i backcasting, D2=Phase ii 2030 forecast)")
    ...
    stage_fn = {
        "A":   lambda a, d, dev: run_substage_a(a, d, dev),
        "B":   lambda a, d, dev: run_substage_b(a, d, dev),
        "B4":  lambda a, d, dev: run_substage_b4(a, d, dev),
        "C":   lambda a, d, dev: run_substage_c(a, d, dev),
        "D":   lambda a, d, dev: run_substage_d(a, d, dev),
        "D2":  lambda a, d, dev: run_substage_d_phaseii(a, dev),
        "all": lambda a, d, dev: run_all(a, d, dev),
    }
