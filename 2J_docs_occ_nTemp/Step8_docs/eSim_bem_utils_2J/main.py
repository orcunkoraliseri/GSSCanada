...
from eSim_bem_utils_2J import integration, simulation, plotting, visualizer, idf_optimizer, neighbourhood, config, reporting

# --- CONFIGURATION ---
# This engine is a VERSIONED COPY (2nd-journal Step 8) living at
#   2J_docs_occ_nTemp/Step8_docs/eSim_bem_utils_2J/main.py
# so the repo root (which holds BEM_Setup/) is FOUR dirnames up, not two.
# NB: the sys.path.insert above intentionally stays at two levels — it must
# point at Step8_docs so `import eSim_bem_utils_2J` resolves.
...
COMPARATIVE_YEARS = ('2005', '2010', '2015', '2022', '2030')  # 2nd journal: 2025 -> 2030 forecast
...
# DTYPE vocab in BEM_Schedules_*.csv: SingleD / MidRise / HighRise / OtherDwelling.
STEP8_ARCHETYPES = [
...
# The 6 climate-zone cities: label -> (EPW filename substring, PR region for HH sampling).
# Kelowna + Vancouver both draw from the BC pool (two CZ cities, one province).
STEP8_CITIES = [
    {"name": "Toronto_5A",   "epw": "Toronto",   "region": "Ontario"},
    {"name": "Kelowna_5B",   "epw": "Kelowna",   "region": "BC"},
    {"name": "Vancouver_5C", "epw": "Vancouver", "region": "BC"},
    {"name": "Montreal_6A",  "epw": "Montreal",  "region": "Quebec"},
    {"name": "Calgary_6B",   "epw": "Calgary",   "region": "Alberta"},
    {"name": "Winnipeg_7A",  "epw": "Winnipeg",  "region": "Prairies"},
]
...
# ===========================================================================
def _step8_cell_seed(base_seed: int, tag: str) -> int:
    """Deterministic per-cell RNG seed, stable across machines / HPC array tasks.

    Python's built-in hash() is salted by PYTHONHASHSEED, so we derive the
    per-cell offset from a SHA-256 of the cell tag instead. This guarantees that
    every independent task running the SAME (archetype x city) cell draws the
    SAME households, whether run locally or as separate SLURM array tasks.
    """
    import hashlib
...
def run_step8_paired_mc(
    ...
    """Step 8 paired Monte-Carlo for ONE (archetype x climate-city) cell.

    Samples ``n`` SIM_HH_IDs ONCE (deterministic per-cell seed) from the frozen
    frame, filtered to ``dtype`` x ``region``, then runs EACH sampled household
    across ALL ``years`` in the SAME archetype IDF under the SAME weather — only
    the occupancy/metabolic schedule changes by year -> within-HH paired Δenergy.

    Persists, per (sample x year): the raw eplusout.sql (E+ writes it in the
    scenario dir) + an hourly_meters.csv (8760 x meters, Joules) parsed from it.
    Writes cell_manifest.csv (sampling provenance). Returns a summary dict.

    sim_mode: 'standard' = full annual (the campaign default, needed for true
    8760 load shapes); 'weekly' = 24 TMY weeks (fast smoke test only).
    """
    import random
    import sqlite3

    years = [str(y) for y in (years or COMPARATIVE_YEARS)]
    cell_label = cell_label or f"{os.path.basename(idf_path).split('.')[0]}__{region}"
    output_dir = output_dir or os.path.join(STEP8_RESULTS_DIR, cell_label)
    os.makedirs(output_dir, exist_ok=True)

    print(f"\n=== Step 8 paired MC | cell={cell_label} | dtype={dtype} | region={region} ===")
    print(f"  IDF : {os.path.basename(idf_path)}")
    print(f"  EPW : {os.path.basename(epw_path)}")
    print(f"  N={n}  seed={seed}  years={years}  mode={sim_mode}")

    # 1. Load schedules per year (filtered by dtype x region), unless preloaded.
    if schedules is None:
        schedules = {}
        for y in years:
            csv_path = os.path.join(BEM_SETUP_DIR, f"BEM_Schedules_{y}.csv")
            if not os.path.exists(csv_path):
                print(f"  ERROR: missing {csv_path}")
                return {"status": "error", "cell": cell_label, "error": f"missing {csv_path}"}
            schedules[y] = integration.load_schedules(
                csv_path, dwelling_type=dtype, region=region
            )

    # 2. Candidate pool = SIM_HH_IDs present in ALL years (frozen frame -> identical sets).
    common = None
    for y in years:
        keys = set(schedules[y].keys())
        common = keys if common is None else (common & keys)
    pool = sorted(common or [])
    if not pool:
        print("  ERROR: empty candidate pool for this cell.")
        return {"status": "error", "cell": cell_label, "error": "empty pool"}

    # 3. Deterministic per-cell sample (decision #2: with-replacement only if pool < n).
    rng = random.Random(_step8_cell_seed(seed, cell_label))
    if len(pool) >= n:
        ...
        print(f"  WARN: pool ({len(pool)}) < N ({n}); sampling WITH replacement.")
    print(f"  Pool={len(pool)}  sampled={len(sampled)}  replacement={with_replacement}")

    # 4. Build all (sample x year) injection jobs. Same hh_id across every year.
    jobs = []
    manifest_rows = []
    for s_idx, hh_id in enumerate(sampled, 1):
        meta = schedules[years[0]][hh_id].get("metadata", {})
        manifest_rows.append({
            "sample": s_idx, "sim_hh_id": hh_id, "hhsize": meta.get("hhsize", ""),
            "dtype": meta.get("dtype", ""), "pr": meta.get("pr", ""),
        })
        sample_tag = f"sample_{s_idx:03d}_HH{hh_id}"
        for y in years:
            sc_dir = os.path.join(output_dir, sample_tag, y)
            os.makedirs(sc_dir, exist_ok=True)
            idf_out = os.path.join(sc_dir, f"Scenario_{y}.idf")
            try:
                integration.inject_schedules(
                    idf_path, idf_out, hh_id, schedules[y][hh_id],
                    epw_path=epw_path, sim_results_dir=output_dir,
                    batch_name=cell_label, run_period_mode=sim_mode,
                    output_frequency="Hourly",
                )
                jobs.append({"idf": idf_out, "epw": epw_path, "output_dir": sc_dir,
                             "name": f"{sample_tag}|{y}"})
            except Exception as e:
                print(f"  [inject FAIL] {sample_tag}|{y}: {e}")
    ...
    if not jobs:
        return {"status": "error", "cell": cell_label, "error": "no jobs built"}

    # 5. Run all jobs (parallel, with Windows sequential fallback).
