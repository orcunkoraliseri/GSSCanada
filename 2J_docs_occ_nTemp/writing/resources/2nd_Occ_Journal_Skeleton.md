...
| 4 | Method (simulation) | 6,000 paired EnergyPlus runs (two independent fixed 50-household panels — 2005-2015 and 2022-2030 — with archetypes and TMY weather frozen throughout) isolate the occupancy effect; activity-resolved end uses anchored to SHEU (48/48 cell-years within ±2.7%). | ✅ |
...
4. **Attribution.** SSE-matched per-scenario ensembles → a paired within-household Monte-Carlo design (fifty households held fixed within each of two cycle-year panels, 2005-2015 and 2022-2030; 6,000 runs).
...
The simulation domain (archetypes, cities, climate zones) is given in Table 3, and the paired frozen-frame logic in Table 4. The factorial is 4 archetypes (SingleDetached, OtherDwelling/attached, MidRise, HighRise; Canadian NECB17/NBC936 Z6 archetypes, *not* DOE prototypes) × 6 climate-zone cities (Toronto 5A, Kelowna 5B, Vancouver 5C, Montréal 6A, Calgary 6B, Winnipeg 7A; Atlantic mapped to the Montréal EPW, a stated limitation) × 5 years (2005/2010/2015/2022 calibrated + 2030 forecast) × 50 paired households = **6,000 EnergyPlus v24.2 runs** ✅. The held-versus-varied logic (Table 4): hold the IDF and the TMY weather fixed across all five years, and vary the occupancy time-series; household IDs are fixed only *within* each of two independent panels (2005-2015; 2022-2030 — the 2026-07-09 region-tier relink means the 2022/2030 panel is a separately-drawn sample from a refined 144,465-household frame, not the same IDs as 2005-2015). Within either panel, household differencing cancels envelope, climate, and stock turnover, and removes between-household MC sampling variance; the cross-year Δ is purely the predicted behavioural change with tight CIs for any comparison made within a single panel — this covers the paper's primary target, 2022→2030 (Chen et al. 2022; DR-S8). Outputs are 8,760-h load profiles + MC bands, load-shape metrics (load factor, midday share, peak-to-average, coincidence factor), peak-hour statistics, the stock-weighted ensemble, and annual EUI as the secondary plausibility anchor. MC convergence: 95% CI half-width mean 1.80%, worst cell 4.04% at N = 50 ✅. Methods footnote: 1 of 6,000 runs needed a deterministic DX-coil sizing fix (Sub-step 8G); effect ≤0.013 kWh/m².
...
**Table 4.** *(insert held-vs-varied table here)* — Factors held constant (archetype IDF and TMY weather file across all five cycle-years; household IDs within each of two independent panels, 2005-2015 and 2022-2030) versus the single varied factor (occupancy time-series), with the resulting 6,000-run factorial and the within-panel household differencing logic.
...
4. **Single MTL Z6 envelope across six climates; two frozen analytical frames (144,507 HH 2005-2015; refined 144,465 HH 2022-2030); TMY weather.** Deliberate isolation choices; the paired Δ cancels the envelope within each panel; the 2015→2022 step therefore crosses the panel boundary (cross-sectional, not within-household); Z7A cold-zone EUI sensitivity available.
...
5. The paired within-household Monte-Carlo design (Table 4) — two independent fixed panels (2005-2015; 2022-2030) — attributes these shifts to occupancy alone within each panel, with envelope and weather held fixed throughout; the headline 2022→2030 comparison is fully within-panel.
