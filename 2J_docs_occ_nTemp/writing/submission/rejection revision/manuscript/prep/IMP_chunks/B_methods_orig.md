...
chain of eleven stages: diary harmonization, a generative day-type model, marginal raking to match
...
|-----------|------------|------------------------------------------|------------------------------------|
| GSS time-use diaries | 2005, 2010, 2015, 2022 cycles | Respondent-level 10-minute activity and location diaries, harmonized to a common activity code list | Source diaries for the generative model (2.1, 2.2) and for the raking targets (2.3) |
...
## 2.1 Diary harmonization to 30-minute slots
...
(who else is present). Training maximizes a weighted combination of the per-slot negative
...
Once every household member has a 48-slot activity and at-home series (from the harmonized diary,
...
standardized version of the same equation, $\text{target}_{\text{std}}$, uses a trend and a shift
...
To summarize the 24 simulated cells as one national figure, each cell's result is weighted by its
dwelling archetype's share of the national housing stock (an external reference composition, held
fixed across all years and not derived from this paper's own samples), renormalized over the four
...
households sharing a city or an archetype; the consequence of that clustering is examined in Section
3.8 and the Supplementary Information, and for midday share, where it matters, the wider
clustering-aware interval is the one reported (Section 3.4).
...
- $s$: day-type stratum (weekday, Saturday, Sunday); in Eqs. 1-2 only, a 30-minute slot index.
- $t$: a 30-minute slot index, $t = 1,\dots,48$, 04:00 origin.
- $h$: a household index (Eqs. 6-7, 17-18); in Eq. 13, $h_i$ is the peak hour of day $i$.
- $d$: a day type, weekday or weekend (Eqs. 6-7, 17-18); in Section 2.10, a day of the year.
- $k$: an hour-of-day index, $k = 1,\dots,24$.
- $a$, $c$: a dwelling archetype and a city (Section 2.9); $C_a$ is the set of cities simulated for archetype $a$.
...
- $R_{d,k}$: the fixed reference schedule of the fixed-schedule arm (Eq. 17); $S$: an hourly schedule value (Eqs. 17-18).
