#!/usr/bin/env python3
"""
T06 -- WP6 on 2022: end use x hour of day. v2.
Task doc: 2J_docs_occ_nTemp/writing/submission/rejection revision/impl/2026-09-15_T06_wp6_enduse_by_hour_2022.md

v2 changes vs enduse_hour_2022.py (job 1328259, FAILED exit 1, 20s):
  1. BUG FIX: INPUT_ROOT was ".../input/campaign_N50" but the uploaded tar
     (t06_2022_input.tar.gz) extracts its 24 cell directories directly under
     ".../input/" (no "campaign_N50/" root inside the tar, despite the Ledger's
     claim otherwise) -- confirmed on Speed: after `tar xzf` (exit 0), `ls
     input/` shows the 24 "<Arch>__<City>" dirs directly, and
     `input/campaign_N50` does not exist (FileNotFoundError at
     discover_2022_runs -> os.listdir(results_dir)). Fixed by pointing
     INPUT_ROOT at the input dir itself. Submit script's re-extract guard
     updated to match (checks a real cell dir, not the phantom campaign_N50).
  2. Manager note 2026-09-15 (this task doc, "Next"): the closure test as
     specified was wrong -- Electricity:Facility also contains electric
     HVAC + water-heating consumption, so lights+equipment+fans cannot close
     to it, and the paper's 0.07% figure is a different check (tabular vs
     hourly Facility, not this component sum). Fix: no pass/fail closure
     threshold. Instead compute and report the remainder
         hvac_dhw_elec_kWh = Electricity:Facility - (lights + equipment + fans)
     as its own end use, in the annual output AND as its own hourly profile
     (same weekday/weekend, season, per-archetype breakdown as every other
     meter), and report it per archetype in place of the old 0.07% pass/fail.
  No other metric changed (grid metrics, weighting, meter rules all identical
  to v1 / 08_simulation_plots.py).

Reads the Step-8 campaign's 2022 hourly meter CSVs (1,200 households: 4 archetypes x 6 cities x 50),
uploaded to /speed-scratch/o_iseri/2J_revision/T06/input/<cell>/sample_NNN_HHxxx/2022/hourly_meters.csv
plus each cell's cell_manifest.csv and cell_manifest.csv.new_2022_2030_* (same directory structure
as the local BEM_Setup/SimResults_Step8/campaign_N50 tree, minus the campaign_N50/ root -- see fix
#1 above).
...
Remainder end use (v2, manager note 2026-09-15):
  - hvac_dhw_elec_kWh (annual) / meter label "HVACDHW:Electricity" (hourly profile) =
    Electricity:Facility - (InteriorLights:Electricity + InteriorEquipment:Electricity +
    Fan Electricity Energy), only computed when all four source columns are present.
"""
import os
import re
import glob
...
# --- fix #1: no "campaign_N50" root inside the uploaded tar -- point at input/ itself ---
...
# -- remainder end use (v2) --
REMAINDER_METER_LABEL = "HVACDHW:Electricity"   # synthetic meter name for the hourly profile output
REMAINDER_ANN_COL = "hvac_dhw_elec_kWh"         # annual column name
...
def process_one(rm):
    ...
    # -- remainder end use (v2): hvac_dhw_elec = Facility - (lights+equip+fan) --
    have_remainder = all(m in meter_2d for m in ELEC_COMPONENTS) and FACILITY in meter_2d
    if have_remainder:
        remainder_2d = meter_2d[FACILITY] - sum(meter_2d[m] for m in ELEC_COMPONENTS)
    else:
        remainder_2d = None
    ...
    annual[REMAINDER_ANN_COL] = float(remainder_2d.sum()) if remainder_2d is not None else np.nan

    # -- (d) closure / remainder check (v2: report the remainder, no 0.07% pass/fail) --
    # Manager note 2026-09-15: Electricity:Facility also contains electric HVAC + water-heating
    # electricity, so lights+equipment+fans cannot close to it -- that is expected, not an error.
    # Report the remainder itself (= hvac_dhw_elec_kWh) per household; per-archetype aggregation
    # happens in main().
    closure = dict(key)
    if have_remainder:
        ...
        closure["hvac_dhw_elec_kWh"] = float(fac_sum - comp_sum)  # the remainder = annual[REMAINDER_ANN_COL]
        closure["hvac_dhw_elec_pct_of_facility"] = float(100.0 * (fac_sum - comp_sum) / fac_sum) if fac_sum else np.nan
    else:
        closure["components_kWh"] = closure["facility_kWh"] = np.nan
        closure["hvac_dhw_elec_kWh"] = closure["hvac_dhw_elec_pct_of_facility"] = np.nan
...
def main():
    ...
    print(f"[T06 v2] discovering 2022 runs under {INPUT_ROOT} ...")
    runs = discover_2022_runs(INPUT_ROOT)
    print(f"[T06 v2] discovered {len(runs)} candidate 2022 household runs")

    n_proc = min(8, cpu_count())
    print(f"[T06 v2] processing with {n_proc} workers")
    ...
    if not annual_df.empty:
        arch_ann_cols = [ANN_COL[m] for m in KEEP_METERS] + [REMAINDER_ANN_COL]
        per_arch = annual_df.groupby("arch")[arch_ann_cols].mean()
        stock_row = {}
    ...
    # -- v2: remainder (hvac_dhw_elec) reported per archetype, no 0.07% pass/fail --
    # NOTE (job 1328268, FAILED exit 1): .agg([...]) on 2 columns produces MultiIndex tuple
    # columns; json.dump cannot serialize tuple dict keys ("keys must be str, int, float, bool
    # or None, not tuple") at json/encoder.py:376. Fixed by flattening the MultiIndex columns to
    # "<col>_<stat>" strings before to_dict().
    remainder_by_arch = None
    if not closure_df.empty:
        agg = (closure_df.groupby("arch")[["hvac_dhw_elec_kWh", "hvac_dhw_elec_pct_of_facility"]]
               .agg(["mean", "median", "std", "min", "max", "count"]))
        agg.columns = ["_".join(c) for c in agg.columns]
        remainder_by_arch = agg.to_dict(orient="index")
    ...
    meta = {
        ...
        "hvac_dhw_elec_kWh_summary": (closure_df["hvac_dhw_elec_kWh"].describe().to_dict()
                                       if not closure_df.empty else None),
        "hvac_dhw_elec_pct_of_facility_by_archetype": remainder_by_arch,
    }
    with open(os.path.join(OUT_DIR, "run_meta.json"), "w") as f:
        json.dump(meta, f, indent=2, default=str)

    print(f"[T06 v2] households_read_ok={len(annual_df)} skipped={len(skipped)}")
    print(f"[T06 v2] done in {time.time()-t0:.1f}s")
