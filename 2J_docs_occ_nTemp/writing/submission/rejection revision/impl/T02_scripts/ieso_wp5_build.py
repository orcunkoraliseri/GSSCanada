    ...
    with zipfile.ZipFile(io.BytesIO(data)) as zf:
        ...
        if not names:
            ...
            return None, {"year": year, "month": month, "file_name": fname, "version": version,
                           "bytes": nbytes, "sha256": sha, "rows_kept": 0, "status": "NO_CSV"}
        with zf.open(names[0]) as fh:
            # First 3 lines are IESO header comments starting with \\ ; pandas skiprows handles it.
            df = pd.read_csv(fh, skiprows=3, usecols=USECOLS, dtype=DTYPES,
