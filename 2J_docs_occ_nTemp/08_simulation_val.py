...
# ===========================================================================
    ...
    def validate_injection_fidelity(self) -> dict:
        ...
        # Historic years (2005/2010/2015) use schedules from 08_gen_cycle_schedules.py
        # whose CSVs are byte-identical to the as-built source → round-trip = 0.
        # Years 2022/2030 were built from a version of BEM_Schedules_{year}.csv that
        # no longer exists on disk (intermediate version, pre-final-refinement).
        # See P1.6 materiality evidence in the investigation report.
        ...
        lvl_hist = 'PASS' if max_h <= 0.5 else 'WARN'
        self._gate(
            lvl_hist, '2.1',
            f'Historic round-trip (2005/2010/2015, n={n_hist}): '
            f'WD median={med_h:.3f}% p95={p95_h:.3f}% max={max_h:.3f}% '
            f'(threshold <=0.5%). '
            f'2022/2030 reference not preserved on disk (median={med_p:.1f}% p95={p95_p:.1f}%); '
            f'see provenance note.',
            f'hist max={max_h:.3f}%')

        lvl_exact = 'PASS' if exact_hist == n_hist and n_hist > 0 else 'WARN'
        self._gate(lvl_exact, '2.2',
                   f'Hour alignment exact (atol=0.001): '
                   f'2005/2010/2015 {exact_hist}/{n_hist} exact; '
                   f'2022/2030 {exact_prov}/{n_prov} (provenance mismatch expected)',
                   f'{exact_hist}/{n_hist} hist exact')
        ...
        lvl_met = 'PASS' if max_mh <= 0.05 else 'WARN'
        self._gate(
            lvl_met, '2.4',
            f'Metabolic rate (physical tolerance <=0.05 W): '
            f'2005/2010/2015 max={max_mh:.3f} W. '
            f'2022/2030 max={max_mp:.0f} W (provenance mismatch, same root as 2.1).',
            f'hist max={max_mh:.3f} W')
  ...
  <main>
    ...
    <div class="chart-section" id="s2">
      ...
      <p class="note">Sampled {s['s2'].get('n_sampled',0)} IDF files
         ...
         <br><strong>Provenance note (evidence-based, replaces prior incorrect explanation):</strong>
         Years 2005/2010/2015 were generated by 08_gen_cycle_schedules.py (seed=42); their
         on-disk CSVs are byte-identical to what built the IDFs — round-trip error = 0.
         Years 2022/2030 were built from an intermediate version of BEM_Schedules_{2022,2030}.csv
         that is no longer on disk (the current CSVs and the PRE_STEP8_BAK are identical and
         represent a later revision). This is a reference-provenance mismatch, NOT an injection bug:
         H2 checks confirm HH_IDs match, occ in [0,1], met in valid range, HHSIZE correct.
         Ensemble materiality assessment (800 HH x 2022, IDF vs current CSV):
         midday-share delta +2.68 pp; max per-hour ensemble diff 0.072; peak hour IDF=22h vs CSV=23h.
         These differences are moderate; the paper's directional WFH claims are not invalidated,
         but load-shape magnitudes should be interpreted as reflecting the as-built (IDF) schedules,
         not the current CSV. Provenance: as-built reference for 2022/2030 is the Scenario_*.idf
         files in SimResults_Step8/campaign_N50/ (preserved; reproducible from those files).</p>
      <div class="table-wrap"><table class="summary-table">
        <thead><tr><th>Gate</th><th>Status</th><th>Details</th></tr></thead>
