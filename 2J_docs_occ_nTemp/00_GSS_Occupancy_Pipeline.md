...
INPUT TO ENCODER
────────────────
• Observed diary sequence:  48 activity tokens + 48 AT_HOME tokens = 96 tokens
                            (30-minute resolution, output of Step 3E downsampling)
• Conditioning vector:      [demographic profile (one-hot + continuous)]
                    ...
                    ┌─────────────────────┐
                    │  Transformer Encoder │  ← Self-attention over 96 tokens (48 slots)
...
• Synthetic diary: 48 activity tokens (14 categories, 30-min resolution)
                 ...
                 = 96 output tokens total
```

### Training Strategy
- **Loss:** Cross-entropy over 14 activity categories × 48 slots + BCE over AT_HOME × 48 slots
...
Each diary: 96 tokens (48 activity + 48 AT_HOME at 30-min resolution)
