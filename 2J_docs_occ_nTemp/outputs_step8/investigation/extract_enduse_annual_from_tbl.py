"""extract_enduse_annual_from_tbl.py — Step 8 Fix v3 End-Uses extractor, LOCAL variant (2J port).

Ported from 3J_docs_occ_nTemp/Leg2_2-split/Step8_docs/investigation/extract_enduse_annual_from_tbl.py
(2026-07-08). Same output/schema (queries eplusout.sql via sqlite3 on the cluster in the sibling
extract_enduse_annual.py), but parses the much smaller eplustbl.csv "End Uses" table instead —
used for a local run while the Speed cluster is unavailable. Values cross-checked identical to
the sqlite path for MidRise sample_001 2022 (heating_gas_GJ=78.34, cooling_elec_GJ=136.12).
Stdlib only (os, glob, csv, re).

Only the default path resolution changed vs the 3J original: 2J's outputs_step8/ sits directly
under 2J_docs_occ_nTemp/ (no intermediate Step8_docs/ layer), so CAMP_DIR/OUT_PATH resolve one
level shallower. Cell/sample directory naming and regexes are unchanged (confirmed identical
layout: <arch>__<city>_<cz>/sample_NNN_HH<id>/<year>/eplustbl.csv, eSim_bem_utils_2J/main.py:2056).

Usage: py investigation/extract_enduse_annual_from_tbl.py
Env:   STEP8_CAMP_DIR (default outputs_step8/campaign_subset_v3_enduse relative to outputs_step8/)
...
CAMP_DIR = os.environ.get(
    "STEP8_CAMP_DIR",
    os.path.join(OUTPUTS_STEP8, "campaign_subset_v3_enduse"),
)
