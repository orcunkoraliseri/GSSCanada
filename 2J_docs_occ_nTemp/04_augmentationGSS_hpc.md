...
## Orchestration Refactor (YAML configs + sbatch arrays + auto-rsync)

*(Merged 2026-04-28 from former `04_augmentationGSS_orchestration.md`.)*

### Aim

Replace per-trial hand-edited shell scripts + manual scp with a YAML-config + sbatch-job-array + auto-rsync system. Zero science changes. F9-a and F9-b (`AUX_STRATUM_LAMBDA=0.02` and `SPOUSE_NEG_WEIGHT=0.4`) became the first sweep on the new system.

**Why:** Every F-trial previously cost 6 manual steps (~20 shell scripts for F1–F9). The refactor separates trial spec (YAML), trial mechanics (one parametrized job array), and result delivery (auto-rsync into a queryable CSV). One sweep cycle post-refactor = write a YAML, one `sbatch --array` submit, wait for email, read one CSV.
...
2. **`04D_train.py` edits** (backward-compatible):
   - `AUX_STRATUM_LAMBDA = float(os.environ.get("AUX_STRATUM_LAMBDA", "0.1"))` at module level; replaces inline `LAMBDA_AUX`.
...
3. **`config_to_env.sh`:** reads `configs/{tag}.yaml`, emits `export VAR=value` + `PY_ARGS`; uses `yq` if available, else falls back to `config_to_env.py`.
4. **`config_to_env.py`:** python fallback for clusters without `yq`.
...
- `2J_docs_occ_nTemp/Speed_Cluster/` has 5 new files: `config_to_env.sh`, `config_to_env.py`, `job_04D_train_array.sh`, `submit_step4_array.sh`, `extract_metrics.py`.
...
1. **YAML parse:** `python -c "import yaml; yaml.safe_load(open(...))"` on each config — no parse errors.
2. **bash -n verify:** `bash -n` on `job_04D_train_array.sh` and `submit_step4_array.sh` — syntax clean.
3. **Backward-compat:** `bash -n submit_step4_F8_retrain.sh` — old scripts still pass.
4. **On cluster (smoke + F1 bit-for-bit):** `bash submit_step4_array.sh configs/sweep_smoke.yaml` → verify `results_index/results.csv` gets 3 rows.
5. **Regression gate:** F1 smoke training_log.csv diff against F1 baseline — zero or float-noise drift only.

### Refactor Progress Log

**2026-04-26 — Session 1 (COMPLETE).** 9 tasks, zero science changes, 13 files created, 3 files edited. Backward-compatible: defaults preserve F8 behaviour exactly. F9-a (job 904709) and F9-b (job 904715) had been submitted manually before the refactor landed; both effectively ran F8 behaviour because `04D_train.py` did not yet read the new env vars. Cancelled and re-launched on the new system as the first sweep.

**2026-04-26 — Session 2 — smoke sweep debugging.** First smoke attempt failed with three bugs:
- *Bug 1 — tag-collapse in `submit_step4_array.sh`:* `tr -d '[:space:]'` deleted newlines, collapsing `F1\nF9a\nF9b` to a single string. Fix: `sed 's/[[:space:]]*$//'`.
- *Bug 2 — `04D_train.py` warmup gate blocks `--sample` mode:* in sample mode `warmup_epochs` (8) exceeded `max_epochs` (5), so `past_warmup` was never true and `best_model.pt` was never written; old F1 checkpoint with `d_cond=91` survived. Fix (line 670): `past_warmup = args.sample or (epoch + 1) > warmup_epochs`.
- *Bug 3 — `04E_inference.py` overrode `d_cond`:* line 287 replaced checkpoint's d_cond (91) with live feature config's d_cond (92), causing `RuntimeError: size mismatch`. Fix: removed the override; 04E now uses checkpoint's `model_config` unmodified.

**2026-04-27 — Session 3 — continued smoke debugging.** Three more bugs:
- *Bug 4 — `BASH_SOURCE[0]` resolves to SLURM spool dir at runtime:* `source "${SCRIPT_DIR}/config_to_env.sh"` silently failed → all trial-specific env vars never applied. Fix: hardcoded `SCRIPT_DIR="$BASE/Speed_Cluster"`.
- *Bug 5 — `DATA_DIR_04E` hardcoded to `outputs_step4`:* in smoke mode 04D writes to `outputs_step4_test`. Fix: smoke-aware `DATA_DIR_04E` variable.
- *Bug 6 — CRLF line endings on Windows-edited cluster scripts:* bash interpreted `\r` as part of command names. Fix: `dos2unix` on cluster + `*.sh text eol=lf`, `*.py text eol=lf` in repo `.gitattributes`.
- *Bug 7 — `outputs_step4_test/` missing inference metadata:* 04E loads `step4_all_meta.csv` and `step4_feature_config.json` from `args.data_dir`; smoke dir had only training tensors. Fix: symlink the two metadata files from `outputs_step4/` into `outputs_step4_test/`.

**2026-04-27 — System validated end-to-end:** smoke sweep passed (F1 + F9a + F9b smoke trials), real F9 sweep submitted via `sweep_F9.yaml`. Subsequent F10 4-arm Spouse-axis bracket sweep (F10a/b/c/d) ran cleanly through the new system with zero per-trial babysitting. F-series complete; orchestration system reusable for any future Step-4 architectural work.
...
| 2026-04-26 | **Orchestration refactor COMPLETE — per-trial shell scripts superseded by YAML configs + sbatch job arrays + auto-rsync.** Manual F9-a (job 904709) and F9-b (job 904715) were submitted before the refactor landed and then cancelled (all 12 jobs 904709–904720 cancelled). Their configs are preserved as `configs/F9a.yaml` and `configs/F9b.yaml` and will be the first sweep targets when re-submitted on the new system. New files: `configs/_schema.md`, `configs/F1.yaml`, `configs/F8.yaml`, `configs/F9a.yaml`, `configs/F9b.yaml`, `configs/sweep_F9.yaml`, `configs/sweep_smoke.yaml`; `Speed_Cluster/job_04D_train_array.sh`, `submit_step4_array.sh`, `config_to_env.sh`, `config_to_env.py`, `extract_metrics.py`. `04D_train.py` edited: `AUX_STRATUM_LAMBDA` and `SPOUSE_NEG_WEIGHT` now read from env at module level (defaults preserve F8 behaviour). Full design captured in the **Orchestration Refactor** section above (merged 2026-04-28 from former `04_augmentationGSS_orchestration.md`). Old per-trial scripts untouched (backward-compat fallback). | COMPLETE |
| 2026-04-25 → 2026-04-28 | **F4 → F10 sweep summary (deep detail in `Speed-Cluster_docs/step4_training.md` §10.7).** F4 INVALID (FP16 NaN from rare-channel cop pos_weights). F5 INVALID (residual NaN, fp16). F6 INVALID (fp32 cleared NaN but warmup-induced early-stop trap saved epoch-1 near-uniform weights). F7 first valid run since F1: composite 1.364, both hard gates PASS, but Spouse +20.3 pp emerged as new dominant failure. F8 isolated AUX_STRATUM_HEAD=1 (single delta vs F1) and confirmed it singularly causes both AT_HOME improvement (+5.3 → +1.41 pp, best of all runs) and Spouse explosion (+19.4 pp); composite 1.376. F9 sweep on the new YAML/array system: F9a (`AUX_STRATUM_LAMBDA=0.02`) drove Spouse the wrong way (+25.4 pp) — aux-lambda axis closed; F9b (`SPOUSE_NEG_WEIGHT=0.4`) overshot the Spouse zero-crossing to −3.5 pp, composite 1.481. F10 4-arm bracket sweep at SPOUSE_NEG_WEIGHT ∈ {0.45, 0.50, 0.55, 0.60} closed the Spouse axis. | COMPLETE |
| 2026-04-28 | **F10 sweep COMPLETE — F-series CLOSED at F10a. HARD STOP, no F11.** Results from `results_index/results.csv`: F10a (0.45) composite **1.306**, AT_HOME 6.98 pp, Spouse **+1.60** pp, act_JS 0.0686 — Pareto-best. F10b (0.50) 1.399 / 7.08 / +2.52 / 0.069. F10c (0.55) 1.428 / 6.67 / +5.42 / 0.071. F10d (0.60) 1.357 / 7.45 / +8.04 / 0.069. Spouse zero-crossing landed below 0.45, not at the linear-interp prediction of 0.49 — F8↔F9b interpolation non-linear. **Hard-gate audit:** Spouse gate ✅ all four arms; composite ✗ all (best 1.306, target 1.045 — 25 % over); AT_HOME ✗ all (best 6.67 pp, target 5.3); act_JS ✗ all (best 0.069, target 0.05). **Verdict:** single-axis HP exploration exhausted. F1 → F10 (10 trials, ~28 GPU-days on `pg`) drove composite from 1.802 → 1.306 — a structural floor, not a tuning gap. **F10a accepted as the chosen Step-4 configuration** (Pareto-best; Spouse gate cleared; the other three gate misses are architectural). **Step-4 training is closed at F10a as best-achievable-under-current-architecture.** Any further work belongs to a separate architectural task (decoder capacity / cross-attention conditioning / loss reformulation), not an F11. Orchestration refactor validated end-to-end across 6 trials (F1 smoke + F9a + F9b + F10a–d) with zero per-trial babysitting; reusable for any future Step-4 architectural work. | COMPLETE — STEP-4 CLOSED |
| 2026-04-28 | **Step-4 v2 (G-series) opened — architectural rebuild after F-series closure. Deep detail in `Speed-Cluster_docs/step4_training_v2.md`.** F10a accepted as best-under-current-architecture; structural gate failures (AT_HOME, act_JS, cop_cal_MAE) are not loss-weight tunable. External proposal (`Speed-Cluster_docs/newArchProposal.md`) reviewed and accepted as direction; "F11 single trial" framing rejected because it bundles five independent changes (cross-attention, scheduled sampling, label smoothing, proportional sampling, decoder capacity). Adopted staged plan: **G1** proportional target sampling in `04C` (data-pipeline only — cheapest, highest-confidence single fix for H1 weekend over-representation); **G2** scheduled sampling + label smoothing in `04D` (training-loop only — targets H3 autoregressive saturation); **G3** cross-attention conditioning + decoder capacity in `04B` (architectural — targets H4 conditioning collapse). Each stage gated independently; G1 first, no bundling. Default env preserves F10a behaviour for backward-compat. | PLANNED |
| 2026-04-28 | **G1 smoke: 04C PASS (job 906102), 04D INVALID (job 906104) — resubmit pending. Deep detail in `Speed-Cluster_docs/step4_training_v2.md`.** **04C PASS:** 141,270 pairs generated; target-strata counts {WD:64480, Sat:38668, Sun:38122} match the replication scheme exactly. **04C finding contradicts H1:** `obs_home_rate` moved 0.725 → **0.7346** (UP, not down). Back-calc gives WD-target diaries ~75% AT_HOME vs ~72% for weekend-target diaries — i.e. weekday over-representation, not weekend, raises AT_HOME. The H1 premise driving G1 is not supported by the data; G1 is unlikely to close the AT_HOME gate, and **G2 (H3 autoregressive feedback) is now the more likely fix**. G1 will still complete as a comparison data point. **04D INVALID:** the smoke was submitted via `sbatch --wrap`, which bypassed the YAML-loading path (`config_to_env.sh`); job picked up F1-baseline env (AUX_STRATUM_HEAD=False, SPOUSE_NEG_WEIGHT=1.0) instead of F10a. Discarded. **Fix:** new `configs/sweep_G1.yaml` (single-tag driver) routes through the array machinery `submit_step4_array.sh` → `job_04D_train_array.sh` → `config_to_env.sh` → `configs/G1.yaml`, correctly setting AUX_STRATUM_HEAD=1, SPOUSE_NEG_WEIGHT=0.45, and all F10a inheritance. Symlinks `step4_train.pt` / `step4_val.pt` / `step4_feature_config.json` already populated in `outputs_step4_G1/` from job 906104. Next: scp `configs/sweep_G1.yaml` to cluster, then `bash Speed_Cluster/submit_step4_array.sh configs/sweep_G1.yaml G1_$(date +%Y%m%d_%H%M)`. G2 prep recommended in parallel given the H1 contradiction. | RESUBMIT PENDING |
| 2026-04-28 | **G1 implemented — proportional target sampling in `04C_training_pairs.py`; smoke pending.** New `--proportional` and `--output_dir` flags. Per-source pair rows replicated by integer-rounded pop_freq ratio (WD src → Sat:1,Sun:1 unchanged; Sat src → WD:5,Sun:1; Sun src → WD:5,Sat:1) computed from actual strata counts {WD:31947, Sat:6175, Sun:6721}. Effective target-strata distribution after 04D's source-stratum-equalized `WeightedRandomSampler` ≈ **55.6% WD / 22.2% Sat / 22.2% Sun** (vs uniform 33/33/33). Structural ceiling: WD-sourced respondents (71% of population) can't target WD, so true 71/14/15 proportional is unreachable under this scheme — 55.6 is the asymptote of integer replication + inverse-source-stratum sampling. Expected `obs_home_rate` shift: ~0.725 → ~0.70 (toward pop-weighted ~0.65), confirmed at runtime by `--proportional` log line. Val pairs replicated under the same scheme. Output: `outputs_step4_G1/training_pairs.pt` (141,270 rows vs 89,686 baseline). New `configs/G1.yaml` (F10a base + `data_dir: outputs_step4_G1`, gates tightened to Spouse ≤ +5 pp). Diff scope: `04C_training_pairs.py` + `configs/G1.yaml` only. Next: smoke on `pg` per v2 §5 (`configs/sweep_smoke.yaml` 1-epoch single-GPU). | IMPLEMENTED — pending smoke |
