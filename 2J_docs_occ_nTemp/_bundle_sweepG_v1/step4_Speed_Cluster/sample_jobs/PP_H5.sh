#!/encs/bin/bash
#SBATCH --job-name=PP_H5
#SBATCH --partition=pg
#SBATCH --gres=gpu:1
#SBATCH --mem=40G
#SBATCH --time=48:00:00
#SBATCH --output=/speed-scratch/o_iseri/occModeling/logs/PP_H5_%j.out
#SBATCH --error=/speed-scratch/o_iseri/occModeling/logs/PP_H5_%j.err

# K=10 neighbors + ORIGINAL mask bounds (test architecture interaction).
set -e
. /encs/pkg/modules-5.3.1/root/init/bash
module load cuda/12.8

export KNN_K=10
# No MDLM_MASK_LO/HI export — uses default 0.05/0.95
...
# Reuse K10 data dir from H2 (already created with symlinks + K=10 pairs)
if [ ! -d "$DATA" ]; then
    mkdir -p "$DATA"
...
echo "=== ${TAG} TRAIN START (K=10 + original mask) ==="
