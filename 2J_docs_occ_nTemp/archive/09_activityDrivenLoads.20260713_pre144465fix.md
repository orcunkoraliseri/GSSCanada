...
> ✅ **CORRECTED RE-RUN COMPLETE 2026-06-10.** 4-hour schedule-injection bug (found 2026-06-08) fully resolved. `07_aug_to_bem.py` now rolls occ24/met24/equip_frac/light_frac +4 h before writing EnergyPlus schedules. Full 24-cell/n=50 corrected campaign COMPLETE (jobs 954482→954635→954636→954637). **Corrected headline: equipment peak shift = 0 ± 1 h** (was −4 h, which was entirely the injection artifact). SHEU 48/48 PASS unchanged (phase-invariant). Figures figS1–S8 + figV1 + HTML report regenerated from corrected CSVs 2026-06-10. SI fixes from the investigation: **R1** (lighting = binary occupied-&-awake, no daylight gate) + **R4** (gross 3700 / net 3252) valid; **R2/R3** (offset-is-benign) rejected. Details: `Step9_docs/investigation/step9_investigation.md`. **Remaining:** SI text (S5.1/S5.2 timing tables) still shows −4 h values — needs update to 0 ± 1 h before submission.
...
- [x] **Plots** — DONE (corrected 2026-06-10). figS1–S8 (`Step9_docs/figures/` + `outputs_step9/`) + figV1 + HTML report (`outputs_step9/`) all regenerated from corrected CSVs. Equipment peak shift now shows 0 h (BL ≈ AC peak hour). 9 SI figures on disk.
- [x] **Write-up** for the supplementary (SI/Appendix) section — DONE 2026-06-08. `Step9_docs/si_appendix_step9.md` rewritten submission-ready. ⚠️ **SI text S5.1/S5.2 timing tables still reference buggy −4 h shift — needs update to 0 ± 1 h before submission.** Numbers that remain valid: SHEU calibration (48/48, max |dev| equip 2.33 %, light 2.63 %), G3 sleep WARN 12/48 (OtherDwelling only in corrected run).
...
> **As-built correction (R1, 2026-06-08; doc annotated 2026-06-10):** the shipped model is
> **`lighting(t) = binary occupied-&-awake(t)`, scaled to the SHEU lighting total — with NO
> daylight gate** (investigation finding R1). The daylight-gated design in the next paragraph was
> the original plan and was **not implemented**; it is retained for history only. Any paper/SI text
> must describe the occupied-&-awake formulation.

*(Superseded design)* `lighting(t) = active_occupancy(t) × daylight_gate(t)`, scaled to the SHEU lighting total. The engine
...
| 2026-06-07 | Step 9 full-grid campaign (BUGGY RUN) — all phases complete | ⚠️ COMPLETE but SUPERSEDED by corrected re-run | Stage A (gen): job 951154 COMPLETE. Precheck 4/4 cells PASS. Smoke 12/12 PASS. Array: job 951598 COMPLETE 4795/4800 (5 warmup fails). Validate: job 951806 — 48/48 SHEU gates PASS (equip max |dev| 2.5%, light max 2.3%). Loadshape: `loadshape_profiles.csv`, `peak_shift_summary.csv` generated. figS1–S8 + figV1 + HTML rendered to `outputs_step9/`. **Reported headline: equip_bldg_shift = −4 h uniformly.** ⚠️ This result is the injection bug artifact — see 2026-06-08 entry below. Annual SHEU calibration (48/48) is phase-invariant and carries over to corrected run. |

| 2026-06-08 | 4-hour schedule-injection bug found + fix applied + corrected re-run initiated | 🔧 FIX APPLIED locally | Manager independently discovered: `07_aug_to_bem.py` wrote GSS 4 AM-origin diary slots positionally (slot@04:00 → Hour 0) instead of rotating to clock (slot@04:00 → Hour 4). The CLASSIC converter `21CEN22GSS_occToBEM.py` used `.dt.hour` resampling and was correct. Confirmed via `investigation/offset_check.py`: BEM_Schedules_2022.csv vs CLASSIC_BAK is a clean +4 h circular shift (SSE 0.043→0.007 at roll +4). **Fix:** `07_aug_to_bem.py` now rolls occ24/met24/equip_frac/light_frac arrays by +4 before writing; predecessor archived `archive/07_aug_to_bem.20260608.py`. BEM_Schedules_2022.csv regenerated locally (144,507 HH); offset_check best-shift drops to 0 (SSE 0.0074). Decision: full re-run on cluster. Buggy run results preserved in `/speed-scratch/o_iseri/step9_run_buggy/`. Old run dir moved aside; Stage A re-run initiated on cluster. All timing claims from the Jun-7 buggy run are invalidated; SHEU 48/48 stands. |

| 2026-06-10 | Corrected re-run COMPLETE — recovery chain jobs 954482→954635→954636→954637 | ✅ CORRECTED COMPLETE | **Stage A** (corrected IDF gen): COMPLETE (07_aug_to_bem.py ran on cluster, regenerated BEM_Schedules_2022.csv + 2030.csv with +4 h roll; IDFs rebuilt). **Array 951598 (corrected)**: 4795/4800 COMPLETE (5 remaining MidRise/HighRise IDFs — 0.1% miss, n=49 in those buckets). **Recovery job 954482** (32 missing IDFs): FAILED 1:0 (expected — 11 HighRise/MidRise IDFs blocked by `SEVERE > 0` warmup-convergence check; 21 others extracted). **Extract-11 job 954635** (afterany:954482): COMPLETED 0:0 in 51 s — extracted hourly_meters.csv for the 11 Severe-blocked IDFs (E+ had completed successfully; only the Severe-check gate failed). **Validate job 954636**: COMPLETED 0:0 in 33:25 — **ALL 48 SHEU gates PASS** (equip max |dev| 2.33%, light max 2.63%, all within ±15%). Sleep WARN 12/48 (OtherDwelling only, multi-unit fridge sum — expected). **Loadshape job 954637**: COMPLETED 0:0 in 6:59 — `loadshape_profiles.csv`, `peak_hours.csv`, `peak_shift_summary.csv` regenerated. **Corrected peak-shift headline: equip_bldg_shift = 0 ± 1 h** across all 24 cells, both years (range −1..+1). The −4 h artifact is confirmed eliminated. Zone-level artifact (equip_zone_shift = −17 h for HighRise/MidRise) persists — fridge-dominated unit-1 zone meter, unrelated to injection bug; building-level metric is canonical. figS1–S8 regenerated to `Step9_docs/figures/`. Full corrected comparison table (48 rows) in `Step9_docs/cluster_run.md` Progress Log 2026-06-10. 5 missing IDFs (MidRise_Toronto/bl, MidRise_Kelowna/act, MidRise_Vancouver/bl, MidRise_Montreal/act, HighRise_Winnipeg/bl) documented as known limitation. |

| 2026-06-10 | figV1 + HTML report regenerated from corrected CSVs | ✅ COMPLETE | Corrected figS6–S8 copied from `Step9_docs/figures/` to `outputs_step9/`. `09_activityDrivenLoads_val.py` script fixes: G2c gate updated to `< 3.0 %` (max observed 2.63 %); G4 expected text updated from "≈ −4.1 h" to "≈ 0 h (range −1..+1)"; HTML date updated to 2026-06-10. Re-ran locally: **6 PASS / 1 WARN / 3 INFO / 0 FAIL**. G4 computed: equip 2022 mean = −0.12 h σ = 0.44; 2030 mean = −0.12 h σ = 0.33; range −1..+1. G4x (shift persistence): Δ = 0.000 h. figV1 now shows BL and AC peaks at the same hour (0 h shift). All `outputs_step9/` figures updated. |

| 2026-06-10 | SI appendix (`si_appendix_step9.md`) rewritten for corrected campaign | ✅ SI UPDATED | **Sections changed:** §S4.3 (cross-ref to §S4.6 updated), §S4.6 (lighting method: replaced "active_occupancy × daylight_gate" with correct description — binary occupied-AND-awake indicator, NO daylight gating in shipped `activity_loads.py`), §S4.8 grid/completeness stats (4,795 runs / 5 missing 0.1%, named the 5 missing buckets; WARN 12/48 not 28/48, OtherDwelling only 426–505 Wh; max |dev| equip 2.33% MidRise_Toronto_5A 2022, light 2.63% MidRise_Toronto_5A 2022; Fig S4 caption updated), §S5.1 (reframed as null-peak-shift result, acknowledged corrected re-run context), §S5.3 (replaced −4 h finding: corrected equip_bldg_shift = 0 ± 1 h, mean −0.12 h; both arms peak h17–18 equip and h18–21 light; Fig S9 text corrected), §S5.4 (prototype note updated to h17–18 corrected peak), **Tables S5.1 and S5.2** (all 48 per-cell rows rebuilt from `peak_shift_summary.csv` + `peak_hours.csv` — corrected values replace buggy −3 to −5 h entries; summary footnotes updated to 0 ± 1 h, no meaningful displacement), §S5.6 (Figs S6–S9 captions rewritten: evening-coincident peaks, null shift, Fig S7 now "essentially coincident dumbbells"), §S6 (excluded-runs count: 8 → 5 missing, 0.1%). **Deleted language:** "daylight gate" from §S4.6 (was wrong), all "−4 h shift" and "early afternoon h13–14" timing claims from §S5, all "mean −4.1 h" summary stats. **R2/R3 benign-offset language:** not present in SI text (no "benign offset" phrases found to delete). **Fridge gross/net (R4):** Table S4.2 and §S4.5 prose were already correct (gross 3,700 / net 3,252 clearly distinguished); no change needed. No citations or References section changed. |
