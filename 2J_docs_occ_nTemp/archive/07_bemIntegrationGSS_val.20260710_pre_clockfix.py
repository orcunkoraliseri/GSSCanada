...
class BEMIntegrationValidator:
    ...
    def validate_step9_channels(self) -> dict:
        ...
        l_peak_clock = _clock(int(lf_by_h.idxmax()))
        l_evening = (l_peak_clock >= 17) or (l_peak_clock <= 1)
        ...
        self._sum("Lighting evening peak", "17-01h", f"{l_peak_clock:02d}h",
    ...
    <div class="provenance">
      ...
      the Step-6 person-diary 76.93%). Regenerated {ts}.
        ...
        dev_rows = [
            ("Metabolic / activity channel un-calibrated",
             "Only hom30 (occupancy) was raked; act30 → Metabolic_Rate rides the raw J3 / forecast mix.",
             "INFO — occupancy (the dominant EnergyPlus driver) is calibrated; internal-gain magnitude "
             "carries the model's activity mix. Documented limitation."),
        ...
        <section class="chart-section" id="deviations">
          ...
          <p class="caption" style="margin-top:14px">
            <strong>Limitations (paper-ready).</strong> The Step-7 BEM schedules calibrate the dominant
            occupancy channel (hom30, raked to the Census-linked 2022 stock and the Step-6 2030 forecast)
            to within ≤1 pp of the diary marginals. The metabolic and Step-9 internal-gain channels ride
            the model's (un-raked) activity mix on a conservative 70 W/MET (~60 kg) basis, and
            Saturday/Sunday are pooled into a single Weekend profile per the EnergyPlus 2-day-type
            contract. None of these affects the occupancy signal EnergyPlus uses for presence; they bound
            the internal-gain magnitude and are documented as limitations.
          </p>
        </section>"""
