...
calibrated to NRCan SHEU 2019 targets (per dwelling type).
...
2026-06-03: added per-DTYPE SHEU targets (SHEU_BY_DTYPE) for HighRise + MidRise cells.
Predecessor archived at archive/activity_loads.20260603.py.
...
# Per-dwelling SHEU 2019 calibration targets (DR-1 §9.4).
# tuple: (equip_net_kwh, light_kwh)
# SingleD:       published — appliances 3700 kWh, lighting 1262 kWh; net = 3700 - 448 fridge = 3252
# HighRise:      total 6583 kWh. Appliances = 29.2%×6583 = 1922 kWh; net = 1922 - 448 = 1474 kWh.
#                Lighting = 736 kWh (published national apartment, DR-1).
# MidRise:       total 7417 kWh (low-rise apt). Appliances = 29.2%×7417 = 2166; net = 1718 kWh.
#                Lighting = 736 kWh.
# OtherDwelling: total 10750 kWh (attached). Appliances = 29.2%×10750 = 3139; net = 2691 kWh.
#                Lighting = 1100 kWh (midpoint SingleD/apt).
# NOTE: Apartment values are DERIVED (§9.4 footnote): appliance fraction (29.2%) is the
# SingleD ratio (3700/12694) applied to each dwelling's SHEU total. SHEU does not publish
# per-end-use splits for apartments; these are model-grade estimates. Deviation documented
# in cluster_run.md §Deviations.
SHEU_BY_DTYPE = {
    'SingleD':       (3252.0, 1262.0),
    'HighRise':      (1474.0,  736.0),
    'MidRise':       (1718.0,  736.0),
    'OtherDwelling': (2691.0, 1100.0),
}
...
def calibrate_schedules(raw: dict, dtype: str = 'SingleD') -> dict:
    """Scale 48-slot raw schedules to per-DTYPE SHEU targets.

    RF2 fix: calibration target for equipment is the net kWh (after fridge subtraction),
    so the IDF's refrigerator1 object and the STEP9 BASELOAD_W are not double-counted.
    Targets are looked up from SHEU_BY_DTYPE; falls back to SingleD if dtype not found.
    ...
    dtype : str
        Dwelling type string matching SHEU_BY_DTYPE keys (e.g. 'SingleD', 'HighRise').
    ...
    """
    equip_target, light_target = SHEU_BY_DTYPE.get(dtype, SHEU_BY_DTYPE['SingleD'])
    ...
    # Equipment calibration (RF2: use per-DTYPE net target, fridge already in IDF)
    equip_kwh_raw = annual_kwh(eq_wd, eq_we)
    peak_equip    = max(float(np.max(eq_wd)), float(np.max(eq_we)), 1.0)

    if equip_kwh_raw > 0:
        equip_scale    = equip_target / equip_kwh_raw
    ...
    # Lighting calibration: D_light * total_frac_hours/yr / 1000 = light_target
    total_frac_h = (N_WEEKDAY * float(np.sum(lt_wd)) +
                    N_WEEKEND * float(np.sum(lt_we)))
    light_kwh_raw_at_1W = total_frac_h / 1000.0
    if total_frac_h > 0:
        light_design_W = light_target * 1000.0 / total_frac_h
    else:
        light_design_W = 0.0
