#!/encs/bin/bash
#SBATCH --job-name=s9_warmup120
#SBATCH --partition=ps
#SBATCH --mem=8G
#SBATCH --cpus-per-task=2
#SBATCH --time=48:00:00
#SBATCH --output=/speed-scratch/o_iseri/step9_run/logs/s9_warmup120_%j.out
#SBATCH --error=/speed-scratch/o_iseri/step9_run/logs/s9_warmup120_%j.err

# Warmup-120 recovery: re-run 8 HighRise warmup-convergence Severe failures with
# Maximum Number of Warmup Days raised from 25 to 120 (restored from .bak_warmup60).
# MidRise Toronto HH1865/2030 already resolved in job 951832 (s9_warmup60) — NOT included.
# MidRise Calgary HH78358/2022 is EXCLUDED (HVAC blow-up) — NOT included.
# Manager decision: Option C — 120 is the practical E+ ceiling; one decisive shot.
# 8 HighRise runs: 4×Toronto_5A, 3×Vancouver_5C, 1×Winnipeg_7A.
...
echo "=== Step 9 warmup-120 recovery | job=${SLURM_JOB_ID} ==="
...
recover_run() {
    ...
    # Verify .bak_warmup60 exists (created by job 951832)
    if [ ! -f "$OUT_DIR/${IDF_NAME}.bak_warmup60" ]; then
        echo "  ABORT: .bak_warmup60 missing for $LABEL — cannot safely patch to 120"
        return 1
    fi

    # Verify backup still shows 25 (not already 60)
    if grep -q "60,.*!- Maximum Number of Warmup Days" "$OUT_DIR/${IDF_NAME}.bak_warmup60"; then
        echo "  ABORT: .bak_warmup60 already reads 60 for $LABEL — backup is corrupted, cannot safely patch"
        return 1
    fi

    # Restore original IDF from backup (returns it to 25 days)
    cp "$OUT_DIR/${IDF_NAME}.bak_warmup60" "$IDF_PATH"
    echo "  Restored from .bak_warmup60 (original 25-day IDF)"

    # Log current tolerance values for the record (do NOT modify)
    ...
    # Verify patch took — abort if not
    if ! grep -q "120,.*!- Maximum Number of Warmup Days" "$IDF_PATH"; then
        echo "  ABORT: patch verification FAILED — Maximum Number of Warmup Days does not read 120 in $LABEL"
        return 1
    fi
...
echo ""
echo "=== Warmup-120 recovery complete: succeeded=$DONE  failed=$FAIL ==="
echo "Date: $(date)"
