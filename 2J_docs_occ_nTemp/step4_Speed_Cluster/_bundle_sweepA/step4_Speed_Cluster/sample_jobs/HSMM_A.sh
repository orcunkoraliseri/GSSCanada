#!/encs/bin/bash
#SBATCH --job-name=HSMM_A
#SBATCH --partition=pg
#SBATCH --gres=gpu:1
#SBATCH --mem=40G
#SBATCH --time=48:00:00
#SBATCH --output=/speed-scratch/o_iseri/occModeling/logs/HSMM_A_%j.out
#SBATCH --error=/speed-scratch/o_iseri/occModeling/logs/HSMM_A_%j.err

# Phase 6 Stage A Trial 4: Neural HSMM decoder (per-slot state + duration heads) + CC.
# 2% stratified sample (~3k respondents), 50 epoch cap, ~1h wall-clock on pg.
# Train -> infer -> 04H -> 04I -> 04J. Output: outputs_step4_sample/HSMM_A/
