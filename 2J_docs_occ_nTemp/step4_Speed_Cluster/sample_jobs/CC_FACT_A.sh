#!/encs/bin/bash
#SBATCH --job-name=CC_FACT_A
#SBATCH --partition=pg
#SBATCH --gres=gpu:1
#SBATCH --mem=40G
#SBATCH --time=48:00:00
#SBATCH --output=/speed-scratch/o_iseri/occModeling/logs/CC_FACT_A_%j.out
#SBATCH --error=/speed-scratch/o_iseri/occModeling/logs/CC_FACT_A_%j.err

# Phase 6 Stage A Trial 3: CC + factorized joint decoding (heavier consistency penalty).
# 2% stratified sample (~3k respondents), 50 epoch cap, ~1h wall-clock on pg.
# Train -> infer -> 04H -> 04I -> 04J. Output: outputs_step4_sample/CC_FACT_A/
