...
### J-4.1 / J-4.2 / J-4.3 — Parallel precision ladder beyond J3 (PARALLEL EXECUTION on cluster)

**Diagnosis from J3 04J:** all four hard gates pass (composite=0.6355, AT_HOME RMS=4.57 pp, Spouse=−2.03 pp, act_JS=0.0191). v2 investigation recommended Step-4 closeout. User overrode that decision: push for absolute precision against the two residual axes — per-stratum AT_HOME wobble (still >4 pp, margin small) and Alone-channel cop_max_gap on 2005_1 (+21.1 pp) / 2010_1 (+17.1 pp). Three single-axis arms run **simultaneously** as three sbatch jobs against the `pg` partition, each forked from frozen `Speed_Cluster/archive/04B_model_J3.py`. After all three complete, compare diagnostics and ship the simplest arm that closes its target without breaking any of J3's four passing gates.

**Why parallel, not sequential:** wall time drops from ~51 h to ~17 h; comparison data clarifies which Blueprint hypothesis (temporal anchoring vs. hierarchical heads vs. logic-loss PINN) is the actual fix; all three baselines are identical (forked from the frozen J3 binary). Same pattern that exposed `arm2_act_proj` as the load-bearing fix in the J2 / J2.5 / J3 ladder.

**Scope discipline:** each arm changes **exactly one axis** vs J3. No bundling. Stacked combinations (e.g. J-4.1 + J-4.2) are deferred to an optional J-4-stack follow-up arm, conditional on multiple arms passing.

**Source spec:** `investigations/J-4 Tasks.md` (this doc's task block summarizes; the full buildable spec lives there). Hypothesis-level rationale: `investigations/J-4 Blueprint.md` (drafted with separate LLM, 2026-05-07).

**Hard-gate non-regression rule:** the four passing gates from J3 are the floor. Any arm that breaks one is excluded from the ship-candidate pool, regardless of how well it improved its target.

#### J-4.1 — Explicit Temporal Context Injection (architecture, single change)

- **Change scope:** `04B_model.py` — `JSeriesHybrid.__init__` adds `tod_emb=Embedding(48,d_model)` and `dow_emb=Embedding(3,d_model)` gated behind `enable_temporal_injection: bool=False`. `_arm2_fuse()` adds `fused_seq += tod_emb(slot_idx) + dow_emb(dow_idx)` after the existing concat+arm2_proj; existing one-hot DDAY_STRATA broadcast retained. `forward()` and `generate()` unchanged. `configs/J4_1.yaml` + smoke yaml inherits J3.yaml + `enable_temporal_injection: true`. `04D_train.py` + `04E_inference.py` MODEL_TYPE dispatch extended for `"J4_1"`. Output dir: `outputs_step4_J4_1/`.
- **Target:** tighten per-stratum AT_HOME variance (4.57 pp → ~3.5–4.0 pp) by giving Arm 2 explicit absolute-time anchors. Blueprint hypothesis: trunk PE diluted by Arm 2 fusion; "Sleep at 2 AM" vs "Sleep at 2 PM" indistinguishable for AT_HOME prediction without explicit time.
- **Rationale:** H_Time precedent in `archive/04B_model_H_Time.py` (lines 188–196) validates learnable PE + cyclical time on a different base class. J-4.1 re-implements the pattern inside `JSeriesHybrid._arm2_fuse()`. Param delta ≈ 19,584 (rounding error vs J3's 29.25M).
- **Architecture delta:** two `nn.Embedding` modules + two add-into-fused-tensor lines. No loss change. No head change. No Arm 1 change. Same lambdas as J3.
- **Smoke gate:** train_loss strictly decreasing eps 1–10; grad_norm finite throughout; home_loss not collapsing below J3's ~0.35 floor.
- **Advancement gate:** all 4 hard gates pass AND AT_HOME RMS ≤ 4.57 pp (no regression vs J3) AND ∆ ≥ 0.5 pp improvement.
- **Effort:** S (two embeddings + two lines + flag wiring + dispatch + 2 yamls + 3 job scripts).
- **Risk:** low. Worst case = zero net effect; gate stays at 4.57 pp (no regression, but ∆ < 0.5 pp → arm fails advancement gate, not the hard gates).
- **Fallback:** J-4.2 / J-4.3 outputs (parallel); J3 remains production if J-4.1 fails.

#### J-4.2 — Hierarchical home → cop conditioning (architecture, single change)

- **Change scope:** `04B_model.py` — `JSeriesHybrid.__init__` widens `cop_head` input from `d_model` to `d_model+1` when `enable_hierarchical_cop=True`. `forward()` and `generate()` re-order under the same flag: compute `home_logits` first → `home_probs = sigmoid(home_logits).detach()` → `cop_input = cat([fused_seq, home_probs])` → `cop_logits = cop_head(cop_input)`. Existing safety mask in `generate()` re-applied unchanged. `configs/J4_2.yaml` + smoke yaml inherits J3.yaml + `enable_hierarchical_cop: true` (no temporal injection — that's J-4.1's axis). `04D_train.py` + `04E_inference.py` dispatch extended for `"J4_2"`. Output dir: `outputs_step4_J4_2/`.
- **Target:** reduce Spouse |gap| (−2.03 → toward 0) and Alone cop_max_gap on 2005_1 (+21.1 pp) / 2010_1 (+17.1 pp). Blueprint hypothesis: P(spouse | home=1) >> P(spouse | home=0) and P(colleagues | home=1) ≈ 0; parallel head structure can't exploit these couplings without explicit hookup.
- **Rationale:** detach pattern mirrors the existing Arm-1 ↔ Arm-2 detach barrier — keeps cop-BCE gradient from leaking into home-head training.
- **Architecture delta:** widen `cop_head` input dim by 1; reorder head computation in `forward()` and `generate()`. Same lambdas as J3. Same Arm 1. Same `arm2_act_proj`. No loss-function change.
- **Smoke gate:** train_loss decreasing; grad_norm finite; home_loss does **not** collapse below 0.30 before ep 10 (head-saturation failure mode).
- **Advancement gate:** all 4 hard gates pass AND |Spouse| ≤ 2.03 pp (no regression) AND cop_max_gap ≤ J3's value.
- **Effort:** M (cop_head dim change + forward/generate reorder + flag wiring + dispatch + 2 yamls + 3 job scripts).
- **Risk:** medium — `home_logits` saturation can degenerate cop predictions. Mitigation: warmup ramp `alpha = min(1, epoch/10)` on the conditioning if `home_loss` collapses early. Watch first 10 epochs.
- **Fallback:** J-4.1 / J-4.3 outputs (parallel); J3 remains production if J-4.2 fails.

#### J-4.3 — Differentiable Logic Loss / PINN (loss, single change)

- **Change scope:** `04B_model.py` UNCHANGED (frozen J3 binary; dispatch only). `04D_train.py compute_loss()` after `cop_logits = output["cop_logits"]` adds `cop_probs = sigmoid(cop_logits)`, `p_alone = cop_probs[..., alone_idx]`, `p_others = cop_probs.sum(-1) - p_alone`, `loss_logic = (p_alone * p_others).mean()`; loss formula adds `+ LAMBDA_LOGIC * loss_logic`. New env var `LAMBDA_LOGIC` (default 0.0). `configs/J4_3.yaml` + smoke yaml inherits J3.yaml + `LAMBDA_LOGIC: 0.1`. `04D_train.py` + `04E_inference.py` dispatch extended for `"J4_3"` (model build identical to J3). Output dir: `outputs_step4_J4_3/`.
- **Target:** Alone cop_max_gap on 2005_1 / 2010_1 → toward ≤3 pp. Blueprint hypothesis: force mutual-exclusivity between Alone and the 8 non-Alone channels at training time so the network learns the structural rule in its weights, addressing the 2005/2010 colleagues=NaN asymmetry from the loss side instead of the data side (alternative to v2 §7's K-cop-A).
- **Rationale:** purely additive loss term. No model topology change. Soft constraint (λ=0.1) can't dominate BCE; can only nudge predictions toward mutually-exclusive states.
- **Architecture delta:** NONE. Same `JSeriesHybrid` build as J3. `04D_train.py compute_loss()` gets one new term; one new env var read.
- **Smoke gate:** train_loss decreasing; grad_norm finite; per-channel cop calibration MAE not exploding (>5× J3 in the smoke window = abort, drop λ to 0.05/0.01).
- **Advancement gate:** all 4 hard gates pass AND Alone cop_max_gap on 2005_1 / 2010_1 each ≤ J3's value AND no other cop channel regresses by more than 2 pp.
- **Effort:** S (one loss term + one env var + dispatch + 2 yamls + 3 job scripts; no model file edits).
- **Risk:** medium-low. Failure mode: model collapses to `p_alone ≈ 0.5, p_others_individually small` everywhere (minimizes product, BCE tolerates loosely) → breaks gates. Mitigation: monitor per-channel calibration; ramp λ from 0 over epochs 1–20 as fallback.
- **Fallback:** J-4.1 / J-4.2 outputs (parallel); J3 remains production if J-4.3 fails.

#### Parallel execution & advancement gate (one decision after all 3 finish)

All three runs submitted simultaneously to `pg` partition (3 sbatch jobs, separate output dirs, identical seed and train/val split). Wall time ~17 h. Single archive event before edits: `cp Speed_Cluster/04B_model.py Speed_Cluster/archive/04B_model_J3.py` and `cp Speed_Cluster/04D_train.py Speed_Cluster/archive/04D_train_J3.py` — all three arms fork from those frozen copies.

After all three eval jobs complete (`outputs_step4_J4_{1,2,3}/diagnostics_J_J4_{1,2,3}.json`):

- **0 of 3 pass:** ship J3 (current production); document residual axes; K-cop-A (v2 §7) becomes the data-side fallback for the Alone gap.
- **1 of 3 passes:** ship that arm. Document the other two as failed hypotheses.
- **2 or 3 of 3 pass:** ship whichever passing arm has the lowest composite. The remaining passing arms become candidates for a follow-up **J-4-stack** arm (single architecture combining the passing axes) — out of scope for this parallel cycle, separate decision afterwards.

#### Effort (combined)

M+ for the Sonnet build cycle (3 configs × 2 = 6 yaml files; 3 job scripts × 3 = 9 sh files; 2 architecture variants in `04B_model.py` controlled by `enable_temporal_injection` / `enable_hierarchical_cop` flags + frozen-J3 build for J-4.3; loss-function term in `04D_train.py` + new env var; 3 dispatch labels in `04D_train.py` and `04E_inference.py`).

#### Risks (combined)

- **R1 — GPU queue depth.** If `pg` has only 1 GPU free, the parallel framing collapses to sequential. Submit J-4.1 first (lowest risk), J-4.2 second, J-4.3 third.
- **R2 — Bundling temptation.** Each arm must change exactly one axis vs J3. **Resist** combining (e.g., temporal injection + hierarchical heads in one arm). Combining means we cannot attribute the win — the J2 / J2.5 / J3 cycle proved this matters.
- **R3 — J-4.2 home_loss saturation.** Hierarchical conditioning can collapse home predictions early. Smoke window watches for `home_loss < 0.30` before ep 10; abort and rerun with warmup ramp if seen.
- **R4 — J-4.3 degenerate-probability collapse.** Logic loss with constant λ=0.1 may push predictions to `p_alone≈0.5, p_others small`. Smoke watches per-channel calibration MAE; abort and drop λ if seen.
- **R5 — Diminishing returns.** J3 already crushes the composite gate (0.6355 ≪ 1.045). All three J-4 arms may improve marginal metrics without changing the publication-relevant headline numbers. This is acknowledged: the user has decided absolute precision is worth the GPU cycles regardless.
...
J2 / J2.5 / J3 full trains COMPLETE (2026-05-07). Official 04E→04J gate eval COMPLETE (2026-05-07): J2=3/4 (AT_HOME RMS=5.70 pp ❌, fails by 0.40 pp), J2.5=3/4 (AT_HOME RMS=6.03 pp ❌, worse than J1), J3=**4/4** (composite=0.6355 ✅, AT_HOME RMS=4.57 pp ✅, Spouse=−2.03 pp ✅, act_JS=0.0191 ✅). **FINAL VERDICT: SHIP J3. Step-4 COMPLETE.**
...
| 2026-05-07 | **J2/J2.5/J3 full trains COMPLETE — all three arms converged.** J1 full-train reference: best val_score=0.0171 (ep60), home_gap=2.56 pp. **J2** (job 913068, 29.1M params): early stop ep75, **best val_score=0.0173 at ep60** (val_JS=0.0035, home_gap=2.75 pp); LR decayed 5.00e-05→3.87e-05 via ReduceLROnPlateau; act_loss ep1→75: 1.505→0.451 (monotone decreasing); grad_norm stable 1.3–1.7, no spikes. **J2.5** (job 913069, 29.1M params): early stop ep75, **best val_score=0.0176 at ep60** (val_JS=0.0048, home_gap=2.58 pp); LR 5.00e-05→4.75e-05; act_loss ep1→75: 1.510→0.381; grad_norm stable. **J3** (job 913070, 29.25M params): early stop ep87, **best val_score=0.0166 at ep72** (val_JS=0.0035, home_gap=2.62 pp); LR 5.00e-05→4.07e-05→3.87e-05 (earlier decay — arm2_act_proj provides richer Arm-2 signal, allowing 12 more productive epochs); act_loss ep1→87: 1.475→0.369; grad_norm stable 1.2–1.6. **Ranking by val_score (lower is better):** J3 (0.0166) > J2 (0.0173) ≈ J1 (0.0171) > J2.5 (0.0176). Proxy home_gap all within 0.2 pp of J1 (2.56 pp baseline): J2.5=2.58 pp, J3=2.62 pp, J2=2.75 pp. **Proxy metrics are in tight band with J1** — whether any arm corrects the home_head collapse (σ=0.0 in 04J) and reduces AT_HOME RMS from 5.83 pp below the 5.3 pp gate requires official 04E→04J eval. Advancement rule: ship simplest arm that passes all 4 gates (J2 first, J2.5, J3). **Gate eval PENDING — eval jobs submitted.** | **FULL TRAINS COMPLETE — gate eval RUNNING** |
| 2026-05-07 | **J2/J2.5/J3 gate eval jobs submitted and RUNNING.** Three simultaneous eval sbatch jobs submitted to `pg` partition: job 913565 (J2_eval, cisr-1), job 913566 (J2_5_eval, cisr-2), job 913567 (J3_eval, speed-01). All three RUNNING at time of squeue check. Output target: `outputs_step4_J{2,2_5,3}/diagnostics_J_J{x}.json`. Gate check on completion: composite < 1.045, AT_HOME RMS ≤ 5.3 pp, Spouse ≤ 5 pp, act_JS ≤ 0.05. Advancement rule: ship simplest arm passing all 4 gates (J2 → J2.5 → J3); if 0 pass, ship J1 with documented AT_HOME caveat (composite=0.6927, 5.83 pp). | **GATE EVAL RUNNING — jobs 913565 / 913566 / 913567** |
| 2026-05-07 | **J2/J2.5/J3 official 04E→04J gate eval COMPLETE — J3 passes all 4 gates.** Official diagnostics from `outputs_step4_J{x}/diagnostics_J_J{x}.json`. **J2** (job 913565, cisr-1): composite=0.6884 ✅, AT_HOME RMS=**5.70 pp** ❌ (gate ≤5.3 pp; fails by 0.40 pp; marginal improvement from J1's 5.83 pp insufficient), Spouse=−1.47 pp ✅, act_JS=0.0239 ✅, cop_cal_MAE=0.226. 3/4 gates — λ_home 0.7→0.90 alone did not clear the AT_HOME blocker. **J2.5** (job 913566, cisr-2): composite=0.6857 ✅, AT_HOME RMS=**6.03 pp** ❌ (fails by 0.73 pp — worse than both J1 and J2; dropping home_head Tanh and replacing with GELU deepened the calibration miss), Spouse=−2.08 pp ✅, act_JS=0.0250 ✅, cop_cal_MAE=0.247. 3/4 gates — GELU replacement did not fix the home_head collapse. **J3** (job 913567, speed-01): composite=**0.6355** ✅, AT_HOME RMS=**4.57 pp** ✅ (gate ≤5.3 pp; margin 0.73 pp; −1.26 pp from J1's 5.83 pp), Spouse=−2.03 pp ✅, act_JS=**0.0191** ✅ (30% better than J1's 0.0274), cop_cal_MAE=0.235. **4/4 gates pass.** Root cause confirmed: `arm2_act_proj Linear(14→384)` before Arm-2 fusion corrected the dim-imbalance that suppressed the home_head gradient signal — projecting act_seq from 14-d raw probs to 384-d gave it comparable weight to the memory vector in the fusion concat. Aggregate AT_HOME gap for J3 is +4.19 pp (higher than J2's +3.11 pp) but per-stratum RMS is lower (4.57 vs 5.70 pp), confirming the projection balanced the signal across strata rather than just the mean. Spouse and act_JS both well within gate with comfortable margin. **FINAL VERDICT: SHIP J3. Step-4 COMPLETE.** | **CLOSED — SHIP J3** |
| 2026-05-07 | **J-4.1 / J-4.2 / J-4.3 bundle built + smoke PASS — all three arms cleared for full train.** Three single-axis arms forked from frozen `Speed_Cluster/archive/04B_model_J3.py` and `04D_train_J3.py`. Architecture archive event: `04B_model.py` → `archive/04B_model_J4.py`; `04D_train.py` → `archive/04D_train_J4.py` (one archive event covers all three arms per §2 rule). **J4_1** (job 913684, temporal injection — `tod_emb(48,384) + dow_emb(3,384)` additive in Arm2 fused_seq): 10 epochs × ~92 s/epoch; best val_score=0.2723 at ep8 (val_JS=0.2039, home_gap=0.1370); act_loss strictly decreasing; grad_norm finite; home_loss ~0.44 (no collapse ✅); tod_emb + dow_emb confirmed in model printout. **J4_2** (job 913685, hierarchical cop — `home_probs.detach()→cop_head`, cop_head in=385): 10 epochs × ~162 s/epoch; best val_score=0.2719 at ep8 (val_JS=0.1907, home_gap=0.1625); home_loss ~0.44 (≥0.30 saturation threshold ✅); ~2× epoch time vs J4_1 (sequential home→cop forward pass). **J4_3** (job 913686, PINN logic loss — `LAMBDA_LOGIC=0.1`, frozen J3 arch): 10 epochs × ~156 s/epoch; best val_score=0.2634 at ep6 (val_JS=0.1795, home_gap=0.1677); logic loss active (marg 0.031→0.029); grad_norm finite ✅. Smoke ranking (lower is better): J4_3 (0.2634) < J4_2 (0.2719) ≈ J4_1 (0.2723). All three full trains submitted simultaneously. **Cluster status:** job 913707 (J4_1), job 913708 (J4_2), job 913709 (J4_3) — all RUNNING on pg partition. | **SMOKE PASS — full trains running, jobs 913707 / 913708 / 913709** |
| 2026-05-08 | **J-4.2 and J-4.3 full trains COMPLETE — J4_1 still running.** J3 reference: best val_score=0.0166 (ep72), val_JS=0.0035, home_gap=2.62 pp. **J4_2** (job 913708, hierarchical cop, 29.25M params): early stop ep68 (patience=15); **best val_score=0.0161 at ep53** (val_JS=0.00422, home_gap=2.38 pp); LR decayed 5.00e-05→4.073e-05 via ReduceLROnPlateau; act_loss ep1→68: 1.955→0.399; grad_norm stable 1.3–1.6, no spikes. J4_2 narrowly beats J3 (0.0161 < 0.0166) with substantially lower JS (0.00422 vs J3 0.0035 — tight; home_gap 2.38 pp vs 2.62 pp). **J4_3** (job 913709, PINN logic loss, 29.25M params): early stop ep52; **best val_score=0.0183 at ep37** (val_JS=0.00731, home_gap=2.19 pp); LR 5.00e-05→4.287e-05; act_loss ep1→52: 1.929→0.462. J4_3 has better home_gap (2.19 pp) but worse JS (0.00731 vs J4_2 0.00422 — J4_2 JS 58% lower). val_score = JS + 0.5×home_gap: J4_2=0.0042+0.0119=0.0161 < J4_3=0.0073+0.0110=0.0183. **J4_2 leads J4_3 and J3** at full scale — hierarchical conditioning improved activity distribution (JS) more than PINN logic loss. J4_3 converged 16 epochs faster (52 vs 68). Proxy gate check (J4_2): val_JS=0.00422 ✅ (gate ≤0.05); home_gap=2.38 pp ✅ (gate ≤5.3 pp). Spouse and composite require 04E→04J. **J4_1** (job 913707): RUNNING, ~13 h elapsed at last check — results PENDING. Three-way comparison deferred until J4_1 completes. | **J4_2 COMPLETE (0.0161, leading) — J4_3 COMPLETE (0.0183) — J4_1 PENDING** |
| 2026-05-08 | **J-4.1 full train COMPLETE — all three J4 arms now done.** J3 reference: best val_score=0.0166 (ep72). **J4_1** (job 913707, temporal injection — tod_emb(48,384) + dow_emb(3,384) additive in Arm2 fused_seq, 29.27M params): early stop ep64 (patience=15); **best val_score=0.0171 at ep49** (val_JS=0.00467, home_gap=2.50 pp); LR decayed 5.00e-05→4.287e-05 via ReduceLROnPlateau; act_loss ep1→64: 1.475→0.099; grad_norm stable 1.5–2.4, no spikes. **J4 series full ranking (val_score, lower is better):** J4_2 (0.0161) < J3 (0.0166) < J4_1 (0.0171) < J4_3 (0.0183). J4_1 is marginally worse than J3 (0.0171 vs 0.0166); tod/dow injection yields a slight home_gap improvement (2.50 pp vs J3 2.62 pp) but worse val_JS (0.00467 vs J3 0.0035) — temporal embedding signal not decisive at full scale. J4_2 (hierarchical cop) is the only J4 arm that outperforms J3 on val_score. **Gate eval (04E→04J) to be submitted for all three J4 arms** to obtain official AT_HOME RMS pp, composite, Spouse, act_JS, cop_cal_MAE. | **ALL J4 ARMS COMPLETE — gate eval PENDING** |
| 2026-05-12 | **04F validation report regenerated for J3 — `step4_validation_report.html` updated (was H_Tanh/v4; now J3 production).** J3 training log pulled from cluster (`outputs_step4_J3/step4_training_log.csv`, 87 epochs). Inputs confirmed: observed=64,061 rows / synthetic=128,122 rows (IS_SYNTHETIC flag verified). **Section 1 (training curves): PASS** — loss convergent over 87 epochs, early stopping triggered, val_JS improved and plateaued. **Section 2 (activity JS): full PASS** — overall JS=0.0242, max per-cell JS=0.0308; all 12 cycle×stratum cells < 0.05 gate. No blocker. **Section 3 (AT_HOME rate): all 12 cells FAIL vs 04F threshold (2 pp)** — max |Δ|=9.69 pp (2022×Weekday), range 2.95–9.69 pp. Assessment: the 2 pp threshold predates the hard gates and is too tight for this model class; J3 shipped on AT_HOME RMS=4.57 pp (gate ≤5.3 pp) which it passed with margin. The 9.69 pp 2022×Weekday cell is a *real* known limitation — 2022 weekday AT_HOME baseline (~70.6%) is structurally higher than earlier cycles (62–65%) due to post-2020 remote work; J3 does not fully capture this shift (model saw it in training but the structural divergence exceeds the model's ability to separate cycle conditioning). **Not a blocker for Step 5 — J3 is the best available checkpoint** (all J-4 arms regressed further on AT_HOME). **Action for paper:** acknowledge in limitations section that synthetic population may underestimate post-2020 AT_HOME for weekday respondents; cross-reference T3 drift threat (`02_T3_census_gss_demographic_drift.md`). **Section 4 (temporal structure): INVESTIGATE** — transition rate ratio=157.95 (synthetic vs observed); act30 NaN count confirmed = 0 (no NaN artifact), so the high transition rate is genuine: J3 produces activity sequences with far lower temporal autocorrelation than observed diaries — the Transformer generates correct marginal distributions (JS PASS) but does not enforce within-diary activity persistence. This is a known limitation of non-autoregressive slot-parallel generation. Known Work/Sleep code-swap FAILs in §4.3 and §7.1/§6.2 are pre-existing bugs flagged in the val doc header — ignore. **Section 5 (co-presence): PASS**. HTML report confirmed at `outputs_step4/step4_validation_report.html` (366 KB, 8 sections). | **04F REPORT UPDATED FOR J3 — Step 5 NOT BLOCKED; paper limitation noted (2022×WD AT_HOME); §4 transition anomaly to investigate** |
| 2026-05-11 | **J-4.1 / J-4.2 / J-4.3 official 04E→04J gate eval COMPLETE — 0/3 arms pass all 4 gates. J3 remains the shipped checkpoint.** Official diagnostics from `outputs_step4_J4_{1,2,3}/diagnostics_J_J4_{1,2,3}.json`. **J4_1** (temporal injection — tod_emb + dow_emb): composite=**0.8247** ✅, AT_HOME RMS=**6.43 pp** ❌ (gate ≤5.3 pp; +1.86 pp worse than J3's 4.57 pp), Spouse=−3.42 pp ✅, act_JS=**0.0400** ✅, cop_max_gap=9.29 pp (Alone). **3/4 gates** — temporal embeddings regressed AT_HOME and roughly doubled act_JS without delivering the Blueprint's expected per-stratum tightening. **J4_2** (hierarchical home→cop, cop_head in=d_model+1): composite=**0.6578** ✅, AT_HOME RMS=**5.88 pp** ❌ (+1.31 pp worse than J3), Spouse=**−1.88 pp** ✅, act_JS=**0.0266** ✅, cop_max_gap=6.22 pp (Alone). **3/4 gates** — hierarchical conditioning gave the best val_score (0.0161) and the best Alone cop_max_gap among J4 arms (6.22 pp vs J4_1's 9.29 pp and J4_3's 8.89 pp), but still regressed the AT_HOME gate. **J4_3** (PINN logic loss, LAMBDA_LOGIC=0.1, frozen J3 arch): composite=**0.9449** ✅, AT_HOME RMS=**7.83 pp** ❌, Spouse=**−8.89 pp** ❌ (largest regression — logic loss collapsed spouse channel), act_JS=**0.0684** ❌, cop_max_gap=8.89 pp (Spouse). **1/4 gates** — the soft mutual-exclusivity penalty distorted the cop heads broadly rather than only suppressing the Alone 2005/2010 artefact. **Verdict per J-4 Tasks doc §2 hard-gate non-regression rule:** every J4 arm breaks the AT_HOME gate that J3 cleared (4.57 pp). Rollback applies to all three: **J-4.1 / J-4.2 / J-4.3 SHELVED. SHIP J3 (unchanged production checkpoint).** Best J4 arm (J4_2) has lower composite than J3 (0.658 vs 0.636) but loses the AT_HOME gate by +1.31 pp — not acceptable under the non-regression rule even though it improved Alone cop_max_gap from 7.04 pp to 6.22 pp. **Step-4 ladder fully closed: J3 (4/4) ships. Next: Step 5 — Census Linkage off the J3 production checkpoint** (was unblocked off J3 the whole time per J-4 Tasks doc §7). | **CLOSED — SHIP J3 (no change); Step 5 unblocked** |
