"""
04B_model_MDLM.py — Stage G: MDLM with intrinsic params + entity embedding support.

Backward-compatible with MDLM_C/F8 when no extra env vars are set.
Env-var knobs:
  ...
  USE_ENTITY_EMBED    — 1 = compress cond_vec via learned bottleneck (default 0)
"""
...
USE_ENTITY_EMBED   = int(os.environ.get("USE_ENTITY_EMBED", "0"))
ENTITY_EMBED_DIM   = int(os.environ.get("ENTITY_EMBED_DIM", "32"))
...
class MDLMHybrid(JSeriesHybrid):
    ...
    def __init__(self, config: dict):
        ...
        if USE_ENTITY_EMBED:
            ...
            print(f"  [MDLM] Entity embedding enabled: d_cond={d_cond} -> 64 -> {d_cond} (bottleneck)")
        else:
            self.entity_encoder = None

    def _encode_cond(self, cond_vec):
        if self.entity_encoder is not None:
            return self.entity_encoder(cond_vec)
        return cond_vec

    def _encode(self, act_seq, aux_seq, cond_vec, cycle_idx, tgt_strata=None):
        cond_vec = self._encode_cond(cond_vec)
        return super()._encode(act_seq, aux_seq, cond_vec, cycle_idx, tgt_strata=tgt_strata)

    def _arm2_fuse(self, memory, act_probs, cond_vec, cycle_idx, tgt_strata):
        cond_vec = self._encode_cond(cond_vec)
        return super()._arm2_fuse(memory, act_probs, cond_vec, cycle_idx, tgt_strata)
    ...
    def forward(self, batch: dict) -> dict:
        ...
        cond_vec  = self._encode_cond(batch["cond_vec"])
        ...
        clean_cond = self._encode_cond(batch["cond_vec"])
    ...
    def infer(self, act_seq, aux_seq, cond_vec, cycle_idx, tgt_strata,
        ...
        cond_enc = self._encode_cond(cond_vec)
        ...
        for step in range(N_DENOISE_STEPS):
            ...
            cls_tok    = self.cls_mlp(torch.cat([cond_enc, cycle_emb], dim=-1)).unsqueeze(1)
            cls_tok    = cls_tok + self.enc_pos_enc[:, :1, :]
            memory     = self.encoder(torch.cat([cls_tok, slot_emb], dim=1))
