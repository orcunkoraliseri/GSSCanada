#!/encs/bin/bash
#SBATCH --job-name=MDLM_G1
#SBATCH --partition=pg
#SBATCH --gres=gpu:1
#SBATCH --mem=40G
#SBATCH --time=48:00:00
#SBATCH --output=/speed-scratch/o_iseri/occModeling/logs/MDLM_G1_%j.out
#SBATCH --error=/speed-scratch/o_iseri/occModeling/logs/MDLM_G1_%j.err

# Stage G Trial 1: F8 winner (mask bounds [0.01, 0.99]) promoted to full data.
# F8 result on 10% sample: composite=0.771 (-16.3% vs control), COP max gap 4.94pp (-42%).
...
echo "=== ${TAG} TRAIN START (MDLM_MASK_LO=0.01, HI=0.99) ==="
