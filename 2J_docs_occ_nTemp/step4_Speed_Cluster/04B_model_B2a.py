"""
04B_model_B2a.py — G4_NAT_COP_HH: B2 + dedicated household conditioning in COP head.

Phase 8B-3 V1: single-axis fix over G4_NAT_COP (B2).

Root cause addressed:
  The NAT cop_head regresses to the population marginal because "married & home at
  night" vs "single & home at night" are identical in act+home space. The only
  discriminator (marital/household latent) reaches the head only faintly through
  the general cond_vec projection via arm2_proj.

Fix:
  Extract MARSTH (dims 9:15, 6-d) + HHSIZE (dims 15:20, 5-d) from cond_vec and
  add a dedicated per-slot projection arm2_hh_proj(11 → d_model) concatenated into
  the arm2 fusion before cop_head. The cop_head now receives an explicit, high-weight
  household signal per slot.

V1 finding: MARSTH and HHSIZE ARE present in cond_vec (d_cond=76, outputs_step4_G1).
  MARSTH: dims 9:15  (6 dims, marital status categories 1–6)
  HHSIZE: dims 15:20 (5 dims, household size categories 1–5)
  → V1 is BUILT (not BLOCKED).
"""
...
class G4NATCopHH(nn.Module):
    """
    Phase 8B-3 V1: G4_NAT_COP_HH — household conditioning in COP head.

    Identical to G4NATCopHybrid (B2) except:
      - arm2_hh_proj(11 → d_model): dedicated projection of MARSTH+HHSIZE dims
      - _arm2_fuse: appends hh_emb (broadcast from hh_dims per slot)
      - cop_head receives explicit household signal alongside general cond_vec

    Goal: cop_head can distinguish "married & home" from "single & home."
    """

    SPOUSE_IDX  = 1
    # cond_vec dim ranges for marital status and household size (d_cond=76 layout)
    MARSTH_START = 9
    MARSTH_END   = 15   # dims 9:15 → 6 dims
    HHSIZE_START = 15
    HHSIZE_END   = 20   # dims 15:20 → 5 dims
    N_HH_DIMS    = 11   # 6 + 5
    ...
    def __init__(self, config: dict):
        ...
        # V1 addition: dedicated projection for MARSTH+HHSIZE (11 dims)
        self.arm2_hh_proj   = nn.Linear(self.N_HH_DIMS, d_model)
        # arm2_proj input: slots_mem + act_emb + home_emb + cond_b + cycle_b + strata_b + hh_emb
        d_arm2_in = d_model + d_model + d_model + d_cond + d_cycle + 3 + d_model
    ...
    # ── NAT Arm-2: COP fusion (V1: +HH dims) ─────────────────────────────────

    def _arm2_fuse(self, memory, act_probs, home_probs, cond_vec, cycle_idx, tgt_strata):
        """
        Per-slot NAT fusion for COP prediction with dedicated household conditioning.
        ...
        # V1: dedicated per-slot broadcast of MARSTH + HHSIZE dims
        hh_dims  = cond_vec[:, self.MARSTH_START:self.HHSIZE_END]  # (B, 11)
        hh_emb   = self.arm2_hh_proj(hh_dims).unsqueeze(1).expand(-1, T, -1)  # (B, T, d_model)

        fused = torch.cat([slots_mem, act_emb, home_emb, cond_b, cycle_b, strata_b, hh_emb], dim=-1)
