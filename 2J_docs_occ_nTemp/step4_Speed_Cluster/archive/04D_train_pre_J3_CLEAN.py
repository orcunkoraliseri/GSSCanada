...
class Step4Dataset(Dataset):
    ...
    Activity + 9-channel co-presence targets: 1-of-K neighbor resampled each epoch.
    AT_HOME target (Phase 3 Lever B(ii), 2026-05-22): K-mean soft target — averages
    the K=5 neighbors' AT_HOME vectors per pair. Static across epochs (the K
    indices themselves don't move). Attacks the K=5 neighbor-disagreement JS floor
    of 0.1888 that bounded both Phase 2 arms (J3_DEMO, J3_DEMO_PSBLite cancelled
    2026-05-22). Cop kept as 1-of-K because averaging dilutes rare positives
    (Spouse/parents/friends/colleagues) below the 0.5 BCE decision threshold and
    would inflate Spouse Δ; activity kept as 1-of-K because averaging multi-class
    CE targets over-smooths and regresses act_JS.
    """

    def __init__(self, data: dict, pairs: dict):
        self.data  = data
        self.pairs = pairs
        # Precompute K-mean AT_HOME soft target per pair (static; K indices fixed).
        k_indices = pairs["tgt_k_indices"]              # (n_pairs, K)
        home_all  = data["aux_seq"][:, :, 0].float()    # (n_resp, 48) — feature 0 is AT_HOME
        self._home_soft = home_all[k_indices].mean(dim=1)  # (n_pairs, 48), values in {0, 1/K, 2/K, ..., 1}
    ...
    def resample(self):
        ...
        Applies to dec_act_seq and dec_aux_seq cop channels (1..9). The AT_HOME
        K-mean soft target in self._home_soft is independent of this draw."""
    ...
    def __getitem__(self, i):
        ...
        return {
            ...
            # Decoder target: sampled neighbor's diary (activity + cop)
            "dec_act_seq":    self.data["act_seq"][t],
            "dec_aux_seq":    self.data["aux_seq"][t],
            "dec_cop_avail":  self.data["cop_avail"][t],
            # AT_HOME target: K-mean soft across all K=5 neighbors (Lever B-ii).
            "dec_home_soft":  self._home_soft[i],
...
def compute_loss(output: dict, batch: dict, device,
    ...
    # AT_HOME target: K-mean soft target across K=5 neighbors (Phase 3 Lever B-ii,
    # 2026-05-22). Values in [0, 1] = {0, 1/K, ..., 1}. Falls back to 1-of-K
    # binary if the batch is from an older pair file without dec_home_soft.
    if "dec_home_soft" in batch:
        home_tgt = batch["dec_home_soft"].float()           # (B, 48) soft target
    else:
        home_tgt = batch["dec_aux_seq"][:, :, 0].float()    # (B, 48) binary fallback
    if HOME_LABEL_SMOOTH > 0.0:
        # Linear shrink toward 0.5 — compatible with soft targets (G2 / H3 fix).
...
def train(args):
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        for batch in train_loader:
            ...
            _clip_norm = 25.0 if MODEL_TYPE in ("I1", "J1", "J2", "J2_5", "J3", "J3_v2", "J3_v3", "J4_1", "J4_2", "J4_3", "J5_X1", "J5_X1b", "J5_A", "J5_B", "J5_F", "J_old", "J5_C") else 1.0
