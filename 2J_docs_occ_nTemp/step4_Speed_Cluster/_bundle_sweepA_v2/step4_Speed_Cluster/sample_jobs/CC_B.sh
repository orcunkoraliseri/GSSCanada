#!/encs/bin/bash
#SBATCH --job-name=CC_B
#SBATCH --partition=pg
#SBATCH --gres=gpu:1
#SBATCH --mem=40G
#SBATCH --time=48:00:00
#SBATCH --output=/speed-scratch/o_iseri/occModeling/logs/CC_B_%j.out
#SBATCH --error=/speed-scratch/o_iseri/occModeling/logs/CC_B_%j.err

# Phase 6 Stage B Trial 1: composite conditioning (FiLM + Fourier PE + per-stratum prefix).
# 20% stratified sample (~30k respondents), 100 epoch cap, patience 15, ~8h wall-clock on pg.
# Train -> infer -> 04H -> 04I -> 04J. Output: outputs_step4_sample/CC_B/
