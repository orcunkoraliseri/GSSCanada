#!/encs/bin/bash
#SBATCH --job-name=J2_eval
#SBATCH --partition=pg
#SBATCH --gres=gpu:1
#SBATCH --mem=40G
#SBATCH --time=4:00:00
#SBATCH --output=/speed-scratch/o_iseri/occModeling/logs/J2_eval_%j.out
#SBATCH --error=/speed-scratch/o_iseri/occModeling/logs/J2_eval_%j.err

# J2 archival gate evaluation: 04E -> 04H -> 04I -> 04J.
...
$PY 04H_diagnostics_cpu.py \
    --data_dir "$OUT" \
    --step3_dir "$S3" \
    --output_json "$OUT/diagnostics_H_J2.json" \
    --no_plot
...
$PY 04I_activity_copresence_diagnostics.py \
    --data_dir "$OUT" \
    --step3_dir "$S3" \
    --output_json "$OUT/diagnostics_I_J2.json" \
    --no_plot
...
$PY 04J_statistical_diagnostics.py \
    --data_dir "$OUT" \
    --step3_dir "$S3" \
    --output_json "$OUT/diagnostics_J_J2.json" \
    --no_plot

echo "=== J2 EVAL COMPLETE — check $OUT/diagnostics_J_J2.json for composite ==="
