"""
04B_model_B2c.py — G4_NAT_COP_SG: B2 + soft home gate for COP.

Phase 8B-3 V3: single-axis fix over G4_NAT_COP (B2).

Root cause addressed:
  The inference-only hard binary multiply (cop_prob[:,:,SPOUSE_IDX] *= gen_home)
  clips real couple co-presence at daytime/away slots. More fundamentally, the
  binary threshold discards the home-probability signal — cop_head sees only a
  0/1 gate rather than a continuous confidence.

Fix:
  Replace the inference-only hard binary multiply with a learned soft gate:
    - cop_head input: torch.cat([arm2_feat, home_probs.unsqueeze(-1)], dim=-1) — d_model+1
    - Training: home_probs = sigmoid(home_logits).detach() (continuous, already the case)
    - Inference: home_prob per slot = sigmoid(home_head output) — NOT thresholded to binary
      (line 316 of B2 thresholded; V3 keeps continuous)
    - The hard multiply cop_prob[:,:,SPOUSE_IDX] *= gen_home is REMOVED
    - gen_home returned is still binary (for posthoc consistency in 04E); cop_head
      receives the continuous sigmoid separately

  Mirror of J3's hierarchical-cop path (04B_model.py:1262-1265), adapted for the
  NAT branch: home_probs concatenated into cop_input rather than using a separate
  hierarchical cop_head flag.
"""
...
class G4NATCopSG(nn.Module):
    """
    Phase 8B-3 V3: G4_NAT_COP_SG — soft home gate for COP head.

    Changes from G4NATCopHybrid (B2):
      - cop_head input: d_model + 1 (appends continuous home_probs per slot)
      - Training: uses sigmoid(home_logits).detach() — continuous (was already the case
        via home_emb in arm2_fuse; V3 adds direct concatenation to cop_head)
      - Inference: collect per-slot continuous home sigmoid (not binary); remove hard multiply
      - gen_home returned as binary (for 04E posthoc consistency); cop_head sees sigmoid
    """
    ...
    def __init__(self, config: dict):
        ...
        # V3: cop_head receives arm2_feat (d_model) + continuous home_prob (1)
        self.cop_head = nn.Sequential(
            nn.Linear(d_model + 1, d_model), nn.Tanh(), nn.Linear(d_model, n_cop)
        )
    ...
    def _arm2_fuse(self, memory, act_probs, home_probs, cond_vec, cycle_idx, tgt_strata):
        """arm2_fuse identical to B2 — home_probs is continuous in both train and infer."""
        T = self.n_slots
        slots_mem = memory[:, 1:, :]
    ...
    def forward(self, batch: dict) -> dict:
        ...
        home_probs = torch.sigmoid(home_logits.detach())  # (B, 48) continuous
        ...
        # V3: concatenate continuous home_probs directly into cop_head input
        cop_input  = torch.cat([arm2_feat, home_probs.unsqueeze(-1)], dim=-1)  # (B, 48, d_model+1)
        cop_logits = self.cop_head(cop_input)
    ...
    def infer(self, act_seq, aux_seq, cond_vec, cycle_idx, tgt_strata,
        ...
        """
        AR loop for activity + AT_HOME, then NAT for COP with soft gate.

        Key differences from B2:
          - home_prob per slot is continuous sigmoid (NOT thresholded to binary)
          - cop_head receives arm2_feat + continuous home_prob
          - No hard binary multiply on Spouse
          - gen_home returned is binary (for 04E posthoc consistency)
        """
        ...
        gen_acts       = []
        gen_homes      = []
        gen_home_probs = []  # continuous sigmoid values for cop_head
        ...
        for t in range(self.n_slots):
            ...
            home_prob_t = torch.sigmoid(self.home_head(out_t).squeeze(-1))  # continuous
            home_tok    = (home_prob_t > 0.5).float()                        # binary for AR feedback

            gen_acts.append(act_tok)
            gen_homes.append(home_tok)
            gen_home_probs.append(home_prob_t)
        ...
        gen_act        = torch.stack(gen_acts,       dim=1)  # (B, 48) int64
        gen_home       = torch.stack(gen_homes,      dim=1)  # (B, 48) binary
        home_probs_seq = torch.stack(gen_home_probs, dim=1)  # (B, 48) continuous sigmoid

        act_probs = F.one_hot(gen_act, num_classes=self.n_act).float()
        # _arm2_fuse receives continuous home_probs (not binary gen_home)
        arm2_feat = self._arm2_fuse(memory, act_probs, home_probs_seq, cond_vec, cycle_idx, tgt_strata)

        # cop_head receives arm2_feat + continuous home_probs — no hard multiply
        cop_input = torch.cat([arm2_feat, home_probs_seq.unsqueeze(-1)], dim=-1)
        cop_prob  = torch.sigmoid(self.cop_head(cop_input))

        # No hard binary multiply: the soft gate is the learned gating mechanism
