"""
16CEN15GSS Profile Matcher Module

Matches Census 2016 agents with GSS 2015 schedules based on demographic profiles.
...
class MatchProfiler:
    ...
    def __init__(
        ...
        # Tier 1: All available matched columns (9 columns for 16CEN15GSS)
...
def main(sample_pct: float = 100.0) -> None:
    ...
    ALIGNED_DIR = BASE_DIR / "Outputs_16CEN15GSS" / "alignment"
    OUTPUT_DIR = BASE_DIR / "Outputs_16CEN15GSS" / "ProfileMatching"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    
    aligned_census = ALIGNED_DIR / "Aligned_Census_2015.csv"
    aligned_gss = ALIGNED_DIR / "Aligned_GSS_2015.csv"
    
    print("=" * 60)
    print("  16CEN15GSS PROFILE MATCHING")
    print("=" * 60)
    ...
    matched_keys_path = OUTPUT_DIR / f"16CEN15GSS_Matched_Keys{suffix}.csv"
    ...
    expanded_path = OUTPUT_DIR / f"16CEN15GSS_Full_Schedules{suffix}.csv"
    generate_full_expansion(df_matched, expander, expanded_path)
    
    # --- 7. Validate Matching Quality ---
    print("\n--- Step 7: Running Validation ---")
    validation_path = OUTPUT_DIR / f"16CEN15GSS_Validation{suffix}.txt"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Profile Matching: Census 2016 to GSS 2015")
    parser.add_argument(
        "--sample",
        type=float,
        default=5,
        help="Percentage of data to sample (1-100). Default: 25"
    )
