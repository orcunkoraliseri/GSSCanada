"""
06CEN05GSS Occupancy to BEM Conversion Module
...
def visualize_bem_distributions(df_bem: pd.DataFrame, output_dir: Path = None) -> None:
    ...
    # Define paths
    path_temporal = output_dir / "06CEN05GSS_BEM_temporals.png"
    path_nontemporal = output_dir / "06CEN05GSS_BEM_non_temporals.png"
...
def main(sample_pct: int = 5) -> None:
    """
    ...
    INPUT_DIR = BASE_DIR / "Outputs_Aligned" / "06CEN05GSS_HH_aggregation"
    OUTPUT_DIR = BASE_DIR / "Outputs_Aligned" / "06CEN05GSS_occToBEM"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    
    # Input file from HH_aggregation
    suffix = f"_sample{sample_pct}pct" if sample_pct < 100 else ""
    full_data_path = INPUT_DIR / f"06CEN05GSS_Full_Aggregated{suffix}.csv"
    
    # Output files
    output_path = OUTPUT_DIR / f"06CEN05GSS_BEM_Schedules{suffix}.csv"
    
    print("=" * 60)
    print("  06CEN05GSS OCCUPANCY TO BEM CONVERSION")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Occupancy to BEM Conversion: Census 2006 + GSS 2005"
    )
    parser.add_argument(
        "--sample",
        type=int,
        default=5,
        help="Sample percentage used in previous steps (default: 5)"
    )
